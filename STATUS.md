...
> Push verified: 2026-07-08 (Prepared the current deploy bundle for `origin/main`: shared-lib full Vitest green (60 files / 719 tests), plus focused overpayments rerun green after test mock typing cleanup (10/10); Credo full Vitest green (10 files / 43 tests); Finance full Vitest green (17 files / 138 tests); Cases focused Vitest green for referrer login, case detail/resend, and project path/directory coverage (7 files / 64 tests); `@zenowethu/ui` project destination-path Vitest green (14/14). `tsc --noEmit` clean for `packages/shared-lib`, `apps/cases` with 8GB heap, `apps/credo`, and `apps/finance`. `.claude/settings.local.json` and `tmp/*.log` intentionally left unstaged as local tool/runtime noise.)
