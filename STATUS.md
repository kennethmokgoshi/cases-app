...
> Push verified: 2026-07-09 (Prepared the Credo consent-link + DHS fee-email follow-up deploy bundle for `origin/main`: Credo auth/session plus consumer consent route Vitest green (20/20); shared-lib accepted-email, consent-reminder-email, and consent-service Vitest green (44/44); Cases DHS decline fee-email route Vitest green (4/4). `tsc --noEmit` clean for `apps/credo`, `packages/shared-lib`, and `apps/cases` with 8GB heap. `.claude/settings.local.json` and `tmp/*.log` intentionally left unstaged as local tool/runtime noise. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)
> Last updated: 2026-07-09 (**DHS decline fee-invoice email check added**: when a DHS decline reason mentions fees, owing, outstanding amounts, invoices, or payments, the Cases DHS decline panel now shows **Check invoice emails**. The new staff-only `POST /api/cases/[id]/dhs-decline/check-fee-emails` route validates input with Zod, blocks B2B partner access, avoids duplicate scan requests within 24 hours, records an internal timeline note and workflow log, and reports whether mailbox ingestion is configured. Verification: Cases focused route Vitest green (4/4); `apps/cases` `tsc --noEmit` clean with 8GB heap. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)
