...
> Push verified: 2026-07-08 (Prepared the current deploy bundle for `origin/main`: focused changed-surface Vitest green for shared-lib DHS/finance helpers (4 files / 56 tests), Credo consumer case APIs/helpers (4 files / 9 tests), Finance case financials (15/15), and Cases internal DRR readiness API (7/7), 87 tests total. `tsc --noEmit` clean for `packages/shared-lib`, `apps/cases` with 8GB heap, `apps/credo`, and `apps/finance`. Broad shared-lib Vitest was also attempted and reached 720/722 passing before two unrelated bcrypt-heavy password-policy tests hit Vitest's default 5s timeout in this shell. `.claude/settings.local.json` and `tmp/*.log` intentionally left unstaged as local tool/runtime noise. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)
