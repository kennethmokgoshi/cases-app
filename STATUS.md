...
> Last updated: 2026-07-30 (**Reporting App Dokploy Deployment & Dynamic PORT Binding Fix**): Resolved Dokploy Bad Gateway issue for `personal-projects-reporting-jm7r4o` (`Dockerfile.reporting`). Root Cause & Fix: (1) **Dynamic PORT Binding** — `apps/reporting/package.json` had hardcoded `"start": "next start -p 3008"`, which overrode Docker `ENV PORT` and caused Traefik/Dokploy (listening on 3000) to receive 502 Bad Gateway. Updated `"start"` to `"next start"`, allowing `next start` to dynamically bind to `process.env.PORT` (3000 or custom Dokploy port) like all other 7 apps in the monorepo. (2) **Monorepo Standalone Runtime Resolution** — Updated `Dockerfile.reporting` runner stage to copy built `node_modules`, `packages/`, and `apps/reporting` directory, running `cd apps/reporting && pnpm start` (matching the proven root `Dockerfile`). (3) **Fixed Vitest Mock Type Errors** — Fixed 5 TS2345 type errors in `apps/reporting` test files (`check-in`, `bulk-verify`, `logs`, `presence/sessions`) by properly casting `auth` mock calls (16/16 tests passing). (4) **Updated tsconfig.json Exclude List** — Excluded `"public"`, `"storage"`, `".next"`, `"dist"`, and `"out"`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.
