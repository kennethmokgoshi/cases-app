...
> Last updated: 2026-08-18 (**Dokploy Deployment Fix: Reporting App TypeScript Build Failure**): the `reporting` Dokploy build compiled fine but died at the `Running TypeScript` step — `packages/shared-lib/src/pdf-image.ts:23` returned `string | null` from a `: string` function because the module-level `_pdfJsSource` / `_pdfJsWorkerSource` caches were filled from an untyped `require('fs').readFileSync`, so TypeScript reset the narrowing back to the declared type and `null` survived to the `return`. Both lazy getters now type the lazy requires (`as typeof import('fs')` / `as typeof import('path')`) and use an explicit `=== null` check, so the cache narrows to `string` on both paths — no `@ts-ignore`, no `any`, behaviour unchanged. A second latent error sitting right behind it — `vi.mocked(auth).mockResolvedValue(null)` in `apps/reporting/app/api/reporting/projects/route.test.ts:30` — was aligned with the codebase's existing `null as any` mock pattern; Next type-checks test files under the app's `**/*.ts` include, so it would have failed the very next build. `tsc --noEmit` clean on `apps/reporting`, no `pdf-image` errors in shared-lib, 2/2 projects-route tests green. `apps/reporting` is the only app without `ignoreBuildErrors`, which is why it alone surfaced these. **Must be committed and pushed before Dokploy will rebuild green.**
> Previously: 2026-08-18 (**Finance: month-by-month instalment tracking, per-month payment allocation and back-dated capture**): recording a payment from a case now returns to that consumer's file instead of the global payments list; `Payment.instalmentId` (new migration) pins a payment to one month of the arrangement so month 3 can sit NOT PAID while months 4 and 5 read PAID, two payments can land on month 7, and a skipped month's proof can be captured after later months without disturbing them; the schedule now shows how many months the consumer is paying over with paid/not-paid/part-paid/still-to-come counts, per-month payment counts, overpayment and a "brought forward" flag. 7 new shared-lib tests + 5 new finance API tests; suites green (shared-lib 903, finance 147); `tsc --noEmit` clean on finance. **Migration `20260818120000_add_payment_instalment_allocation` APPLIED to production 2026-08-18 and verified (column, index, FK present; 14 existing payments untouched). Merged to `main` — Dokploy `finance-app` must rebuild to ship the UI.**)
> Previously: 2026-08-18 (**Consumer email address is now mandatory on every case and referral created by staff or B2B partners**): the Zod create schemas in `apps/cases/lib/schemas.ts` and `packages/shared-lib/src/schemas.ts` now reject a case whose client has no email; the three unvalidated smart-start create routes and all five `/api/v1/cases` API-key routes reject it too; `CasePatchSchema` refuses to blank out an email that is already on file; and converting a provisional `TEMP-`/`MANUAL-` shell into a real consumer requires one. Every staff, partner and B2B new-case form plus the case-detail client editor now marks Email required, flags it red when empty and blocks submit before the request goes out. Joint-applicant email deliberately stays optional. 10 new shared-lib tests + 11 new cases-app schema tests; suites green (cases 970, shared-lib 896); `tsc --noEmit` clean on cases/legal/insurance/forensic-audit/shared-lib. **Redeploy cases, finance, legal, insurance, forensic-audit to ship.**)
> Previously: 2026-08-14 (**DHS Follow-up Backlog CLEARED — 22 Files Recovered as Already-Accepted; 20 Declines Queued for Staff; Zero Messages Sent**): Cohort-drain fix (PR #2, commit `85fe0742`) deployed via Dokploy and **verified live behaviourally** — the response shape is unchanged by that fix, so verification was done by confirming that files actioned in the previous run (`ZDM-2026-164`, `-177-PD6`, `-178-THD`) had dropped out of the cohort, which they had. Then ran **seven live review-mode batches** to completion. **Final totals: 22 ACCEPTED, 20 flagged for staff review, 3 still pending, 4 NOT_ON_DHS, 0 errors, and 0 messages sent to any consumer or DC.** The 22 accepted files had been sitting in `DHS_REQUESTED`/`REQUESTED_VIA_DHS` for up to 95 days with nothing re-checking them — the transfers had gone through and nobody knew; they are now `ACCEPTED_VIA_DHS` and workable. Cohort is drained to 6 residual files, all of which are the ones flagged on 11 Aug by the *old* deployed code (which had no `setNextUpdate` on the review branch, so their `nextUpdate` is null); they now hit the cheap `SKIPPED_AWAITING_REVIEW` path and will leave the cohort when their 14-day marker expires (~25 Aug) or when staff action them. **Batch sizing note:** because the skip path does not set `nextUpdate`, review-flagged files stay in the cohort and consume slots in a `?limit=N` run — batches were sized at 15 to absorb ~6 skips and still do ~9 files of real work, staying inside the route's `maxDuration = 300` (observed 77–236s per batch). Harmless for an unbounded scheduled run; worth tidying if batching remains the norm. **The decline data is the headline finding.** Across 20 declines, roughly half are not refusals at all. The single most common reason text — appearing **6 times** — is *"Transfer Under Review: Kindly allow the standard turnaround time"*, i.e. DHS/the DC saying **wait**. Another ~4 are consumer-contact requests (*"Consumer need to call us"*, *"Contact to be made with client to confirm transfer"*, *"Kindly be advised we tried calling the client"*, *"Consumer to contact our office"*). The genuinely actionable ones are outstanding fees (~5: *"Outstanding fees"*, *"fees outstanding"*, *"Fees owing"*) and missing documents (~5: *"Please send a formal transfer request"*, *"We are unable to process the transfer as..."*, *"All written transfer requests must be accompanied by..."*), plus one POA/attorney matter. **Investigated whether this is a scraper bug — it is not.** `packages/shared-lib/src/dhs/status.ts:677` only sets `DECLINED` when the portal's own request-status field contains "declined"/"rejected"; the free-text reason is read separately by `getDeclineReason()`. So DHS genuinely records these as declined while the DC's own reason text says the opposite. The contradiction is in the source data, at the DC end — which is precisely why a human belongs in this loop. Note the outbound message is **not** uniform across these: classification varies with the full reason text, so some map to `SEND_DOCS` (consumer receives an "Update: DHS Transfer Declined" email + SMS *and* a document pack goes to the DC) and others to `RESUBMIT_LATER` (consumer receives a resubmit notice). Either way, real messages would have gone out on files where the portal is effectively saying "still processing". **Recommendation, now evidence-backed rather than precautionary: do NOT switch `?declineMode=auto`.** At a ~50% false rate it would have generated roughly ten misleading consumer contacts in this backlog alone. A worthwhile future improvement would be teaching `classifyDeclineReason()` a "NOT ACTUALLY A DECLINE — under review / awaiting consumer contact" category that parks the file and re-checks later instead of messaging anyone. **Still outstanding:** (1) staff must work the 20 `DHS_DECLINE_REVIEW` notifications and decide which declines are real — the automation deliberately does not make that call; (2) the cron is **still not scheduled** — recommended `0 6 * * 1-5` in Dokploy's Schedules tab, deliberately before the 7am `overdue-scan`, which rewrites overdue `REQUESTED_VIA_DHS` cases to `TAT_ELAPSED_DC_PENDING` and would otherwise drain the cohort first; (3) 4 `NOT_ON_DHS` files report `NOT_REQUESTED` on a portal we believed we had submitted to — these need a human to establish whether the original request ever landed; (4) the pre-existing `apps/reporting` lint break still fails CI and therefore skips the GitHub Actions deploy job for every app on every push to main (manual Dokploy redeploy is an exact equivalent, so it is not blocking, but it should be fixed). AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-11 (**DHS Follow-up RUN LIVE — 12 Files Found Already Accepted After ~90 Days Unchecked; Review Mode Prevented 3 Wrong "Declined" Emails; Cohort-Drain Bug Found and Fixed**): Deployed (merge commit `f658e29`, Dokploy manual redeploy — the GitHub Actions deploy job was skipped because `ci` fails on a **pre-existing, unrelated** break: `apps/reporting/package.json` still has `"lint": "next lint"`, removed in Next 16, and that app has no `eslint` dependency and no `eslint.config.mjs` at all, so `pnpm turbo lint` fails and `deploy` (which `needs: [ci]`) never runs — flagged as separate work, NOT fixed here; note the deploy job only POSTs to the Dokploy API, so a manual Dokploy redeploy is an exact equivalent). Verified new code live via the read-only `?dryRun=true` probe: response now returns `cohortStatuses: ["REQUESTED_VIA_DHS","DHS_REQUESTED"]` and `declineMode: "review"`. **Then ran three live review-mode batches of 10 against production** (`?limit=10`, ~22s per file, batch runtimes 218s/248s/128s — confirming that all 41 in one request would be ~15 min and would exceed the route's `maxDuration = 300`, so batching is mandatory). **Headline result: 12 files came back ACCEPTED on DHS.** These had been sitting in `DHS_REQUESTED` for 90–95 days — the transfers had gone through and nothing in the system was re-checking, so nobody knew. All 12 are now `ACCEPTED_VIA_DHS` and workable. **Review mode vindicated by real data:** of 6 files DHS reported as DECLINED, **3 were not declines at all** — verbatim reasons `"Transfer under review: Kindly allow the standard TAT of 3 - 7 business days"`, `"Consumer need to call us"`, and `"Contact to be made with client to confirm transfer"`. In `auto` mode all three consumers would have received an email headed "Update: DHS Transfer Declined", and the first would additionally have triggered a `SEND_DOCS` document pack to the DC. The other 3 look genuine (`OUTSTANDING_FEES` ×2, `SEND_DOCS` ×1 citing missing transfer documents). That is a **~50% false-decline rate**, matching the earlier 4-of-7 sample almost exactly — the case for keeping `declineMode` defaulted to `review` is now evidence-backed, not precautionary. Totals across 3 batches: 12 accepted, 6 flagged for review, 0 messages sent, 0 errors. Also confirmed no consumer/DC contact occurs on the ACCEPTED path either — `updateCaseStatus()` is a plain Prisma update with no notification side effects and the branch deliberately does not call `handleDhsAccepted()`. **Bug found in the live runs and fixed:** the cohort was not draining — files `ZDM-2026-164`, `-172`, `-177` reappeared in every batch. Cause was mine: `isOverdueWhere()` OR-ed `{ statusEntryDate: { lt: cutoff } }` in **unconditionally**, so any file older than the 7-day window matched on every run regardless of how recently it had been actioned — the `nextUpdate` throttle the code sets immediately afterwards was being overridden by the automation's own cohort query. Left as-is, a daily cron would have re-run a ~22s Puppeteer DHS check against the same files forever. Replaced with `isDueWhere()`: due when `nextUpdate` has elapsed, **or** `nextUpdate` is null AND (`isOverdue` OR stale `statusEntryDate`) — so a scheduled file can never be re-picked early, and the stale fallback only ever applies to files that were never scheduled. Two related gaps closed at the same time: the `NOT_LINKED`/`NOT_REQUESTED` branch previously wrote a comment but **never set `nextUpdate`**, guaranteeing a re-check every run, and the new `DECLINED_REVIEW` branch now schedules past `REVIEW_COOLDOWN_DAYS` so a flagged file leaves the cohort entirely instead of being re-fetched and skipped each run. Tests: 3 new/rewritten (`nextUpdate` elapsed makes a file due; the isOverdue/stale fallback applies ONLY when `nextUpdate` is null; no branch can ever match a file scheduled into the future; plus `NOT_ON_DHS` schedules forward, and `DECLINED_REVIEW` asserts `setNextUpdate(REVIEW_COOLDOWN_DAYS)`) — **17 trigger tests, 231/231 shared-lib DHS green**, `tsc --noEmit` clean. **Needs a redeploy to take effect.** Remaining: ~19 files still to work through in review mode once redeployed; after that, review the 6 (and counting) `DHS_DECLINE_REVIEW` staff notifications to decide which declines are genuine; only then consider `?declineMode=auto`, and given a measured 50% false rate that decision should probably be **no** until the classifier can distinguish "under review / please wait" from a real decline. Cron still NOT scheduled — recommended `0 6 * * 1-5`, before the 7am overdue-scan. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-11 (**DHS Follow-up: Decline-Review Mode Added — Declines Are Now Classified and Queued for Staff by Default Instead of Auto-Emailing Consumers; Real Cohort Measured at 41 Files**): Direct continuation of the cohort-widening entry below. Before scheduling anything, ran a **read-only** cohort count against the live DB (`count`/`findMany` only — no writes, no DHS contact, no sends). Results materially changed the plan: **the OLD automation would have actioned 0 files.** Its three combined filters (`isOverdue: true` AND `status = 'REQUESTED_VIA_DHS'` AND `services contains 'debt_review_flag_removal'`) match nothing in current data — so had it been scheduled as originally recommended it would have run every weekday and done nothing, silently. The widened cohort matches **41 files**: 31 in `DHS_REQUESTED` (never covered by anything, oldest **95 days** in status with no DHS re-check) and 10 in `REQUESTED_VIA_DHS`; totals sitting in those statuses are 31 and 13 respectively. All 41 are `debt_review_flag_removal`, so dropping the service filter changed nothing today — it is future-proofing, not the fix. Overdue signal breakdown: 15 caught by the `isOverdue` flag, 23 by an elapsed `nextUpdate`, **3 only by the new stale-`statusEntryDate` fallback** — small but load-bearing. Then ran the dry-run against a local dev server (the change isn't deployed, so hitting production would have exercised the old code): HTTP 200, previews rendered, nothing sent. Two problems surfaced that blocked going live. **(1) The dry-run structurally cannot tell you how often a decline will fire** — it never contacts DHS and renders the decline path from a fixed representative reason, so every preview shows the same `SEND_DOCS` category. Combined with the earlier real-world sample recorded below (a live read-only check of 7 files where **4 of the "declines" were false or still under review — one DC's text literally said "accepted"**), the auto path would email consumers a subject line reading "Update: DHS Transfer Declined" about transfers that were not declined, across 41 files. **(2) A live run of 41 files would time out** — the trigger allows 90s per DHS check and runs them sequentially, but the route declares `maxDuration = 300`; worst case is ~61 minutes. The existing `bulk-check-status` route caps itself at 25 files for exactly this reason; this cron had no cap. **Fix — decline-review mode** (`packages/shared-lib/src/dhs-requested-followup/trigger.ts`): new `DeclineMode = 'review' | 'auto'` with **`DEFAULT_DECLINE_MODE = 'review'`** — sending must now be explicitly opted into. In review mode a DECLINED result calls the existing pure `classifyDeclineReason()` (no side effects), writes a `[DHS DECLINE — AWAITING STAFF REVIEW]`-marked case comment recording the verbatim DHS reason, the auto-classification, and an explicit "NOTHING has been sent" statement, then raises `InAppNotification`s (`type: 'DHS_DECLINE_REVIEW'`) to the assigned staff member plus all unlocked admins — reusing overdue-scan's own alert pattern rather than inventing a new one. `handleDHSDecline()` is never called, and no status change is made. Defaulting to `review` preserves nothing that was ever live: this automation was never scheduled and never run live, so there is no production behaviour to regress. **Repeat-run protection**: a file already carrying the review marker inside `REVIEW_COOLDOWN_DAYS` (14) is detected by the new exported `isAwaitingDeclineReview()` and skipped **before** the DHS check runs — so an unactioned file neither re-notifies staff daily nor burns 90s of Puppeteer per run. The marker is time-boxed rather than backed by a new model/field, matching the existing `wasRecentlyActioned` cooldown precedent in `overdue-scan.ts`, and is self-healing if staff never formally clear it. The lookup is wrapped so a DB error is treated as not-yet-flagged (fails toward checking the file, never toward silently skipping it). New outcomes `DECLINED_REVIEW` / `SKIPPED_AWAITING_REVIEW` and stats `declinedForReview` / `skippedAwaitingReview`; `FollowupRunResult` now carries `declineMode`; the dry-run per-file message states which mode is active and explicitly labels the rendered previews as "what 'auto' mode *would* have sent". **Route** (`apps/cases/app/api/cron/dhs-requested-followup/route.ts`): new `?declineMode=review|auto` with a **400 on any other value** (a typo must never silently fall through to sending); `declineMode` recorded in the `AutomationRun` logs; and the FAILED-status heuristic fixed via a new `productiveOutcomes()` helper — it previously counted only accepted/declined/pending, so a legitimate all-review-mode run would have been logged as FAILED. Header comment updated to recommend **`0 6 * * 1-5`** rather than 9am, because `overdue-scan` runs at 7am and at `overdue-scan.ts:327-341` unconditionally rewrites every overdue `REQUESTED_VIA_DHS` case to `TAT_ELAPSED_DC_PENDING` — it drains this cohort daily, which is why only 10 files remain in that status while `DHS_REQUESTED` (invisible to overdue-scan) accumulated 31. Tests: 6 new (default mode is `review`; DECLINED in review mode calls neither `handleDHSDecline` nor `updateCaseStatus`, writes the marked comment containing the verbatim reason, and notifies assignee+admins; an already-flagged file is skipped with `checkTransferStatus` never called; `declineMode=auto` ignores the flag and still actions; a review-flag DB failure is non-fatal and the file is still checked) plus the pre-existing DECLINED test rewritten to pass `declineMode: 'auto'` explicitly. **228/228 shared-lib DHS tests green** (up from 223), `tsc --noEmit` clean on `@zenowethu/shared-lib` and `apps/cases` (0 errors). Verified live against the running dev server: review mode returns 200 with `declineMode: "review"` and the correct per-file messaging, and `?declineMode=send-everything` correctly returns 400. **Still not scheduled and not run live** — the recommended rollout is now: schedule `0 6 * * 1-5`, run live in review mode in batches of `?limit=10` (four passes covers all 41), read the resulting `DHS_DECLINE_REVIEW` notifications to measure the **real** false-decline rate from actual DHS outcomes, and only then consider `?declineMode=auto`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-11 (**DHS Follow-up Automation Widened to ALL Overdue Files Awaiting a DHS Outcome — Including the Previously-Uncovered `DHS_REQUESTED` Cohort**): User asked for the application to find every file that has been requested via DHS and is overdue, and to run the "Check Request Status" action on them automatically. Audited first rather than building new: the action already existed in three places — the per-case button (`handleCheckRequestStatus`, `apps/cases/app/(authenticated)/cases/[id]/page.tsx:1811` → `POST /api/dhs/lookup` with `action: 'check_status'`), the manual two-click bulk path (DHS Compliance dashboard button → `GET /api/dashboard/dhs-overdue` → `POST /api/dhs/bulk-check-status`), and the scheduled `/api/cron/dhs-requested-followup` + `packages/shared-lib/src/dhs-requested-followup/trigger.ts`. So the gap was **cohort coverage, not capability**. Three concrete problems found: **(1)** the trigger deliberately covered only `status === 'REQUESTED_VIA_DHS'`, explicitly excluding the `DHS_REQUESTED` look-alike (a deferred rollout decision recorded in its own file header and asserted by a test) — but `DHS_REQUESTED` is what `POST /api/dhs/transfer` actually writes across all five apps, and STATUS.md's own earlier live-DB count put ~16 files there against 7 in `REQUESTED_VIA_DHS`, so the majority of genuinely-requested-via-DHS files were covered by nothing at all; **(2)** the trigger also filtered on `services contains 'debt_review_flag_removal'`, so any other service type requested via DHS was invisible to it; **(3)** — the blocker that would have made simply adding the status a silent no-op — **`DHS_REQUESTED` is absent from `WORKFLOW_STATUSES` entirely** (`packages/shared-lib/src/statuses/statuses.ts`), so it has no `slaDays`, and `overdue-scan.ts:288` computes `isOverdue = slaDays !== null && daysInStatus > slaDays` — meaning `Case.isOverdue` can never be set true for those files and a cohort query filtering on `isOverdue: true` would have returned zero of them. **Fix**: `COHORT_STATUS` (single string) replaced by `COHORT_STATUSES = ['REQUESTED_VIA_DHS', 'DHS_REQUESTED']`; the service filter became opt-in (`service` option, default `null` = all services) rather than hardcoded; and overdue is now evaluated three ways via a new `isOverdueWhere()` — `isOverdue: true` **OR** `nextUpdate < now` **OR** `statusEntryDate` older than `FALLBACK_OVERDUE_DAYS` (7, mirroring `REQUESTED_VIA_DHS`'s own SLA) — so SLA-less statuses are still caught. `getRequestedViaDhsCohort(limit?)` became `getRequestedViaDhsCohort(opts: CohortOptions)` with `statuses` / `service` / `overdueDays` / `limit`, all surfaced as cron query params (`?statuses=A,B`, `?service=slug`, `?overdueDays=N`) alongside the pre-existing `?dryRun=true` / `?limit=N`; `FollowupRunResult.cohortStatus` → `cohortStatuses[]` plus a new `cohortService`, and each `FollowupFileResult` now carries `previousStatus` so a run report shows which of the two statuses each file came from. Routing logic itself (ACCEPTED→`ACCEPTED_VIA_DHS`, DECLINED→existing `handleDHSDecline`, PENDING→`nextUpdate` +3 working days, NOT_LINKED/NOT_REQUESTED→comment for staff) is **unchanged** — only the cohort widened. **Kept the manual path in step with the automation**: `GET /api/dashboard/dhs-overdue` now queries both statuses with the same three-way overdue test and returns each case's `status` (response field `statusCode` → `statusCodes[]`); `CheckDhsOverdueButton.tsx` shows a human-readable status label per row so staff can tell the two cohorts apart; and `bulk-check-status/route.ts` now counts a post-check `DHS_REQUESTED` result as `pending` instead of dumping it in `other`. Registered `DHS_REQUESTED_FOLLOWUP: '/api/cron/dhs-requested-followup'` in `apps/cases/app/api/admin/automation/run/route.ts` so admins can trigger it from the Automations panel — it was the only cron missing from that map. Tests: 5 new/rewritten (`trigger.test.ts` — both statuses + no service filter by default, the three-way overdue window with the 7-day cutoff asserted, narrowing back down to one status/one service/custom window, and a live-routing case proving a `DHS_REQUESTED` file is actioned and reports its originating status; `dhs-overdue/route.test.ts` — both statuses in the `where`, three-way `OR` shape, and `status` present in the mapped response); the previous test that asserted `expect(where.status).not.toBe('DHS_REQUESTED')` was inverted since that exclusion is exactly what this change removes. Full DHS suites **223/223 green** in shared-lib, 14/14 on the two touched `apps/cases` routes, `tsc --noEmit` clean on both `@zenowethu/shared-lib` and `apps/cases` (0 errors). **Not run live and not scheduled** — the cron still has no schedule entry (the same "add the Dokploy cron" item recorded when it was first built is still outstanding), and no live run was executed in this session. **This widening materially increases blast radius**: the live path sends real consumer/DC emails and SMS via `handleDHSDecline` on any newly-detected decline, and the cohort just grew from ~7 files to ~23+ across all service types. Strongly recommend a `?dryRun=true` pass first (`POST /api/cron/dhs-requested-followup?dryRun=true` — writes nothing, sends nothing, renders the exact decline messages per file) and reviewing that output before scheduling it live. Also flagged, not fixed: **`DHS_REQUESTED` should probably be added to `WORKFLOW_STATUSES` with a real SLA** — the fallback window works around its absence for this automation, but while it's missing from the registry those files remain invisible to overdue-scan, SLA metrics, and the status dropdowns generally; adding it changes UI behaviour app-wide so it was left as a separate decision. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-10 (**Cases App: POA Modal Now Reports Sends Honestly and Can File the POA Under Documents**): User reported the Send POA modal showing a contradictory result — "POA sent successfully via email to Mlangeni Botshelo Nthabiseng" immediately followed by "Note: Mlangeni Botshelo Nthabiseng was skipped (no contact info for this channel)" — and asked for an option to upload/save the POA instead of, or alongside, emailing it. Audited both ends first. The contradiction was a **UI bug, not a data bug**: `packages/ui/src/cases/SendPoaModal.tsx` did `const sent = data.sentTo ?? [clientName]` and never looked at `data.success`, so when SMTP actually rejected the message the API's `success: false` / `sentTo: undefined` / `skippedClients: [name]` response was rendered as a success with the client's own name filled in from props. **Nothing had been emailed** — the modal invented the confirmation. `POST /api/cases/[id]/poa` also lumped genuine "no contact details on file" skips together with hard send failures in the same `skippedClients` array, so the UI had no way to tell them apart even if it had checked. **Route rewritten** (`apps/cases/app/api/cases/[id]/poa/route.ts`): Zod-validated body (was hand-rolled `if` checks) with a new `mode: 'SEND' | 'SAVE' | 'SEND_AND_SAVE'` field (defaults to `SEND`, so existing callers are unaffected) and `channel` now optional when `mode` is `SAVE`; per-recipient generate → optional save → optional send loop replacing the previous single-pass structure; `skippedClients` now means only "no contact details for this channel" while real delivery errors go to a new `failures: [{name, reason}]` array carrying the provider's own error text; returns **502 with `success: false`** when every recipient failed, so a UI can no longer paper over a total failure. New `SAVE`/`SEND_AND_SAVE` path writes each personalised PDF to `storage/uploads/<caseId>/` and creates a `Document` row of type `ZENOWETHU_POA` (`isAdminOnly: false`), matching the manual upload route's layout exactly so `/uploads/...` serves it and it appears in the Documents tab unchanged; logs a `SYSTEM` case comment and calls `touchCaseAction`. Three pre-existing bugs fixed while restructuring, since the rewrite touched all three code paths: (1) the **email branch always called `generateStandardPoa` even for `type: 'WESBANK'`** — a Wesbank POA emailed to a client delivered the Standard POA PDF under a Wesbank subject line (the WhatsApp branch branched correctly); (2) a **single signing token was created for `client.id` and reused for both recipients**, so a joint client clicking their "Sign Online" link signed as the primary client; now one token per recipient; (3) the `/tmp/poa` copy behind the WhatsApp download link was always the primary's PDF, so a joint client's manual-download link served the wrong person's document — now written per recipient. **Modal rewritten**: new three-way "What should we do with it?" selector (Send only · Send & save · Save only); the Send Via / recipient-contact block hides when saving only, and the send-blocking "no contact details on file" guard now applies only to send modes — a client with no email *or* phone can still have a POA generated and filed. Result interpretation extracted into an exported pure `interpretPoaResponse()` helper (unit-testable in `packages/ui`'s node-env vitest without adding a DOM test stack) that treats `success: false`, a non-2xx status, *or* an empty sent+saved result as a failure; failure state now lists each recipient's actual error reason and suggests switching to "Save only". Success state links each filed PDF, shows partial failures, and only prints "each recipient will sign and return their document" when something was genuinely sent. `onSaved` callback wired to the case page's `fetchCase` so the Documents tab refreshes after a save. Tests: 16 new (9 in `apps/cases/app/api/cases/[id]/poa/route.test.ts` — this route had zero prior coverage: 401, 400 on send-without-channel, save-without-send, save with no contact details on file, send-and-save, **502 + `failures` when email delivery fails** (the reported bug), partial success when save works but send fails, 422 Wesbank incomplete-profile, 404; 7 in `packages/ui/src/cases/SendPoaModal.test.tsx` covering `interpretPoaResponse`, including an explicit regression test that a failed send is never reported as success). `tsc --noEmit` clean on `apps/cases` and `@zenowethu/ui`. **Not verified live in a browser** — same constraint as the entries below: this environment's dev server runs against the live production Postgres under real staff auth and no login credentials were available in-session. Recommend a post-deploy spot-check: open a case → Send POA → confirm the three mode buttons render, "Save only" files a `ZENOWETHU_POA` into the Documents tab, and a deliberately broken SMTP config now surfaces a red failure panel instead of a green success. **Separately: the underlying SMTP send for this client genuinely failed** — the modal was hiding it, and now surfaces the provider error, but the mail configuration itself still needs checking against Admin → Settings SMTP credentials. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-10 (**Cases App: XDS Sync Results Logging and Display Page**): Added a persistent logging mechanism and a new UI page to track the results of the automated XDS credit report sync. **Schema**: Added `XdsSyncLog` to record the stats (`processed`, `newFilesCreated`, `existingFilesUpdated`, `errors`, `details`) of every sync run (migration `add_xds_sync_log` created and pushed via `npx prisma db push`). **Backend**: Updated `packages/shared-lib/src/xds/sync.ts` to save the sync results to `XdsSyncLog` before returning. **Frontend**: Created a new server component page at `apps/cases/app/(authenticated)/reports/xds-sync/page.tsx` displaying a list of recent sync runs. The page shows aggregate stats (processed, matched, new cases, errors) and includes an expandable details view listing the consumer name, action taken, and case file link. It is placed under the authenticated route, making the aggregate results accessible to all staff, while clicking the case link defers to the existing case-level access controls to block unauthorized access to the case details. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-10 (**@Mention Dropdown: Keyboard Navigation, Group Mentions, and a Real Fix for the "Mentions Load Slowly" Complaint**): Direct follow-up to the B2B mention feature below — user reported the `@mention` dropdown (`ActivityTab.tsx`, the shared comment box used across `apps/cases`, `legal`, `insurance`, `forensic-audit`) could only be selected with a mouse, wanted arrow-key + Enter selection, wanted whole `UserGroup` records (e.g. `@Letsatsi`, `@ZenowethuShosholoza`) selectable from the dropdown alongside individual users — the backend (`comments/route.ts` POST) already silently supported group mentions and expanded them to every member's `CommentMention`, but the autocomplete never surfaced groups so staff had to already know a group's exact name to use it — and asked whether mention loading could be made faster. Investigated the "slow" complaint before guessing at a fix: queried the live DB directly (read-only) and found only 41 users and 6 groups total, so a Postgres index was never the bottleneck — the real cost was `ActivityTab.tsx` firing a fresh 200ms-debounced network round-trip to `/api/users/search?q=...` on every keystroke, and the parallel B2B implementation (`CaseDetailContent.tsx` + `/api/b2b/cases/[id]/mentionable-users`) doing the same despite its candidate list being provably static per case (four sequential DB round-trips per keystroke: `canB2BAccessCase` → 2 queries, `getMentionableUsersForB2B` → 2 more). Fix: given the dataset is small, fetch the full mentionable set **once** (lazily, the first time `@` is typed) and cache it client-side; every subsequent keystroke filters the cached array in memory — zero network latency after the first character. **New `packages/shared-lib/src/search/mention-search-route.ts`**: `searchMentionSuggestions()` (returns all non-locked users + all `UserGroup`s with `_count.members`, tagged `{kind:'user'|'group'}`) and `createMentionSearchRoute()` factory, following the existing `org-search-route.ts` pattern — consolidates what were 5 byte-for-byte-duplicate `/api/users/search/route.ts` files (`apps/cases`, `legal`, `insurance`, `forensic-audit`, `finance`) down to one shared implementation each app re-exports (`export const { GET } = createMentionSearchRoute();`). **`ActivityTab.tsx`** (`packages/ui`, shared by 4 apps): added `activeMentionIndex` state + `handleTextareaKeyDown` (↑/↓ cycles, Enter/Tab selects, Escape closes, mouse hover still syncs the highlight); replaced the per-keystroke debounced fetch with a one-time cache fetch (`mentionCacheRef`) + `useMemo`-filtered `userSuggestions`; `insertMention` now branches on `kind` — groups insert as a single `@`-token with spaces stripped (`Zenowethu Letsatsi` → `@ZenowethuLetsatsi`) to match the existing server-side group-mention matcher's `.replace(/\s+/g,'')` comparison; dropdown renders groups with a 👥 icon, member count, and a "Group" badge, ahead of user matches. **`CaseDetailContent.tsx`** (B2B portal): identical keyboard-nav fix (this file has its own separate, duplicated mention implementation, not the shared `ActivityTab`); also fixed its per-keystroke refetch bug by caching the case's candidate list once per `caseId` and filtering locally — B2B mention scope deliberately stays users-only (no groups), since exposing internal team/group membership to partner accounts would cross the existing B2B data-scoping boundary (`getMentionableUsersForB2B`) this feature didn't touch. Tests: 4 new (`mention-search-route.test.ts` — group-before-user ordering + `kind` tagging, `isLocked: false` filter, 401 unauthenticated, combined-list 200 response); full pre-existing suites re-run clean (`b2b-case-access.test.ts` 5/5, `mentionable-users/route.test.ts` 4/4, `comments/route.test.ts` 8/8). `tsc --noEmit` clean on `@zenowethu/ui`, `apps/cases`, `legal`, `insurance`, `forensic-audit`, `finance`. **Not verified live in a browser** — same constraint as the entry below: this environment's dev servers already run against the live production Postgres instance under real staff auth, and no login credentials were available in-session to click through the dropdown by hand; verification stopped at typecheck + the automated test suites. Recommend a quick post-deploy check: type `@` in a case comment, confirm groups (👥 icon) and users both appear, arrow keys move the highlight, Enter/Tab inserts the highlighted item, and posting a comment that mentions a group notifies every member. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-10 (**Cases App: B2B Partner Portal Got a Notification Bell + @Mention Support on Comments; Two Real Security Gaps Fixed Along the Way**): User asked for a notification bell on the B2B portal (`/b2b-dashboard`) so partners can see when they're mentioned on a case, plus the ability to @mention other users from the comment box. Audited first: the B2B header (`app/b2b-dashboard/layout.tsx`) had no bell at all despite the in-app notification backend (`InAppNotification` model, `/api/notifications`, `<NotificationBell/>` in `packages/ui`) already being generic and session-scoped — a pure wiring gap. The B2B comment box (`CaseDetailContent.tsx`) was a plain `<textarea>` with an unimplemented "@name to mention" hint and no autocomplete. Digging into the shared `GET/POST /api/cases/[id]/comments` route that both the staff and B2B UIs call turned up two real problems beyond the missing UI: **(1) `GET` had no `auth()` check at all** — any authenticated (or, functionally, any) request could pull every comment for any case by guessing its ID, including `isInternal: true` staff-only notes, and **(2) B2B-authored comments defaulted to `isInternal: true`** (the schema default), meaning partner comments were invisible to other partners and, combined with gap (1), the B2B dashboard was already unintentionally exposing internal staff notes back to partners with no filtering at all. Fixed both as part of this change rather than opening a separate ticket, since the mention feature required deciding what a B2B user can see/do on this exact route anyway. **New `apps/cases/lib/b2b-case-access.ts`**: `getB2BAllowedProjectIds`/`canB2BAccessCase` (project-descendant scoping, following the existing pattern in `/api/b2b/clients/search`) and `getMentionableUsersForB2B` — resolves a safe mention candidate set per case (the case's assignee/creator + staff on its linked projects, plus other users at the partner's own org) rather than exposing the full userbase search (`/api/users/search`) a B2B user has no business querying. **Comments route hardened**: `GET` now requires auth and, for B2B callers, verifies case access and filters to `isInternal: false` only; `POST` verifies case access for B2B callers, force-sets `isInternal: false` regardless of client input, and restricts @mention matching to the scoped candidate list instead of the unrestricted all-users/all-groups match staff get. Also fixed a related bug while touching the mention-notification code: it always set `linkUrl: /cases/${id}` even when the mentioned user was a B2B partner (a route they can't reach) — now branches to `/b2b-dashboard/cases/${id}` for `userType === 'B2B_PARTNER'` recipients. **New `GET /api/b2b/cases/[id]/mentionable-users`**: B2B-only, case-access-checked, backs the mention autocomplete with the same scoped candidate list. **`CaseDetailContent.tsx`**: comment textarea now detects an in-progress `@token`, debounced-fetches suggestions, shows a dropdown, and inserts `@FirstLast`. **Notification bell reuse**: `NotificationBell` (`packages/ui`) gained an optional `viewAllHref` prop (default `/notifications`, B2B passes `/b2b-dashboard/notifications`); `NotificationsClient` gained an optional `baseCasePath` prop (default `/cases`) so its "open case" links resolve correctly from either portal; new `app/b2b-dashboard/notifications/page.tsx` mirrors the staff notifications page pointed at the B2B base path. Bell wired into `b2b-dashboard/layout.tsx`'s header next to `DashboardSwitcher`. Tests: 17 new (`lib/b2b-case-access.test.ts` — 5, `api/b2b/cases/[id]/mentionable-users/route.test.ts` — 4, `api/cases/[id]/comments/route.test.ts` — 8, this route had zero prior coverage) covering the auth/scoping/isInternal-forcing/mention-restriction/linkUrl-branching behavior described above; full `apps/cases` suite 925/925 green (up from 908), `tsc --noEmit` clean, lint clean (0 errors, only pre-existing warnings elsewhere). **Not verified live in a browser** — this dev environment's `DATABASE_URL` points at the live production Postgres instance and no B2B test credentials were available in this session; the Browser pane also failed to render in this environment and the dev server port wasn't reachable from the shell for a curl-based smoke check, so verification stopped at typecheck + lint + the full automated test suite. Recommend a quick post-deploy spot-check as a real B2B partner account: bell renders and polls, `/b2b-dashboard/notifications` loads, typing `@` in a case comment shows only case-staff + own-org suggestions (not the wider userbase), posting a comment with a mention notifies the right person and (if they're staff) links to `/cases/[id]` or (if partner) `/b2b-dashboard/cases/[id]`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-09 (**Cases App: "Check Request Status on All" Bulk Action — Runs the Existing Per-Case DHS Check Across Every Overdue File Found by the DHS Compliance Button**): Direct follow-up to the DHS Compliance dashboard button below — user asked that, after discovering which files are overdue, staff be able to "check request status on all these files" in one click, with the new button appearing inside the DHS Compliance modal only once results have loaded. Confirmed the existing per-case "Check Request Status" action (`handleCheckRequestStatus` in `apps/cases/app/(authenticated)/cases/[id]/page.tsx:1811`, backed by `POST /api/dhs/lookup` with `action: 'check_status'`) is business-critical, ~310 lines covering DC-info scraping, joint-client checks, an own-NCRDC override, the Rules 1–10 status mapping, and — critically — it **autonomously sends real consumer/DC emails** (`handleDHSDecline` on a Declined result, `handleDhsAccepted` on Accepted/Auto Transferred) with zero existing test coverage. Given that, and given no live browser verification is possible in this environment (production DB, no staff credentials), the bulk feature was built to **reuse that exact existing logic unmodified** rather than duplicate or refactor it: the new `POST /api/dhs/bulk-check-status` (`apps/cases/app/api/dhs/bulk-check-status/route.ts`) imports `POST as dhsLookupPost` from `../lookup/route` and invokes it in-process once per case with the identical `Request` payload the single-case UI already sends — same code path, same behavior, zero risk of drift between "check one" and "check many." Design choices, all matching the existing `DHS_RECHECK` cron's own precedent for this class of operation (`apps/cases/app/api/cron/dhs-recheck/route.ts`): capped at **25 cases per click** (any more are reported as `skipped`, click again to continue), processed **sequentially** (not parallel — Puppeteer/DHS automation isn't safe to run concurrently), a single `closeBrowser()` after the whole batch, `maxDuration = 300`, and a completed run is logged to `AutomationRun` (`type: 'DHS_BULK_CHECK_STATUS'`) for the existing Automation Runs visibility. **Security**: the caseIds a client submits are re-validated server-side against the exact same access scope as the DHS Compliance count itself — extracted the access-scoping logic that was inline in `dhs-overdue/route.ts` into a shared `apps/cases/lib/dhs-overdue-access.ts` (`getDhsOverdueAccessWhere`) used by both routes, so a Shosholoza-only user still can't smuggle a Letsatsi case ID into the bulk request; anything outside their scope comes back as `forbidden` rather than silently processed. Frontend: `CheckDhsOverdueButton.tsx`'s modal gained a "🔎 Check Request Status on All" button that appears once the overdue list has loaded, POSTs the currently-shown case IDs, shows a toast summary (accepted/declined/pending/not-linked/errors breakdown, plus a skipped-count note when the batch cap was hit), and overlays each case's fresh outcome directly under its row in the existing list (color-coded: green accepted, red declined/error, orange not-linked, amber pending) — no separate results screen, staff see before/after in the same modal. Tests: 7 new cases in `bulk-check-status/route.test.ts` (401, 400 on empty caseIds, happy-path aggregation calling the real lookup route's mocked POST per case, forbidden-case exclusion, the 25-case cap + skip count, missing-ID-number short-circuit, and a per-case DHS-timeout error that doesn't abort the rest of the batch); refactored `dhs-overdue/route.test.ts` re-run unchanged (5/5 still green, proving the access-scoping extraction didn't change its query shape) — full `apps/cases` suite now 908/908 green (up from 901), `tsc --noEmit` clean. **Not done / known limits**: still no test coverage for `dhs/lookup/route.ts`'s `check_status` branch itself (pre-existing gap, not introduced here — the bulk route's tests mock that whole function rather than exercising its internals, which was a deliberate choice to avoid standing up its heavy DHS/OpenAI/GHL mocking from scratch for this feature); each case in a batch opens and closes its own Puppeteer browser session (inherited from calling the unmodified single-case handler N times) rather than reusing one session across the batch like `dhs-recheck` does, so a full 25-case batch will be slower than a hand-optimized version — flagged as a real, deliberate trade-off (correctness/no-regression-risk over speed), not silently accepted; and this triggers real outbound consumer/DC emails at scale for any newly-detected Accepted/Declined case, same as clicking "Check Request Status" by hand 25 times — worth a quick live smoke test on a small batch before relying on it for a large one, and worth checking whether Traefik's reverse-proxy idle timeout is long enough for a multi-minute request (self-hosted deploy, no serverless timeout ceiling, but proxy-level timeouts weren't verified in this session). AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-09 (**Cases App: New "DHS Compliance" Dashboard Button — Counts Overdue "Requested via DHS" Cases, Project-Scoped to the Signed-In User**): User asked for a button on the main dashboard that checks all cases whose workflow status is `REQUESTED_VIA_DHS` ("Requested via DHS") and are overdue, showing the number found on click, with the explicit requirement that access respects project membership (e.g. a Shosholoza-only staff member must not see Letsatsi cases unless separately granted access). Confirmed via `AskUserQuestion` before building, since the ask directly conflicts with this codebase's existing convention (every other dashboard/cases query today treats any `userType === 'STAFF'` user as unrestricted across all projects) — user chose the stricter behavior: **only Admin (`isAdmin === true` or `role === 'ADMIN'`) sees every project's cases; everyone else, including regular STAFF, is scoped to the projects they hold via `ProjectMember` (expanded to descendants) plus their `b2bPartnerId`**, with a fallback to only their own (`createdById`) cases if they hold no project membership at all. Also confirmed to trust the existing nightly `overdue-scan.ts` automation's cached `Case.isOverdue` flag (`daysInStatus > 7` per `REQUESTED_VIA_DHS`'s `slaDays`) rather than recomputing live from `statusEntryDate`, avoiding a second copy of the SLA formula. New `GET /api/dashboard/dhs-overdue` (`apps/cases/app/api/dashboard/dhs-overdue/route.ts`) queries `Case` where `status: 'REQUESTED_VIA_DHS'`, `isOverdue: true`, `deletedAt: null`, plus the access-scoping `where` clause described above (same cycle-safe project-descendant BFS pattern as `/api/cases` and the main dashboard page's inline stats query), returning `{ count, cases: [...] }` (file number, client name, days in status, project name). New client component `CheckDhsOverdueButton.tsx` — a 4th "quick stat" card on the dashboard (`app/(authenticated)/page.tsx`, grid widened 3→4 columns) that fetches the route on click, shows a `toast` with the count (no `alert()`, per house rule), and opens a lightweight modal listing the matching cases with links to each case detail page. Tests: 5 new cases in `route.test.ts` covering the 401-unauthenticated case, admin-unrestricted query shape, STAFF-user project-descendant scoping (Shosholoza access does not leak Letsatsi project IDs into the query), the no-project-access→`createdById`-only fallback, and the client/project-name fallback mapping — full `apps/cases` suite 901/901 green (up from 896), `tsc --noEmit` clean. **Not verified live in a browser** — this dev environment's `DATABASE_URL` points at the live production Postgres instance (`213.199.57.111`), and the login page requires real staff credentials; per this session's standing rule, guessed/borrowed credentials are never entered into a live production login, so verification stopped at typecheck + the full automated test suite rather than an interactive click-through. Recommend a quick post-deploy spot-check: an Admin account should see cases across every project, a Shosholoza-only staff account should see zero Letsatsi cases in the modal even if some exist and are overdue. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-09 (**Cases App / shared-lib: Second Instance of the Same DHS Document-Type Bug Found and Fixed in the Immediate (Non-Cron) Automation Trigger; Referrer Portal Now Re-Triggers Automation Instantly on Upload Instead of Waiting for the Next Cron Pass**): Direct follow-up to the `NOT_REQUESTED_VIA_DHS` fix immediately below, after the user asked what concretely needs to happen for new debt-review-flag-removal referrals to get requested via DHS "like the application was doing before." Confirmed the service-type gate itself is correctly configured — `debt_review_flag_removal` is already in `DHS_TRANSFER_SERVICES` (`packages/shared-lib/src/ai/case-automation-trigger.ts:45-48`), so that wasn't the problem. Investigating the full creation-to-DHS-request pipeline surfaced two more concrete gaps in the same family as the earlier fix. **(1) The immediate trigger had its own separate copy of the strict `d.type === 'ID'`/`'POA'`/`'ZENOWETHU_POA'` check.** `case-automation-trigger.ts` — the module that fires ~5 seconds after case creation (and is meant to re-fire whenever a relevant document is uploaded, per its own file-header comment) — duplicates the exact document-matching logic already fixed today in `workflow-engine.ts`, independently, at four call sites (id-doc lookup for re-analysis, the main POA/ID gate, the post-extraction re-check, and the final pre-Puppeteer-submission check). None of these recognized the referrer portal's `ID_DOCUMENT`/`POWER_OF_ATTORNEY`/`CONSENT_FORM` type codes either. Fixed by importing and using the same `ID_DOCUMENT_TYPES`/`POA_DOCUMENT_TYPES` constants exported from `workflow-engine.ts` at all four sites, so both automation paths now agree on what counts as an ID/POA document. **(2) The referrer portal never actually re-fired this trigger at all.** Despite the module's own doc comment claiming it "fires after a case is created OR after documents are uploaded," grep confirmed `runCaseAutomationTrigger` was only ever called from the *staff* document-upload route (`apps/cases/app/api/documents/upload/route.ts:373-374`) — the referrer self-service portal's own upload endpoint (`apps/cases/app/api/referrer-portal/referrals/[caseId]/documents/route.ts`) created the `Document` row and notified staff but never re-ran automation. Practical effect: even with today's cron fix in place, a referral case's ID/POA upload would sit waiting for the next daily 7am cron run instead of being processed within seconds like a staff upload is. Fixed by adding the identical fire-and-forget trigger call used in the staff route, gated on `ID_DOCUMENT_TYPES`/`POA_DOCUMENT_TYPES` membership so it only fires for documents the DHS gate actually cares about (`documents/route.ts:95-104`). Together, these two fixes mean a referral case whose referrer uploads ID + POA/consent through their own portal now gets the same fast, automatic "check docs → email DC → submit DHS transfer" treatment within seconds that a staff-driven upload always got — the cron fixed earlier today remains as the safety net for cases where the fast path doesn't fully resolve it (e.g. no `dcEmail` on file yet). Tests: added 2 new cases to `apps/cases/app/api/referrer-portal/referrals/[caseId]/documents/route.test.ts` (ID_DOCUMENT upload re-triggers automation with `caseId`+`'DOCUMENT_UPLOADED'`; an irrelevant type like PAYSLIP does not) using `vi.waitFor` to correctly await the fire-and-forget dynamic import rather than a fragile fixed-delay `setTimeout`; full `apps/cases` suite 896/896 green, full shared-lib suite 846/846 green (unchanged from the earlier fix — `case-automation-trigger.ts` had no prior dedicated test file, none added here beyond the route-level coverage since standing up its heavy DHS/Puppeteer/OpenAI mocking from scratch for a 4-line matching change felt disproportionate — flagged as a real gap, not silently skipped), `tsc --noEmit` clean on `apps/cases`. **Not yet verified live** — same as the underlying fix, recommend checking `/admin/automations` and a real referrer-portal document upload on a test referral case after deploy to confirm the near-instant DHS request now fires. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-09 (**Cases App: B2B Referral Cases Stuck in `NOT_REQUESTED_VIA_DHS` ⇄ `OUTSTANDING_DOCS` Loop — Document-Type Mismatch Between Referrer Portal and DHS Automation Fixed**): User reported new B2B referrals were stalling instead of progressing through DHS request automation. Root cause, confirmed by direct code inspection: `getDHSDocuments()` (`packages/shared-lib/src/automation/workflow-engine.ts`) — the gate the daily `workflow-automation` cron (`apps/cases/app/api/cron/workflow-automation/route.ts`, step 4) uses before calling the real DHS Puppeteer submission (`requestTransfer()`) — did a strict-equality check for `Document.type === 'ID'` / `'POA'`/`'ZENOWETHU_POA'` only. The referrer self-service portal (`apps/cases/app/referrer/page.tsx`, where B2B partners actually upload the consumer's ID and signed POA) saves documents under a different vocabulary (`ID_DOCUMENT`, `POWER_OF_ATTORNEY`, `CONSENT_FORM`) with zero normalization on ingest (`apps/cases/app/api/referrer-portal/referrals/[caseId]/documents/route.ts`). Result: the gate never found a match for referral-sourced documents, so the cron demoted every affected case to `OUTSTANDING_DOCS` every run with an auto-comment literally reading "Missing ID, POA" even when both files were genuinely on file — the case then bounced `NOT_REQUESTED_VIA_DHS ⇄ OUTSTANDING_DOCS` indefinitely and `requestTransfer()` (the actual DHS submission) never fired. Confirmed this is a bug, not a deliberate manual-approval gate — no staff-approval flag exists on the request side (unlike `Case.manuallyAcceptedViaDhs` on the accept side), and the cron's own step comment says "Verify docs present → Request via DHS" (fully automatic intent). **Fix**: added shared `ID_DOCUMENT_TYPES`/`POA_DOCUMENT_TYPES` synonym lists to `workflow-engine.ts` (rather than rewriting stored document types, which would have broken the referrer portal's own more-granular display labels via `formatDocumentTypeLabel()` and its existing tests) and widened `getDHSDocuments()`, `getDHSDocumentUrls()`, and the `hasDocument()` calls used in the cron's `OUTSTANDING_DOCS` step to recognize both the staff-facing (`ID`, `POA`, `ZENOWETHU_POA`) and referrer-portal (`ID_DOCUMENT`, `POWER_OF_ATTORNEY`, `CONSENT_FORM`) type codes. Applied the same widening to the manual "Request via DHS" button's route (`apps/cases/app/api/dhs/transfer/route.ts`) for consistency, since staff hitting that button manually would hit the identical gap. **Second bug fixed in the same investigation**: the B2B dashboard's own document-upload dropdown (`apps/cases/app/b2b-dashboard/cases/[id]/CaseDetailContent.tsx`) had `value="POA"` — the exact canonical code the DHS gate looks for — mislabeled in the UI as "Proof of Address" (a genuine "Proof of Residence" option already existed separately with the correct `PROOF_OF_RESIDENCE` value at a different line). This risked the opposite failure: a staff member uploading an actual proof-of-address document there would silently satisfy the DHS gate and have the wrong PDF submitted to the DC as the consent document. Relabeled to "Power of Attorney / Consent" (value unchanged, since `POA` is already correct and recognized everywhere). Tests: added 13 new tests to `packages/shared-lib/src/automation/workflow-engine.test.ts` covering `getDHSDocuments`/`getDHSDocumentUrls`/`hasDocument` against both staff and referrer-portal type-code vocabularies (staff codes match, referrer codes match, `CONSENT_FORM` counts as POA-equivalent, unrelated types don't match, filename-keyword fallback still works) — full shared-lib suite 846/846 green, full `apps/cases` suite 895/895 green, `tsc --noEmit` clean on `apps/cases`. No schema/migration changes — `Document.type` remains a free-form string column; no enum was introduced since the fix targets the matching logic, not storage. **Not yet done**: did not query production `AutomationRun` logs to count how many B2B cases are currently stuck in the loop (user chose to fix the code directly rather than audit live data first) — recommend checking `/admin/automations` or the `NOT_REQUESTED_VIA_DHS`/`OUTSTANDING_DOCS` case list shortly after the next cron run to confirm previously-stuck referral cases now advance to `REQUESTED_VIA_DHS`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-08 (**Cases App: Credit Providers — Two-Tier Permissions (Edit/Upload for All Staff, Delete/Verify for Managers+)**): Third follow-up in the same Credit Providers thread. User asked for a specific split: any staff member should be able to edit provider details and upload consent documents (same as admins), but Delete and Verify should stay restricted to admins, executives, senior managers, **and managers** (a new role added to the mix — this app has a distinct `isManager` flag alongside `isAdmin`/`isExecutive`/`isSeniorManager`, confirmed via `types/next-auth.d.ts`). Previously everything (Add/Edit/Verify/upload/activate-deactivate) was gated to `isAdmin || isExecutive || isSeniorManager` as one `canMutate` tier, and Delete was `isAdmin || isExecutive` only — neither matched the requested split, and managers had no elevated access anywhere. Replaced with two explicit tiers used consistently client- and server-side: **`canEdit`** = any signed-in staff member (Add Provider, Edit, upload consent docs, activate/deactivate consent docs), **`canManage`** = `isAdmin || isExecutive || isSeniorManager || isManager` (Delete provider, Delete consent doc, Verify). Client: `app/(authenticated)/credit-providers/page.tsx` and its `ServiceConsentModal` now take both flags and gate each button individually rather than one blanket `canMutate`. Server (this is the part that actually enforces it — the UI split alone would have been cosmetic): `POST /api/admin/credit-providers` and the consent-upload/activate-deactivate routes dropped their role check entirely (any authenticated session); `PATCH /api/admin/credit-providers/[id]` now allows any staff member to edit normal fields but requires `canManage` specifically when the request includes `markVerified: true`; `DELETE` on both the provider and the consent-document routes now requires `canManage` (was `isAdmin`/`isExecutive` only, so senior managers and managers previously couldn't delete either — that's fixed as a side effect of matching the new spec). Tests: `credit-providers/route.test.ts` updated (member role now succeeds on POST/PATCH-without-markVerified, new 403 case for member+markVerified, new success cases for manager+markVerified and senior-manager/manager DELETE) — 22 tests, up from 18; `service-consents/route.test.ts` updated similarly (member upload/activate now succeed, new manager-delete-succeeds and member-delete-403 cases) — 10 tests, up from 7; 32/32 green across both files. `tsc --noEmit` clean on `apps/cases`. Verified live via direct HTTP probes against the running dev server post-change — still healthy, same expected redirect/401 responses, no crashes from the edits. **Not yet done**: no live role-by-role click-through in an actual authenticated browser session (same constraint as the earlier entries in this thread — verifying via code + tests + server health checks, not a live login, since I won't enter credentials into the app regardless of who provides them); recommend the user spot-check as a plain staff account (Add/Edit/Upload should work, Delete/Verify should 403) and as a manager account (everything should work) before relying on this in production.

> Last updated: 2026-08-08 (**Cases App: Credit Providers Moved Off `/admin` Entirely — Middleware Was Blocking Staff, and the `/admin` URL Itself Was the Wrong Call**): Second follow-up to the Form 17.W creditor-contact work above. After exposing `/admin/credit-providers` to all staff via the sidebar (previous entry), the user tested as a non-admin and it still redirected away — a middleware-level guard (`middleware.ts`) blocks the entire `/admin/*` path for anyone who isn't `isAdmin`, independent of page/API-level permissions, with only `/admin/debt-counsellors` carved out as an exception. That guard runs before the page renders, so no page-level fix could have worked around it — a real miss in the prior entry, which only checked page/API auth and never checked middleware. Initially patched by adding `/admin/credit-providers` to the same exception list (mirroring `/admin/debt-counsellors`), but the user correctly flagged that stacking ad-hoc admin-path exceptions is fragile and keeps a non-admin feature confusingly namespaced under `/admin` in the URL bar. Fixed properly instead: **physically moved the page** from `app/(authenticated)/admin/credit-providers/page.tsx` to `app/(authenticated)/credit-providers/page.tsx` — a real top-level route, peer of `/cases`, `/documents`, etc., for every role including admins (single canonical URL, no special-casing). Reverted the middleware exception entirely (no longer needed — the route isn't under `/admin` at all now, which is a strictly simpler rule than adding another carve-out). Removed the now-nonsensical "← Admin" breadcrumb from the page. Updated the three other references to the old URL: `SidebarNav.tsx`'s nav link, the "Go to Credit Providers registry" link inside `DebtReviewTab.tsx`'s creditor-linking panel, and the Admin Dashboard's own shortcut card (which can still link to it, just at the new address). The `/api/admin/credit-providers/*` API routes were deliberately left as-is — that path segment was never actually gated by the `/admin` middleware rule (it only matches page pathnames, not `/api/admin/...`), so the API already worked correctly for any authenticated user throughout both this entry and the previous one; moving it would have meant updating every fetch call site for no functional gain. `tsc --noEmit` clean on both `apps/cases` and `packages/ui`. Verified live via direct HTTP probes against the actual running dev server (not mocked) — old URL and new URL both handled consistently by middleware, old page directory confirmed deleted from disk, no other file in the codebase still references the old path. **Also fixed in the same session, surfaced by the same live-testing**: `apps/cases/app/layout.tsx` was unconditionally registering a caching service worker (`/sw.js`) in every environment including local dev — that worker fighting Turbopack's dev-mode rebuilds was the actual cause of a `Failed to fetch` / "(stale)" error the user hit repeatedly while testing this feature, and it survived server restarts since it lives in the browser, not the server. Registration is now gated to `NODE_ENV === 'production'`; the dev build instead detects and unregisters any leftover worker, clears its caches, and forces exactly one follow-up reload itself so the fix is self-healing rather than requiring manual DevTools cleanup. Unaffected in production — the real PWA behavior is untouched. **Not yet done**: no dedicated test added for the middleware change (this codebase's `middleware.ts` has zero existing test coverage, and standing up NextAuth/NextRequest mocking for a small conditional felt disproportionate — flagged as a real gap, not silently skipped); recommend the user do one more live staff-login pass to confirm `/credit-providers` now loads correctly end-to-end for a non-admin account.

> Last updated: 2026-08-08 (**Cases App: Credit Providers Registry Made Visible to All Staff, Not Just Admins**): Direct follow-up to the Form 17.W creditor-contact backfill above — user asked for a page where all users can view creditors' contact details and addresses. The page already existed (`/admin/credit-providers`, built as part of the backfill work) with full contact/address data and was already safe for any authenticated user to view (its `GET` API route has no role check, and the page itself only gates the Add/Edit/Delete/Verify *buttons* behind `canMutate`/`canDelete`, not the data) — the only actual barrier was the sidebar nav hiding the "Credit Providers" link inside the `{isAdmin && ...}` block. Fixed by moving the link out to the "visible to ALL staff" section of `SidebarNav.tsx`, directly mirroring the existing "Debt Counsellors" link's established precedent (same comment, same pattern: view-only nav visible to everyone, editing gated server-side) rather than inventing a new pattern or duplicating the page. Also made the page's "← Admin" breadcrumb conditional on `isAdmin` (previously always rendered, would have linked non-admin viewers to a dashboard they can't access) and updated the page's subtitle to state the view/edit split explicitly. No backend or API changes — mutation permissions (`isAdmin`/`isExecutive`/`isSeniorManager` to edit, `isAdmin`/`isExecutive` to delete) were already correctly enforced server-side before this change and are unaffected. `tsc --noEmit` clean on both `apps/cases` and `packages/ui`. **Not fully verified live** — this is a live-production-DB dev environment requiring a real staff login, and entering credentials into a login form is outside what I'll do regardless of who provides them; verified via direct code reading (API route has no role gate, page has no role gate beyond button visibility) instead. Recommend the user or a non-admin staff member spot-checks the sidebar and page load once deployed.

> Last updated: 2026-08-08 (**Cases App: Form 17.W "Creditors Notified" Table Was Blank Because the CreditProvider Registry Was Empty — Backfilled + Guarded Against Recurring**): User flagged a live-generated Form 17.W showing every creditor with no address/phone/email — heading says "CREDITORS NOTIFIED" but nothing was actually notifiable. Root cause, confirmed via direct read-only query against the live DB: `CreditProvider` (name/type/email/phone/address/attorney — schema, admin CRUD UI at `/admin/credit-providers`, and the inline `CreditorLinkPicker` linker on the case page all already existed and worked) had **zero rows** in production, so all 19 live `CreditAccount` rows across 16 distinct creditor names were unlinked and `form17w-pdf.ts` was faithfully rendering the empty contact fields it was given — not a rendering bug, an empty registry. Four-phase fix, all explicitly scoped to **research only, no creditor contact** per user instruction. **(1) Backfill**: web-searched each of the 15 real creditors behind the 16 account names (deduped "LEWIS STORES" / "LEWIS STORES - LEWIS" into one `Lewis Stores` provider per user correction; kept `NIMBLE` / `NIMBLE GROUP` as two separate providers per explicit user choice) for official contact details, created all 15 `CreditProvider` records, and linked all 19 `CreditAccount` rows — zero left unlinked. Search confidence varied a lot in practice (conflicting addresses for Bhuti Cash Loans, a redacted court-filing address for FC African Trading, an unconfirmed name match for "DMCS", MultiChoice's actual DPP underwriter unconfirmed) — **left contact fields blank rather than guess** for 7 of the 15 where no reliably-sourced answer was found, since a wrong address on an NCA legal notice is worse than a visibly-missing one. **(2) Schema**: added `contactSource` (`MANUAL`/`WEB_LOOKUP`/`XDS`), `contactSourceNotes`, `contactVerifiedAt`, `needsReview` to `CreditProvider` (migration `20260808120000_add_credit_provider_contact_provenance`, applied via `prisma migrate deploy` — `migrate dev` is non-interactive-blocked in this environment, so the migration was hand-authored following the existing folder convention and applied non-interactively per Prisma's own suggested command) — every one of the 15 backfilled records is stamped `WEB_LOOKUP`/`needsReview: true` with a note citing its source and any known ambiguity, never silently treated as verified. Admin UI (`/admin/credit-providers`) gained a "Needs Review" stat tile + filter, an amber badge on flagged rows, and a one-click "Verify" action (`PATCH .../[id]` with `markVerified: true` → stamps `MANUAL`/`contactVerifiedAt`/clears the flag) so staff confirm before anything is trusted for a real notice. **(3) Auto-enrichment on sync — scoped down from the original plan**: no live web-search/browsing API exists anywhere in this codebase (confirmed by search), and hand-verifying this session's own lookups showed how unreliable an unsupervised auto-lookup would be for small/ambiguous lenders — wiring one up would mean silently picking a new paid provider/API key on the user's behalf, so that piece was **not** built. What *was* built: `credit-accounts/sync/route.ts` now auto-creates a `needsReview: true`, `contactSource: 'XDS'` stub `CreditProvider` the moment a synced account's creditor name matches nothing in the registry, so no future creditor can ever again go silently unlinked — the actual root cause is closed going forward; live web-lookup automation remains a separate decision (which provider, cost) for the user to make explicitly. **(4) Form 17.W guard**: `debt-review/generate/route.ts` now flags any FORM_17_W creditor with *no* address, phone, or email at all in its response (`warnings: string[]`) instead of silently emitting blank columns; surfaced in both UI entry points — `DebtReviewTab.tsx`'s toast gained a third `warning` (amber) variant, and `GenerateClearanceButton.tsx`'s success modal now shows an itemized amber warning block and — deliberately — stays open instead of auto-closing after 2.5s when warnings are present, so staff can't miss it. Tests: `credit-providers/route.test.ts` +1 (`needsReview` meta count) +1 (`markVerified` PATCH behavior), `credit-accounts/sync/route.test.ts` updated for the new stub-creation path, `debt-review/generate/route.test.ts` +2 (warnings populated / empty), 52/52 green across all touched suites; `tsc --noEmit` clean on `apps/cases`. **Verified live against the real production DB** (not mocked) — re-queried after the backfill+link script ran: all 19 accounts now resolve to a named provider, 7 correctly still show blank contact fields (the ones with no confidently-sourced answer) which will now correctly trigger the Phase 4 warning instead of a silent blank Form 17.W. No migration to STATUS-list "What's Next" — this was reactive to a direct user report, not a queued roadmap item. **Not yet done / explicit gaps**: the 7 blank-contact providers and all "unconfirmed"-noted ones still need a human to actually look them up and click Verify before any of them should go out on a real notice; no automated live web-search enrichment (see above); no UI test coverage added for `DebtReviewTab.tsx`/`GenerateClearanceButton.tsx` (both pre-existing, untested components — only the API-level warning computation has test coverage, the UI wiring is thin display logic on top of it).

> Last updated: 2026-08-07 (**Cases App: Total Open Balance Bug, Instalment & Opening Balance Columns added**): Fixed a bug where the Total Open Balance in the Clearance generation modal was concatenating strings instead of summing up numerical balances (due to Prisma decimals acting as strings during reduce). Updated `apps/cases/app/api/cases/[id]/clearance/evaluate/route.ts` to explicitly cast balances to `Number` and also calculated `totalInstalment`. Updated `apps/cases/components/GenerateClearanceButton.tsx` to add "Opening" and "Instalment" columns to the accounts table and display the total instalment alongside the balance. Implemented styling fixes (`table-fixed`, `w-[...]` percentages, `truncate`) to the table and tucked the "Status/Notes" section neatly under the Creditor name column to prevent horizontal scrolling per user's request. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Sync Credit Accounts Error Fix**): Fixed a bug where importing credit accounts from the AI analysis would throw an error "Argument 'toStatus' is missing" when trying to create a `WorkflowLog` entry. Updated the API route `apps/cases/app/api/cases/[id]/credit-accounts/sync/route.ts` to fetch the case's current status and include it as `toStatus` in the workflow log creation, satisfying the Prisma schema requirement. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Removed Horizontal Scrolling on Credit Report Insights Accounts Table**): User requested to make the "All Accounts" table fit within the window and avoid horizontal scrolling. Found the table container was forcing `overflow-x-auto` with a `min-w-[700px]` on the table element in `apps/cases/app/(authenticated)/cases/[id]/CreditReportInsightsTab.tsx`. Removed both classes, allowing the table to naturally fit the container and wrap text if necessary, relying only on vertical scrolling for page overflow. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Compliance: Signed POA + Barcoded ID Now Attached on Outbound DC/Creditor Emails Sent on Consumer's Behalf**): User flagged a compliance gap: any email sent to a Debt Counsellor or credit provider while acting on behalf of a consumer must carry the signed POA + barcoded ID as documentary proof of consent — several automated/AI-drafted letters were sending the *claim* ("we act under a valid Power of Attorney") without the actual attachment. Fixed the two scenarios the user named. **(1) Cases app — OUTSTANDING_FEES invoice request** (`packages/shared-lib/src/dhs/decline-handler.ts`, `handleDHSDecline()`): the DC-facing "please send us an invoice/statement" email (sent when a DHS decline reason asserts the consumer owes fees) previously had zero attachments — only the `SEND_DOCS`/`SEND_DOCS_WITH_NCR` and `CONTACT_ATTORNEY` categories attached the already-computed `docAttachments` (case documents of type `ID`/`POA`/`ZENOWETHU_POA`). Now attaches the same `docAttachments` and adds a `hasAttachments`-gated line to `buildRequestInvoiceEmail()`'s body referencing them as "proof of our authority to act on the consumer's behalf." Mirrored identically in the read-only `decline-preview.ts` (single source of truth for email copy shown before a real send) so the preview matches what actually goes out. **(2) Legal app — AI-drafted letters to DCs/credit providers/bureaus** (`apps/legal/app/api/ai/draft/route.ts`): this is the one shared send point behind every AI Autopilot-executed `draftLegalDocument()` output — LOD, `PRESCRIPTION_NOTICE` (the "disputing a prescribed account" scenario the user named — Section 126B NCA notices), `CLEARANCE_DEMAND`, `BUREAU_FILE_REQUEST`, `PROVIDER_FILE_REQUEST`, and `DC_DRR_FILE_REQUEST` all funnel through one `sendManualMessage()` call that previously sent zero attachments regardless of type. Added `documents: true` to the case query and attach `ID`/`POA`/`ZENOWETHU_POA` doc URLs the same way, sourced from `CASES_APP_URL` (new env var, documented in `apps/legal/.env.example`) since document files physically live under the Cases app's `public/uploads`, not the Legal app's — confirmed via the existing (previously undocumented) `CASES_APP_URL` usage in `apps/legal/proxy.ts`. Non-autopilot drafts (`shouldExecute: false`) are unaffected — they're saved as `DRAFT` with no send at all, a separate pre-existing gap (no "send this reviewed draft" endpoint exists yet) not in scope here. **Other scenarios surfaced but not yet audited**: DCCP/COLMS correspondence (still scaffolded, `loginAs()` TODO — Tier 2 Sprint 2), NCT eFiling submissions (scaffolded), Crediva's consumer-facing dispute letter generator (`packages/shared-lib/src/disputes/dispute-pdf.ts`, wired into `apps/crediva/app/api/crediva/disputes/generate/route.ts` — generates a PDF but its send path wasn't traced in this session), and any GHL-based document sends via `GhlService.sendDocuments()`/`requestFileFromDC()` (`packages/shared-lib/src/integrations/ghl-service.ts` — these already attach POA+ID correctly, used as the reference pattern for this fix, GHL itself not yet configured per prior session notes). Tests: shared-lib +2 (`buildRequestInvoiceEmail` POA-line present/absent), decline-handler persistence +2 (DC email attachments present when docs exist / empty when they don't, exact `actionsPerformed` string updated), decline-preview +1 assertion; new `apps/legal/app/api/ai/draft/route.test.ts` (3 tests: autopilot attaches POA+ID, manual/co-pilot sends nothing, empty-attachments fallback when no docs on file) — this route had no test coverage before. Full shared-lib suite 838/838 green, legal app suite 21/21 green (2 files), `tsc --noEmit` clean on both `shared-lib` and `legal`, and `cases` (consumer of the shared-lib change) also clean. Not yet verified against a live send (no SMTP/GHL credentials in this session) — logic and attachment-URL construction verified via mocked tests only. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Open Date & Last Payment Date Added to All Accounts, Missing Ones Flagged**): User's live "ALL ACCOUNTS" table (now correctly showing 15/15 accounts after the closed-account extraction fix) requested Open Date and Last Payment Date columns, with explicit indication when either is missing rather than a silent blank. Both fields (`contractStart`, `lastPaymentDate`) were already being extracted per-account but never surfaced. Added `formatReportDate()` helper to `CreditReportInsightsTab.tsx` — parses `YYYY-MM-DD` or treats empty/`NA`/unparseable as "Not stated", rendered in amber italics distinct from normal (gray) date text so staff can tell "nothing extracted" apart from a real date at a glance. Adverse listings correctly show no Open Date (that field isn't extracted for them — `adverseDate` is a different concept, when the listing was placed, not when the account opened) while still showing Last Payment Date where available. `tsc --noEmit` clean; full suite 866/866 green (no new dedicated tests — pure display formatting on already-tested extraction data). Verified live with mocked API data: dates render correctly when present, "Not stated" renders correctly for both fully-missing and partially-missing rows. Deployed via commit `c3466d70`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Affordability Assessment Already Exists — Living Expenses Table Is the Real Gap (Reminder, Not Yet Built)**): User asked for (1) closed accounts to be saved into the real `CreditAccount` table, not just displayed, so prescription/dispute/affordability workflows never need to re-pull the credit report, and (2) a button for Affordability Assessment (which they noted requires payslips, credit report, and a living expenses table) — or, failing that, just a reminder to build one. Investigated via a research agent before writing any code. **(1) Already works, no code needed**: since the extraction prompt was widened earlier today to capture closed accounts, they now flow into `accounts[]` alongside open ones, and `Sync Credit Accounts`'s candidate generation was never status-filtered — so once staff run Sync and click Import, closed accounts land in the real `CreditAccount` table (the same table every downstream feature already queries), no re-pull required. The staff-review gate before import (deliberately added earlier this session, since this feeds legal clearance decisions) stays in place — this is not fully automatic, and removing that gate would need explicit confirmation first. **(2) No button gap** — `apps/cases/app/(authenticated)/cases/[id]/DebtReviewTab.tsx` already has a complete, functioning "Credit Report & Affordability Check" panel: pulls `client.netSalary`, `creditAccounts`, and analyzed PAYSLIP/BANK_STATEMENT documents via `apps/cases/lib/affordability-check.ts` (`computeAffordabilityForCase`), verifies income against the bank statement, and shows a "Generate Rejection Pack" button (Form 16, Form 17.2(a), Affordability Assessment PDF, Consumer Info Record) when the numbers indicate the consumer isn't over-indebted, plus a "Generate Rescission Pack" button for Status C/D3/D4. PDF generator (`apps/cases/lib/affordability-assessment-pdf.ts`) is fully implemented and already wired into `debt-review/generate/route.ts`. **The real gap, confirmed absent platform-wide**: a "living expenses" data model. No `LivingExpense`/`MonthlyExpense` Prisma model exists; `CreditAccount` has no expense fields; the only places "living expenses" appears anywhere in the codebase are a hardcoded `null` passed into `debt-restructuring-pdf.ts` and a rough `netSalary * 0.50` heuristic guess in two *unrelated* apps (`apps/insurance/lib/risk-scoring.ts`, `apps/forensic-audit/lib/forensic-engine.ts`) — never real user-entered data feeding the actual affordability calculation anywhere. **REMINDER — not yet built, user explicitly chose to log this rather than build now**: add a real `LivingExpense`/similar Prisma model (rent, groceries, transport, insurance, etc. per case), a staff-facing form to capture it, and wire it into `computeAffordabilityForCase()`/`affordability-assessment-pdf.ts` so the assessment reflects actual declared expenses instead of having no expense input at all. No code changed this entry — investigation + reminder only. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Extraction Prompt Never Asked for Closed Accounts or Opening Balance — Fixed, Plus New "All Accounts" Table**): User compared Sync Credit Accounts (5 candidates) against Credit Report Insights' own summary tile (7 open / 8 closed = 15 total) and asked why. Root cause, confirmed by reading the actual `CREDIT_REPORT` prompt (`packages/shared-lib/src/openai/prompts.ts`, section 8): it explicitly instructed the AI to scan for *"OPEN/ACTIVE/ARREARS accounts"* only — closed/paid-up accounts were never asked for as individual line items, only counted in the separate summary-totals pass added earlier this session. That fully accounts for the 8 missing closed accounts (nothing was ever extracted for them to show). The other 3 (4 of 7 open accounts captured) is a separate, harder-to-eliminate AI recall gap scanning a long dense table — distinct from the page-limit truncation fixed earlier, since all pages are now visible to the model; this is about how completely it enumerates every row within what it sees. Fix: widened prompt section 8 to explicitly scan and extract closed/paid-up accounts too (excluding only adverse/written-off/judgment, which stay in `adverseListings`), added explicit "go through the entire table row by row, don't skip rows" guidance, and added a new `originalAmount` field (opening balance / original loan amount) to the `accounts[]` schema — neither closed accounts nor an opening-balance figure existed anywhere in the extraction before this. Threaded `originalAmount` end-to-end: `ExtractedAccountRaw`/`CreditAccountCandidate` in `credit-account-sync.ts`, the sync route's Zod schema + `CreditAccount` create/update (DB column already existed, just never populated), and a new "Opening" column in the Sync Credit Accounts review table. Also built the second half of the ask — a new **"All Accounts"** table on each Credit Report Insights card (creditor, status, opening balance, current balance, monthly instalment) covering every extracted account *and* adverse listing, not just ones still pending sync — so staff see the full picture immediately after analysis rather than only unsynced candidates. Tests: `credit-account-sync.test.ts` +2 (closed/paid-up account mapping with originalAmount, null-when-absent); full suite 866/866 green; shared-lib 836/836 green (prompt is a string constant, no logic to test directly); `tsc --noEmit` clean. Verified live (mocked API responses, real component tree — same approach as earlier in this session since local dev still can't reach production-only files): confirmed the "ALL ACCOUNTS (3)" table renders creditor/status/opening/current/instalment correctly for a mix of open, closed, and adverse-listing rows, and confirmed the Sync modal's new "Opening" column renders correctly. Deployed via commit `124c7638`. **Known limit, not fully solvable by this fix alone**: even with closed accounts now requested, AI recall on very long/dense tables isn't guaranteed to be 100% complete — re-running "Force Re-analyze All" on Stephen Rampai's case should recover meaningfully more accounts than before, but perfect 15/15 extraction isn't guaranteed and needs live confirmation. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: Fixed "View in Portal" Scroll Landing Under Sticky Header, Added Prescribed/Debt-Review Stats to Credit Report Insights**): Two follow-ups after confirming the summary-totals + uncapped-limits fixes worked live (screenshot showed R28,919 debt / R2,750 instalments / 7 active / 8 closed matching the PDF). (1) **Scroll fix**: the "View in Portal" scroll landed the "Credit Report Insights" heading hidden underneath the app's fixed header (`h-16`, ~56-64px) plus a sticky sub-header (~63px) — confirmed via `getBoundingClientRect()` in a live browser session. Added `scroll-mt-[140px]` to the tabs panel container. Separately, the scroll wasn't firing at all in practice: the tabs panel unmounts/remounts as part of the case refetch "View in Portal" triggers, and the original single `useEffect` (dependent on `[pendingScrollToCreditReport, loading]`) could run in the gap between unmount and remount, capturing a stale/null `tabsPanelRef.current`. Replaced with a short poll (50ms interval, ~5s safety cutoff) that waits for both `loading` to clear and the ref to be non-null before scrolling. Verified the underlying mechanism directly (manual `scrollIntoView` call landed the element at exactly 140px from viewport top, confirming the offset is correct) — could not fully verify the polling effect's live timing through the automated browser tool, since it reported `document.visibilityState: 'hidden'` / `hasFocus: false` throughout testing regardless of `tabs_select`, a limitation of that tool's tab-focus model rather than something fixable in app code; relying on code review + the direct mechanism confirmation instead. (2) **New stats**: the Credit Report Insights summary strip (previously Total Debt/Instalments/Active/Closed) now also shows a **Prescribed** count (accounts flagged `dispute` + title starting "Possible prescription") and a **Debt Review** status tile (registration NCRDC number, colored amber if it's a different debt counsellor than Zenowethu's own NCRDC3693, green if it matches, "None found" if absent) — grid widened from 4 to 6 columns (`lg:grid-cols-6`) per explicit request to surface "how many accounts are open and closed, total monthly instalment, the balance, number of prescribed accounts, and the debt review flag" directly in the card header rather than only inside individual insight items. `tsc --noEmit` clean; full suite 864/864 green (no new dedicated tests — both changes are display/timing logic layered on already-tested `insights`/`data` structures, consistent with this codebase's existing lack of test coverage for page-level Tab components like `DebtReviewTab`/`CourtDocsTab`). Deployed via commit `347b5e9f`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-07 (**Cases App: "View in Portal" Button on Analyse Credit Reports — Fixed a Bug That Made It Impossible to Build**): User confirmed the summary-totals + uncapped-page-limit fixes worked live (Stephen Rampai's report now shows R28,919 total debt / R27,250 instalments / 7 active / 8 closed, matching the actual PDF almost exactly instead of all zeros), then asked for the analysis results modal to include a "View in Portal" button that jumps straight to the Credit Report Insights tab. Added the button to `AnalyseCreditReportsButton.tsx`, wired via a new `onViewInsights` prop and a `goToCreditReportTab()` handler in the case page (switches `activeDetailTab` to `'CREDIT_REPORT'`). While verifying this locally (fetch-mocked, since local dev still can't read the production-only file), found the button would never actually be clickable in practice: the existing success handler called `onAnalyzed()` (→ `fetchCase()`) immediately after a successful run, and `fetchCase()` unconditionally calls `setLoading(true)` at its start — which unmounts the entire tabs panel, including the just-opened results modal, before the user has any chance to see or click anything. Confirmed via a scripted browser test: the modal showed "1 analyzed, 0 failed" for roughly one render tick, then vanished entirely. Fixed by deferring the `onAnalyzed()` refetch to modal *dismissal* (Close, ✕, or View in Portal) instead of firing it the instant analysis succeeds — the modal's own result state doesn't depend on `caseData` anyway, so nothing needs the refetch to happen while results are still being shown. Also replaced a fixed-delay scroll-into-view (which would have raced against the same refetch) with a `pendingScrollToCreditReport` flag that waits for the parent's `loading` state to actually clear before scrolling. Verified live end-to-end (mocked fetch, real component tree): Analyze → results with "View in Portal" visible and stable → click → lands directly on a fully-loaded Credit Report Insights tab. `tsc --noEmit` clean; full suite 864/864 green (no new dedicated test file — this is a UI wiring/timing fix in a component already covered by `credit-account-sync`-adjacent tests elsewhere; the meaningful verification here was the live browser reproduction, not something a unit test would have caught). Deployed via commit `c54b1cc7`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-06 (**shared-lib: Removed Hardcoded Page/Text Caps in `analyzeDocument()` — Accuracy Over API Cost, Per Explicit User Direction**): Direct follow-up to the summary-totals fix above. User asked to raise the page limits and explicitly said: "Uncapped I want accurate results not saving costs results." `packages/shared-lib/src/openai/extraction.ts`'s `analyzeDocument()` — the single function every document-analysis call site in every app routes through — hardcoded a 6-page image-OCR limit (`convertPdfToImages(base64Pdf, 6, ...)`), a 10-page text-extraction limit (`extractTextFromPdf(base64Pdf, 10, ...)`), and a 20,000-character cap on the extracted text actually sent to the model (`extractedText.substring(0, 20000)`) — all regardless of actual document length. Confirmed via `pdf-image.ts` that both `convertPdfToImages`/`extractTextFromPdf` already correctly treat `maxPages=0` as "no limit, process all pages" (`pagesToRender = max > 0 ? Math.min(totalPages, max) : totalPages`), so this was purely a caller-side restriction, safe to remove. Changed all three to unlimited: `0` for both page limits, `200000` chars for the text cap (a safety ceiling against pathological inputs, not a practical limit for any real report). This is a blanket change affecting every document type analyzed via `analyzeDocument()` (ID, POA, PAYSLIP, BANK_STATEMENT, CREDIT_REPORT, CREDIT_REPORT_OTHER, DHS_SUMMARY_REPORT) across all 5 apps that use it — intentional per the user's stated preference, since the same silent-truncation risk applies to any long upload, not just Stephen Rampai's 16-page Experian report. Left `identifyCreditReportType()`'s separate 5-page text limit untouched — that call only identifies which bureau produced a report (always determinable from page 1-2), not full account data, so it's a genuinely different, still-appropriately-scoped use case. **Trade-off knowingly accepted, not mitigated**: this increases OpenAI API cost and analysis latency for every document analyzed platform-wide going forward, with no ceiling beyond the 200k-char text safety net and whatever token budget the model itself enforces — explicitly the user's call, not a default to revisit without asking first. Tests: full shared-lib suite 836/836 green (no dedicated test added for this specific change — a 3-constant edit with clear pre-existing test coverage on `convertPdfToImages`/`extractTextFromPdf`'s `maxPages=0` behavior already establishing correctness); `apps/cases` `tsc --noEmit` clean. Deployed via commit `2ea6c324`. **Not yet verified against the live site** — same as the summary-totals fix above, requires the user to re-run "Force Re-analyze All" on Stephen Rampai's case and confirm `activeAccounts`/`closedAccounts` now show the real 7/8 instead of 0/0. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-06 (**Cases App: Analyse Credit Reports Was Missing the Accurate Summary-Totals Pass — Accounts Summary Table Showed as All Zeros**): User compared the new Credit Report Insights tab against the actual PDF's own "Accounts Summary" panel for Stephen Rampai's Experian report and found a real mismatch: the PDF shows Balance Exposure R28,919, Monthly Instalment R27,820, 7 active/8 closed accounts, but Insights showed R0 / R0 / 0 / 0 for the same analyzed document. Root cause: `apps/cases/app/api/cases/[id]/credit-reports/analyze/route.ts` (the "Analyse Credit Reports" button) ran only a single `analyzeDocument(..., 'CREDIT_REPORT', ...)` pass. That single pass is already known within this codebase to be unreliable for reading the "Accounts Summary" table specifically — a dedicated `CREDIT_REPORT_SUMMARY` prompt exists precisely because of this (`packages/shared-lib/src/openai/prompts.ts:526`, "extracting FINANCIAL TOTALS ONLY... DO NOT extract account counts"), already wired into the older, unused `/api/documents/[id]/analyze` route's two-pass merge — but my newer, isolated route never picked up that pattern. Fix: after the detailed pass, run a second `CREDIT_REPORT_SUMMARY` pass for major-bureau report types and merge `totalDebt`/`totalInstallment` into the nested `analysis.summary` object correctly (the older route's merge attempt spreads a flat summary-pass object over a nested one and doesn't actually land in the right place — not touched here, since that route isn't called by any UI). Summary-pass failure is caught and swallowed, keeping the detailed pass's totals rather than losing data. **Does not fix** `activeAccounts`/`closedAccounts` (still likely 0/0) — the summary prompt deliberately excludes account counts, and getting real counts requires the detailed pass to actually find the 15 individual account rows this report has, which ties into a second, separate, NOT-yet-fixed issue also discovered during this investigation: `analyzeDocument()` (`packages/shared-lib/src/openai/extraction.ts:39,48`) hardcodes a 10-page text-extraction limit and a 6-page image-OCR limit regardless of actual document length — Stephen's Experian report is 16 pages, meaning content beyond page 6 (image OCR) or page 10 (text) may be silently invisible to the AI. This is platform-wide (baked into the shared extraction function every caller uses, not something introduced this session) and raising the limits has real OpenAI cost/latency/context-window tradeoffs — flagged to the user, not yet decided or fixed. Tests: `analyze/route.test.ts` +2 (summary pass runs for major-bureau types and not for alternative-bureau types; detailed-pass totals survive a summary-pass failure) and the existing tests updated for the new second `analyzeDocument` call; full suite 864/864 green; `tsc --noEmit` clean (one unavoidable `as unknown as DocType` cast for `'CREDIT_REPORT_SUMMARY'`, matching the same cast already used at the only other call site, since that prompt key was never added to the shared `DocType` union). **Not yet verified against the live site** — local dev cannot read the physical file (same pre-existing environment gap as earlier in this investigation), so verification requires the user to re-run "Force Re-analyze All" on production and confirm the totals now match the PDF. Deployed via commit `d259d353`. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-06 (**Cases App: New "Credit Report Insights" Tab — Dispute/Improve/Great-About-It Breakdown of Analyzed Credit Reports**): User request following the Sync Credit Accounts / adverseListings fix above: "create a page on cases where the extracted data is saved or displayed for users to see... indicate everything that can be used to dispute or improved or that is great about the report." Built a new case-detail tab surfacing the AI-extracted credit report data (`Document.extractedData`, CREDIT_REPORT prompt shape) in human-readable form, grouped into four categories per report. (1) **`apps/cases/lib/credit-report-insights.ts`** — pure `buildCreditReportInsights(data)` function, grounded directly in the canonical extraction schema (`packages/shared-lib/src/openai/prompts.ts:225-397`), not invented heuristics: **Dispute candidates** — prescription-eligible adverse listings/accounts (3-year Prescription Act threshold for ordinary debt, 30-year for judgment debt — correctly distinguished via `adverseCode` containing "JUDG", since judgment debt prescribes on a different timeline); duplicate creditor+account-number listings; inconsistent ID number/surname/cell number across the report (using the extraction's own `_occurrences` tracking — a built-in data-integrity signal that was already being extracted but never surfaced anywhere); excessive-enquiries flag. **Could be improved** — each credit score suppressor listed individually; accounts with arrears > 0; high debt-to-income ratio (≥40%, calculated from the report's own declared income/instalment totals) — deliberately phrased as a nudge toward the existing, more rigorous `apps/cases/lib/affordability-check.ts` (bank-statement-confirmed) rather than asserting "reckless lending" itself, to avoid contradicting that authoritative feature with looser math. **What's great** — no adverse listings; Good/Great score band; enquiries within normal range; consistent identity data. **For your information** — active debt review registration, explicitly flagging when it's under a *different* debt counsellor than Zenowethu's own NCRDC3693 (directly surfaces the exact discrepancy found investigating Stephen Rampai's case); the automated bureau decline/approve outcome and reason. Every adverse listing not already flagged as a dispute candidate still gets a plain informational entry (creditor, balance, status, prescription-eligibility countdown) — added after live-testing on Stephen Rampai's real data showed his one real debt (Lewis Stores, R33,330, written off, not yet 3 years old) was silently invisible on the page despite being genuinely present, since it didn't trigger any specific flag. (2) **`GET /api/cases/[id]/credit-reports/insights`** — parses every analyzed credit-report-type document's `extractedData` and runs it through the insight builder; separately lists any credit reports still awaiting analysis so staff know to run "Analyse Credit Reports" first; malformed JSON degrades to an empty-data insight set instead of a 500. (3) **`CreditReportInsightsTab.tsx`** — new "📊 Credit Report" tab on the case detail page (added to `VALID_TABS`, positioned right after Documents), one card per analyzed report with a quick-stats strip (score/band, total debt, instalments, active/closed accounts) plus the four insight columns, and a fixed compliance disclaimer footer ("AI-assisted analysis for staff review only — not legal or financial advice... not a guarantee of removal or success") per the platform's no-overpromising rule. (4) **Tests** — `credit-report-insights.test.ts` (23 tests covering every category, the judgment-vs-ordinary prescription distinction, the always-visible-adverse-listing behavior, and a full real-world-shaped fixture matching Stephen Rampai's actual extracted data) + `insights/route.test.ts` (6 tests); full suite 862/862 green; `tsc --noEmit` clean. Fixed a locale-formatting bug caught by the test suite itself: bare `.toLocaleString()` on Rand amounts is non-deterministic across environments (renders "33,330" vs "33 330" depending on default ICU locale) — replaced with an explicit `Intl.NumberFormat('en-ZA')` `formatRand()` helper used consistently across all four money-formatting call sites in the new file. (5) **Verified live** against the running dev server (shared prod DB) on Stephen Rampai's actual case: confirmed the tab renders his real report correctly — excessive enquiries under Dispute, the score suppressor under Improve, consistent identity data under What's Great, and both the different-debt-counsellor flag and the now-visible Lewis Stores adverse listing under For Your Information. Deployed via commit to `main` — Dokploy auto-deploys on push. **Not yet built**: no way to select which specific report to focus on when a case has multiple analyzed credit reports (currently shows all as separate cards, which is fine for the common 1-2 report case but could get long); no Crediva (consumer-facing) equivalent — this is staff-only, matching Tier 3's Crediva credit report deep-analysis roadmap item, which remains a separate, not-yet-started piece of work. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-06 (**Cases App: Sync Credit Accounts Was Missing `adverseListings[]` — Written-Off Debt Wasn't Reaching the Clearance Evaluator**): User ran "Analyse Credit Reports" live on production (`cases.zenowethu.co.za`, not local dev) for Stephen Rampai's case and reported "the credit report is not being read" after Sync Credit Accounts showed "No new accounts found." Verified directly against the live DB (`Document.analyzedAt: 2026-08-06T14:46:45Z`, `extractedData` populated with real content — credit score suppressors, debt review status under a *different* debt counsellor (NCRDC2967), 9 recent enquiries) that analysis genuinely succeeded — the AI read the report correctly. Root cause of the empty Sync result: the report's one real debt — LEWIS STORES, R33,330, WRITTEN OFF — was extracted into a separate `adverseListings[]` array, not `accounts[]`, and Sync Credit Accounts (`apps/cases/lib/credit-account-sync.ts`) only ever read `accounts[]`. Written-off/handed-over/judgment accounts — exactly the kind of debt relevant to a debt-review-removal decision — were structurally invisible to the sync/clearance pipeline. Fix: (1) added `ExtractedAdverseListingRaw` + `mapExtractedAdverseListingToCandidate()` to `credit-account-sync.ts`, mapping `creditor/accountNumber/adverseCode/openBalance/overdueBalance/lastPaymentDate/status` onto the same `CreditAccountCandidate` shape (balance falls back to `overdueBalance` when `openBalance` is absent; status falls back to `adverseCode` then a generic `ADVERSE LISTING` label); (2) wired it into `GET /api/cases/[id]/credit-accounts/sync` alongside the existing `accounts[]` loop, sharing the same cross-document dedupe. Also fixed a real UI bug found while diagnosing this: `AnalyseCreditReportsButton.tsx`'s `runAnalysis()` didn't clear previous `ran`/`summary`/`results` state before starting a new run, so re-running (e.g. "Force Re-analyze All") rendered the new loading spinner on top of the *previous* run's stale result message simultaneously — confusing during live debugging since it looked like two contradictory states at once. Two earlier same-day fixes on this same investigation thread: surfaced the resolved absolute file path in "File not found on disk" errors (`credit-reports/analyze/route.ts`) instead of a bare unhelpful string, and moved the per-file failure reason from a hover-only `title` tooltip to inline text (invisible in screenshots, impossible to relay from a live debugging session). Tests: `credit-account-sync.test.ts` +4 (adverse listing mapping: openBalance, overdueBalance fallback, adverseCode/generic status fallback, dedupe-by-account-number), `sync/route.test.ts` +1 (adverseListings-only report still produces a candidate); full suite 835/835 green; `tsc --noEmit` clean. Deployed via 3 commits to `main` (`28d855dd`, `8b2af962`, `34dc3f36`) — Dokploy auto-deploys on push to `main`. **Not yet verified against the live site** — awaiting user confirmation that Sync Credit Accounts now surfaces the Lewis Stores written-off account for Stephen Rampai's case. **Separately worth flagging to a human**: this Experian report shows an *active* debt review registration under a different debt counsellor (NCRDC2967, "SEMAR MUHAMMAD") than Zenowethu (NCRDC3693) — a compliance-relevant discrepancy unrelated to this bug fix, not yet investigated. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-02 (**Cases App: "Analyse Credit Reports" — Isolated AI Analysis Button for Credit Report Documents Only**): Follow-up to the "Sync Credit Accounts" work above — discovered that neither existing AI-analysis entry point in the app (the case-header "Analyse with AI" → `/api/cases/[id]/compare-analysis`, or the Documents tab's "Re-analyse with AI" → `/api/documents/reanalyze`) can actually reach a document typed `CREDIT_REPORT_EXPERIAN`/`_XDS`/`_TRANSUNION`/`_LIGHTSTONE`: both only match the literal type `CREDIT_REPORT` in their document lookups (`DocumentsTab.tsx:512`, `reanalyze/route.ts:238`), even though the Documents tab's own "Credit Bureau Reports" panel explicitly uploads reports under those bureau-specific types. Practical effect: no bureau-specific-typed credit report anywhere on the platform could be AI-analyzed via any existing button — confirmed on Stephen Rampai's case (ID 8908305317089), whose Experian report is typed `CREDIT_REPORT_EXPERIAN` and was therefore invisible to both flows. Rather than widen the existing shared `/api/documents/reanalyze` route (which also touches ID/POA/Payslip/BankStatement documents in the same batch and overwrites Client profile fields like address/employer/phone/salary as a side effect — a bigger blast radius than desired), built a new, deliberately isolated button per explicit user request ("isolated to deal with only credit reports all credit reports create new one"): (1) **`apps/cases/lib/credit-report-analysis.ts`** — `resolveCreditReportPromptType()` maps any bureau-specific type onto the two prompt keys that actually exist in `PROMPTS` (`CREDIT_REPORT` for major bureaus — which already auto-identifies TransUnion/Experian/XDS/CPB internally via `identifyCreditReportType()` — or `CREDIT_REPORT_OTHER` for ClearScore/Kudough-style alternative reports), without renaming the document's stored `type`. (2) **`POST /api/cases/[id]/credit-reports/analyze`** — scans only documents whose type is in the existing `CREDIT_REPORT_DOC_TYPES` set (reused from the Sync feature), analyzes each via `analyzeDocument()`, writes `extractedData`/`analyzedAt` per document, and returns a per-file success/failure/account-count summary; by default skips already-analyzed reports, with a `force: true` option to re-run all of them. Deliberately does **not** call `updateClientData()` or touch any Client/Case field — the entire mutation surface is `Document.extractedData`/`analyzedAt` for credit-report-typed rows only. (3) **`AnalyseCreditReportsButton.tsx`** — new case-header button (🧮, violet accent) placed between "Evaluate Clearance" and "Sync Credit Accounts", forming a clear left-to-right workflow: Analyse → Sync → Evaluate. Shows a per-file results list after running, with a "Force Re-analyze All" follow-up action. Wired into `apps/cases/app/(authenticated)/cases/[id]/page.tsx`. (4) **Tests** — `credit-report-analysis.test.ts` (4 tests) + `credit-reports/analyze/route.test.ts` (6 tests, covering auth, no-op with no credit reports, default skip-already-analyzed, `force` re-analyze, and partial-batch-failure isolation), all passing; full suite 830/830 green; `tsc --noEmit` clean for all new files (same 3 pre-existing unrelated errors, untouched). (5) **Verified live** against the running dev server: confirmed all three buttons render in the correct order on Stephen Rampai's case, and confirmed the new modal opens with the correct "only touches credit report documents" messaging. Did not actually trigger the real AI analysis call during verification (real OpenAI cost + would mutate the live document row) — left for the user to run explicitly. (6) **Known gap** — this button lives only in `apps/cases` (not the shared `DocumentsTab.tsx`), consistent with the Sync feature's scoping decision, since `DocumentsTab` is used by 4 other apps that don't have this route. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-02 (**Cases App: "Sync Credit Accounts" — Auto-Populate `CreditAccount` Table from Analyzed Credit Reports**): Investigated a specific case (Stephen Rampai, ID 8908305317089) and found the "Evaluate Clearance" button (`GenerateClearanceButton.tsx`) was recommending Form 19 F2 (Full Clearance) purely because `CreditAccount` was empty for that case — not because debts were actually settled. Root cause: an uploaded Experian report existed but had never been AI-analyzed (`Document.analyzedAt: null`), and even when a credit report *is* analyzed, nothing in the codebase ever copied the AI-extracted `accounts[]` array (stored in `Document.extractedData`) into the `CreditAccount` table — that table was previously populated only by manual staff form entry (`POST /api/finance/credit-accounts`). Built a bridge: (1) **`apps/cases/lib/credit-account-sync.ts`** — pure mapping functions (`inferAccountType`, `parseAiDate`, `accountsMatch`, `mapExtractedAccountToCandidate`) that turn a raw AI-extracted account row into a reviewable `CreditAccount` candidate, matching it against existing accounts on the case by account number (falling back to creditor name) to flag `NEW` vs `DUPLICATE`. (2) **`GET/POST /api/cases/[id]/credit-accounts/sync`** — `GET` scans every credit-report-type document on the case, reads `extractedData.accounts` from analyzed ones, cross-document-dedupes, and returns a preview (also reports which credit report docs still need AI analysis first, since this route intentionally does not trigger analysis itself — avoids duplicating the existing two-pass analysis logic in `/api/documents/[id]/analyze`); `POST` accepts the staff-confirmed/edited list and creates new `CreditAccount` rows or updates matched existing ones (balance/instalment/last-payment/status/prescription), logging a `WorkflowLog` entry. Zod-validated, Prisma-derived types throughout, no `any` on the request boundary. (3) **`SyncCreditAccountsButton.tsx`** — new case-header button (next to "Evaluate Clearance") mirroring its modal pattern: staff must review and select which accounts to import (mandatory review gate — this table feeds legally significant Form 19/17.W clearance decisions, so nothing auto-inserts without a human confirming), with an editable account-type dropdown per row since the AI doesn't extract that field. Wired into `apps/cases/app/(authenticated)/cases/[id]/page.tsx`. (4) **Tests** — `credit-account-sync.test.ts` (10 tests) + `sync/route.test.ts` (7 tests), all passing; full suite 820/820 green; `tsc --noEmit` clean for all new files (3 pre-existing unrelated errors in `DebtReviewTab.tsx`/`d4-pack-pdfs.test.ts`/`form17w-pdf.ts` untouched). (5) **Verified live** against the running dev server + prod DB: confirmed the "needs analysis" warning renders correctly for Stephen Rampai's case, and confirmed the full candidate/dedupe flow renders correctly against a real multi-document case (Samkelo Felezeweni, 2 analyzed Compuscan reports, 3 existing manually-entered `CreditAccount` rows) — correctly deduped an existing Lewis Stores account by matching account number, correctly treated a same-name-different-account-number Capfin SA row as a distinct NEW account, and defaulted an accountless AI row to "Unknown Creditor" without crashing. No data was actually imported during verification (read-only preview only) to avoid mutating production records without an explicit user request. (6) **Known gap / not yet built** — no per-document sync entry point in `DocumentsTab.tsx` (case-level "sync all credit reports on this case" was built per explicit user request instead, since `DocumentsTab` is a shared `@zenowethu/ui` component used by 5 other apps and a credit-report-specific action there would need cross-app gating); the AI extraction fields don't include account type, original amount, or interest rate, so those need staff input/defaults at review time. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-02 (**Reporting App: Fixed Duplicate Concurrent `EmployeeSession` Rows — Root Cause of "Two Check-Ins"**): User-reported bug: the header presence selector showed a stale "two check-ins" state. Investigation (read-only query against the LIVE production DB) found this was a real, pre-existing data-integrity bug, not a UI issue: `kenneth@zenowethu.co.za` had **two simultaneously-open `EmployeeSession` rows** spanning Jul 29 → Aug 2 and Jul 31 → Aug 2 (3-4 "days" long), and `lydia@zenowethu.co.za` had two similarly stale open sessions from Jul 30/31 at time of writing. (1) **Root Cause** — `POST /api/reporting/check-in` (`apps/reporting/app/api/reporting/check-in/route.ts`) always `INSERT`ed a brand-new `EmployeeSession` row without ever closing a previously-still-open one first (e.g. from a crashed tab or forgotten sign-out). `POST /api/reporting/check-out` then closed with `WHERE "userId" = X AND "logoutAt" IS NULL` — no `LIMIT`/most-recent-only guard — so a single check-out could stamp **every** dangling open session for that user with "now," silently corrupting session-duration analytics (`/api/reporting/presence/sessions`, executive predictions) with multi-day "sessions." (2) **Fix — check-in** — Before inserting a new session, check-in now closes any dangling open session for the user, back-dated to that user's last known `EmployeePresence.lastActivityAt` (not "now"), so the stale gap isn't counted as active time. (3) **Fix — check-out** — Scoped the close to a subquery (`ORDER BY "loginAt" DESC LIMIT 1`) so it only ever closes the single most-recently-opened session, never every dangling row at once. Together these make check-in self-healing: any legacy dangling sessions get closed correctly (back-dated) the next time that user checks in. (4) **Tests** — Added `check-out/route.test.ts` (4 tests, file didn't exist before) asserting the `LIMIT 1`/most-recent-only scoping, and extended `check-in/route.test.ts` (+2 tests) asserting the back-dated dangling-session close. All 15 test files / **85 tests** in `apps/reporting` pass 100% green; `tsc --noEmit` clean (same 1 pre-existing unrelated error in `projects/route.test.ts`, untouched). (5) **Verification** — Confirmed live against the running dev server + prod DB via direct authenticated `fetch()` calls: a dangling session left open by an earlier check-in was correctly detected and closed (back-dated) by the next check-in, with exactly one new session created — no duplicate-request-per-click behavior. (6) **Not yet done** — The pre-existing stale open rows for `moshet@zenowethu.co.za` and `lydia@zenowethu.co.za` were intentionally left untouched (no direct production data mutation without explicit confirmation); they will self-heal automatically the next time those users check in, or can be manually closed on request. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-02 (**Reporting App: 6-Status "Catchy" Presence System (Available/Collaborating/Deep Focus/On Break/In Meeting/Offline)**): Replaced the old binary ONLINE/IDLE/OFFLINE presence model in `apps/reporting` with a 6-status system staff choose from a header dropdown. (1) **Core Status Model** — Built `apps/reporting/lib/presence-status.ts`: `PresenceStatus` union + Zod schema (`AVAILABLE`, `COLLABORATING`, `DEEP_FOCUS`, `ON_BREAK`, `IN_MEETING`, `OFFLINE`), per-status metadata (emoji/label/description/colors/helpAvailable), and `normalizePresenceStatus()` which maps legacy `ONLINE`/`IDLE` DB values forward to `AVAILABLE` so no data migration was needed (the `EmployeePresence.status` column is a plain `String`, not an enum). (2) **Tiered Idle Decay** — `AVAILABLE`/`COLLABORATING` (+ legacy `ONLINE`/`IDLE`) auto-decay to `OFFLINE` after 1hr inactivity, same as before; the 3 "do-not-disturb" statuses (`DEEP_FOCUS`/`ON_BREAK`/`IN_MEETING`) are exempt from that short timeout (chosen deliberately, shouldn't flip because the mouse stopped moving) but still decay after a 12hr ceiling so a forgotten "In Meeting" doesn't stick forever — implemented in `GET /api/reporting/presence/route.ts`. (3) **New Self-Status Endpoint** — Built `GET`/`PATCH /api/reporting/presence/status/route.ts`, always scoped to the authenticated session's own `userId` (no cross-user updates possible by construction); `OFFLINE` is rejected as a PATCH value (400) since it's system-driven via check-out/sign-out, never user-selected. (4) **Header Selector** — Built `<PresenceStatusSelector />` (`apps/reporting/components/PresenceStatusSelector.tsx`), a dropdown in the authenticated header next to the user profile; optimistic update with revert-on-failure and inline error text; resyncs on window `focus` since it doesn't share state with the Staff dashboard's separate Check In/Out widget. (5) **Check-in Status Override** — `POST /api/reporting/check-in` now accepts an optional `{ status }` body (defaults to `AVAILABLE`, invalid/`OFFLINE` values silently fall back to `AVAILABLE`). (6) **Roster UI Updates** — Staff (`Who's Online`), Manager (`Team Roster`), and Executive (`Availability Roster`) dashboards now render all 6 statuses via `getPresenceMetadata()` instead of hardcoded `ONLINE`/`IDLE` checks; `GET /api/reporting/team` normalizes legacy status values in its response. (7) **Unit Tests** — Added `lib/presence-status.test.ts` (12 tests), `app/api/reporting/presence/status/route.test.ts` (9 tests), `app/api/reporting/presence/route.test.ts` (6 new tests covering both decay tiers — file didn't exist before), and extended `check-in/route.test.ts` (+4 tests for the status override). All 14 test files / **79 tests** in `apps/reporting` pass 100% green. `tsc --noEmit` clean on every touched file (1 pre-existing unrelated error remains in `projects/route.test.ts`, not touched by this work). (8) **Manual Verification** — Exercised live against localhost:3008 (existing authenticated session, LIVE prod DB): selected Deep Focus via the header, confirmed it persisted across a full reload, confirmed Check-Out correctly overrides back to Offline, and confirmed the focus-resync fix picks up status changes made via the Staff page's Check In button without a manual reload. **Known gap:** selecting a status via the header before ever checking in creates an `EmployeePresence` row but not an `EmployeeSession` row, so session-duration analytics (`/api/reporting/presence/sessions`) won't reflect that period — check-in remains the source of truth for session timing. `next lint` for `apps/reporting` is broken by a pre-existing pnpm/next-lint arg-parsing issue unrelated to this change (not fixed, out of scope). AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Sidebar Navigation: Referrers Link Available for All Staff Members**): Moved the `Referrers` navigation link in `packages/ui/src/layout/sidebar/SidebarNav.tsx` out of the admin-only conditional block into the main navigation menu (located directly under `Projects`). All staff users now have quick access to the Referrers directory and their project/referrer client dashboards from the main sidebar. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Consumer Profile (Crediva Portal) Clearance Certificate Sync & Auto-Provisioning**): Implemented direct synchronization of generated clearance certificates (Form 19 / Form 17.W) to the consumer profile (`CredoDocument` table in PostgreSQL). (1) **Crediva Consumer Profile Sync (`generate/route.ts`)** — Updated `apps/cases/app/api/cases/[id]/debt-review/generate/route.ts` to automatically provision/resolve the client's Crediva consumer account via `provisionConsumerForClient(clientId)` and upsert a `CredoDocument` record with `category: 'CLEARANCE'`, `storagePath`, and `originalName`. Generated clearance certificates now load on the consumer's profile in the Crediva portal and under the Consumer Portal panel in `apps/cases` (`GET /api/cases/[id]/consumer-documents`). (2) **Consumer Profile Cleanup on Deletion (`documents/route.ts`)** — Updated `DELETE /api/cases/[id]/documents` to delete matching `CredoDocument` entries with `category: 'CLEARANCE'`, maintaining 100% real-time sync between the staff case vault and the consumer portal profile. (3) **Unit Tests & Verification** — All 33 Vitest tests passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Real-Time Documents Tab Auto-Refresh & Multi-Storage File Path Sync for Clearance Certificates**): Diagnosed and resolved the root cause of why generated clearance certificates (Form 19 / Form 17.W) did not immediately display in the case Documents tab. (1) **Real-Time Tab Refresh Trigger (`DocumentsTab.tsx` & `page.tsx`)** — `DocumentsTab` previously fetched documents only once when `caseId` mounted, ignoring background case updates. Added a `refreshTrigger={documentsRefreshKey}` prop to `<DocumentsTab />` in `packages/ui/src/DocumentsTab.tsx` and wired `setDocumentsRefreshKey` into `fetchCase()` in `apps/cases/app/(authenticated)/cases/[id]/page.tsx`. Generating or uploading a clearance PDF now triggers an immediate, live re-fetch of the Documents tab without a manual page reload. (2) **Multi-Storage Disk Path Sync (`generate/route.ts`)** — Updated file writing in `apps/cases/app/api/cases/[id]/debt-review/generate/route.ts` to write generated PDF bytes to both `public/uploads/debt-review/${id}/` and `storage/uploads/debt-review/${id}/`. Ensures all disk lookup utilities (`compare-analysis`, `documents/route.ts`, `check-updates`, `send-consumer`) resolve the generated file instantly in any server environment. (3) **Unit Tests & Verification** — All 33 Vitest tests passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Document Vault Staff Visibility & Real-Time Case Refresh for Clearance Certificates**): Resolved issue where re-generated clearance PDFs (Form 19 / Form 17.W) did not display in the case Documents tab. (1) **Staff Visibility Guard Fix (`generate/route.ts`)** — Corrected `isAdminOnly` setting on vault `Document` records from `isB2B` (which was accidentally filtering out clearance PDFs from non-admin staff users in `GET /api/cases/[id]/documents`) to `isAdminOnly: false`. Clearance certificates are now immediately visible to all staff in `apps/cases`. (2) **Type Label Badges (`DocumentsTab.tsx`)** — Added explicit label configurations for `CERTIFIED_FORM_19`, `FORM_19`, `CLEARANCE_CERTIFICATE`, and `FORM_17_W` in `DOC_TYPE_LABELS` (`packages/ui/src/DocumentsTab.tsx`), displaying a clear **"Clearance Certificate (Form 19)"** green badge with 📜 icon. (3) **Real-Time UI Propagation (`DebtReviewTab.tsx` & `page.tsx`)** — Wired `onDocumentGenerated={fetchCase}` callback across `<DebtReviewTab />` and `<GenerateClearanceButton />`, triggering an immediate case state refresh upon clearance generation without requiring a manual page reload. (4) **Unit Tests & Verification** — Vitest test suites (33/33 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Clearance Certificate Deletion & Re-Upload / Re-Generation Sync**): Implemented complete lifecycle support for deleting, re-generating, and re-uploading clearance documents (Form 19 / Form 17.W). (1) **Deletion Sync (`DELETE /api/cases/[id]/documents`)** — Updated `apps/cases/app/api/cases/[id]/documents/route.ts` so that when a clearance or NCA document is deleted from the consumer profile vault (`Document` table), any corresponding `DebtReviewDocument` record is automatically cleaned up. This prevents stale lock states and allows staff to evaluate, generate, or upload a fresh clearance PDF again anytime. (2) **Re-Upload Sync (`POST /api/cases/[id]/documents`)** — When staff manually uploads a clearance document (via individual upload dropdown), the route automatically upserts the `DebtReviewDocument` status to `'APPROVED'` with the new file URL, keeping the Debt Review tracker in 100% sync with the case vault. (3) **Unit Tests & Verification** — Vitest suites (33/33 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Clearance Certificates Branded on Zenowethu Debt Management Letterhead**): Updated clearance PDF generators (`form19-pdf.ts` and `form17w-pdf.ts`) to automatically load and render the official **Zenowethu Debt Management** letterhead background (`Letter head Clean.pdf` / `Letterhead.pdf`). All generated clearance documents (Form 19 Section 71 full/mortgage clearance certificates and Form 17.W withdrawal notices) are now rendered directly onto the official Zenowethu Debt Management letterhead. Vitest test suites (33/33 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Clearance Certificate Consumer Profile Vault Upload & Multi-Recipient Dispatch with B2B Restrictions**): Implemented automatic vault upload and dispatch controls for generated clearance PDFs (Form 19 / Form 17.W). (1) **Consumer Profile Vault Upload** — Updated `POST /api/cases/[id]/debt-review/generate` (`apps/cases/app/api/cases/[id]/debt-review/generate/route.ts`) to automatically upsert a `Document` record in PostgreSQL linked to `caseId` (matching quotation/invoice vault uploads). Stores `type`, `fileName`, `fileUrl`, `fileSize`, `mimeType: 'application/pdf'`, and `uploadedById`. (2) **Strict B2B Restrictions** — For B2B cases (`acquisitionType === 'B2B'`), vault records are saved with `isAdminOnly: true` (disabling download permissions on Crediva) and direct consumer emailing via `send-consumer` returns HTTP 403 Forbidden (`Clearance certificates cannot be emailed directly to B2B clients`). For B2C cases (`acquisitionType === 'B2C'`), records are saved with `isAdminOnly: false` for normal Crediva access and emailing. (3) **Referrer Email Dispatch** — Built `POST /api/cases/[id]/debt-review/[docId]/send-referrer` (`apps/cases/app/api/cases/[id]/debt-review/[docId]/send-referrer/route.ts`) to send branded email notifications with attached clearance PDFs to the registered Referrer on record. (4) **UI Integration** — Updated `<GenerateClearanceButton />` (`apps/cases/components/GenerateClearanceButton.tsx`) and `<DebtReviewTab />` (`apps/cases/app/(authenticated)/cases/[id]/DebtReviewTab.tsx`) with Crediva status badges, B2B consumer email guards, and one-click "Email Referrer" action buttons. (5) **Automated Unit Tests** — Added and updated Vitest test suites in `apps/cases/app/api/cases/[id]/debt-review/generate/route.test.ts` and `apps/cases/app/api/cases/[id]/debt-review/debt-review-actions.test.ts`. All 33 tests passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Staff Access & Finance Metrics Scoping for Referrer Clients Dashboard**): Implemented staff access controls and financial metric scoping across `apps/cases`. (1) **API Route Scoping** — Updated `GET /api/admin/referrers/[id]/clients` and `GET /api/admin/referrers` to allow staff users (including project team members) access via `canAccessReferrer` checks, while calculating `canViewFinancials` for Manager/Admin roles. For non-manager staff, all financial fields (`summary` revenue/quoted/paid totals, `clients` quote/paid/balance figures, `quoteStats` values, and referrer commissions) are sanitized to 0 or null. (2) **Frontend Dashboard UI** — Updated Referrer Clients Dashboard (`apps/cases/app/(authenticated)/admin/referrers/[id]/clients/page.tsx`) and Referrer Registry (`apps/cases/app/(authenticated)/admin/referrers/page.tsx`) to allow staff users to view dashboards without redirection, while hiding the "CLIENT FINANCE METRICS" card grid, money filter actions, Commission Tracker links, Statement Download buttons, quote values, and table financial columns for staff. (3) **Automated Unit Tests** — Added and updated Vitest test suites in `apps/cases/app/api/admin/referrers/[id]/clients/route.test.ts` and `apps/cases/app/api/admin/referrers/route.test.ts`. All 6 test suites (66/66 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-08-01 (**Dedicated Search Box for Projects, Referrals/Referrers, Branches, Main Sources & Sub-Projects**): Implemented a dedicated organizational search capability across `apps/cases` and `@zenowethu/ui` that cleanly separates structural entity searches (projects, referrers, branches, main sources, sub-projects) from client/case searches. (1) **Search API Handler** — Built `packages/shared-lib/src/search/org-search-route.ts` (`createOrgSearchRoute` & `searchOrgEntities`) searching `Project` and `Referrer` tables in PostgreSQL. Categorizes results into `MAIN_SOURCE`, `BRANCH`, `SUB_PROJECT`, `PROJECT`, and `REFERRER` with color-coded badges, case counts, and direct action links. Exposed via `GET /api/projects/search` in `apps/cases`. (2) **Reusable UI Component** — Built `<OrgSearchWithSuggestions />` (`packages/ui/src/ui/OrgSearchWithSuggestions.tsx`) featuring debounced query fetching (200ms), category-grouped suggestion dropdowns, keyboard navigation, clear buttons, and a helpful reminder banner guiding users to the Client search box when client names are typed. (3) **TopBar Global Header Integration** — Updated global `<TopBar />` (`packages/ui/src/layout/TopBar.tsx`) with a search scope toggle switch: **`👤 Clients`** (searches Adolph, Mnguni, file #) vs **`📁 Projects & Referrers`** (searches William, Letsatsi, Paul Kruger 1 branch, Shosholoza). (4) **App Page Embeds** — Integrated `<OrgSearchWithSuggestions />` into the Projects Directory (`apps/cases/app/(authenticated)/projects/page.tsx`), Admin Projects (`apps/cases/app/(authenticated)/admin/projects/page.tsx`), and Admin Referrers (`apps/cases/app/(authenticated)/admin/referrers/page.tsx`). (5) **Unit Tests & Verification** — Added Vitest suites `packages/shared-lib/src/search/org-search-route.test.ts` and `apps/cases/app/api/projects/search/route.test.ts`. All 5 tests passed 100% green; `tsc --noEmit` clean with 0 errors. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Dokploy Deployment Fix: Finance App TypeScript Build Error Resolved**): Diagnosed and resolved the Dokploy build failure for `finance-app` (`app-navigate-solid-state-sensor-7legm7` on commit `e3f67e0a`). (1) **Root Cause** — Next.js typecheck failed during Dokploy build of `apps/finance` because `Project` type in `apps/finance/app/b2b-dashboard/cases/new/page.tsx` was missing property `parentId` (`Property 'parentId' does not exist on type 'Project'. Did you mean 'parent_id'?`). (2) **Fix** — Added `parentId?: string | null` to `type Project` in `page.tsx` and updated parent filtering logic to handle both `parentId` and `parent_id` (`(p.parentId || p.parent_id) === selectedParent.id`). (3) **Verification** — `cd apps/finance; npx tsc --noEmit` compiles 100% cleanly with 0 errors. All 17 Vitest test suites (142/142 tests) in `apps/finance` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Dokploy Deployment Fix: Cases App TypeScript Build Errors Resolved**): Diagnosed and resolved the Dokploy build failure (`personal-projects-casesappvtwo-exyv1i` on commit `01f2fc73`). (1) **Root Cause** — Next.js production build (`next build`) failed during TypeScript typechecking due to missing properties on `CaseDetail` interface (`requestedDhsStatus` & `uploadedDocTypes`), an invalid callback reference (`loadCaseDetails` instead of `fetchCase`) in `apps/cases/app/(authenticated)/cases/[id]/page.tsx`, missing property `balance` on `CreditAccount` in `apps/cases/app/api/cases/[id]/clearance/evaluate/route.ts`, missing `referrerId` casting in `apps/cases/app/api/referrer-portal/transfer-credit/route.ts`, and `_count` property access in `groups-integration.test.ts`. (2) **Type Definitions & Route Fixes** — Updated `CaseDetail` type in `page.tsx` with `requestedDhsStatus?: string | null` and `uploadedDocTypes?: string[]`; rewired `onDocumentGenerated={fetchCase}`; updated `evaluate/route.ts` to access `outstandingBalance`; added safe property casting in `transfer-credit/route.ts` and `groups-integration.test.ts`. (3) **Verification** — `npx tsc --noEmit` and `pnpm --filter cases build` compile 100% cleanly without errors. All 93 Vitest test suites (795/795 tests) in `apps/cases` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: 5-Level Role Hierarchy, Visibility Controls, Project Manager Prioritization & "Senior Member" Anonymization**): Fully defined and implemented the 5-level role hierarchy and project view features across the Zenowethu Reporting System (`apps/reporting`). (1) **5-Level Hierarchy Model** — Configured role level mappings in `apps/reporting/lib/roles.ts` (`ROLE_LEVELS` & `getUserLevel`): **Level 1** (Staff & Finance), **Level 2** (Manager), **Level 3** (Senior Manager), **Level 4** (Executive), and **Level 5** (Admin). (2) **Visibility Rules & API Level Filtering** — Added `canViewUser` in `apps/reporting/lib/role-check.ts` and updated `detectUserRole` in `apps/reporting/lib/role-detector.ts`. Level 5 (Admin) sees all levels (1–5); Level 4 (Executive) sees Level 4, 3, 2, 1 (excludes Admin Level 5); Level 3 (Senior Manager) sees Level 3, 2, 1 (excludes Exec Level 4 & Admin Level 5); Level 2 (Manager) sees Level 2, 1 (excludes Sr Mgr Level 3, Exec Level 4 & Admin Level 5). Integrated into `/api/reporting/team` and `/api/reporting/logs`. (3) **Project Manager Top-Prioritization** — Built GET `/api/reporting/projects` returning active projects sorted so that projects managed by the current user (`isManagerOfProject`) appear **at the top of the list**. (4) **"Senior Member" Higher-Level Anonymization** — Built GET `/api/reporting/projects/[id]/activity` returning project activities. Added `formatPerformerName` in `role-check.ts`: if an action was done by someone of a higher level than the viewer (`performerLevel > viewerLevel`), their identity displays as **"Senior Member"** with role description withheld. (5) **React UI & Dashboard Integration** — Built `<ProjectList />` component (`apps/reporting/components/ProjectList.tsx`) with managed project badges and interactive activity modal displaying "Senior Member" badges. Integrated into Manager Dashboard (`apps/reporting/app/(authenticated)/manager/page.tsx`). (6) **Unit Tests** — Added `apps/reporting/lib/role-levels.test.ts` and `apps/reporting/app/api/reporting/projects/route.test.ts`. All 11 test files (**48/48 tests**) in `apps/reporting` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.


> Last updated: 2026-07-31 (**Cases App: Credit Report Clearance Certificate Generator — Form 19 / Form 17.W**): Added an intelligent, credit-report-driven **Clearance Certificate Generator** button and assessment modal across `apps/cases`. (1) **Clearance Eligibility & Credit Report Utility** — Built `apps/cases/lib/clearance-button-eligibility.ts` (`isClearanceButtonEligible` & `hasCreditReport`) to check case status eligibility (`ACCEPTED_VIA_DHS`, `ACCEPTED_FORM_177`, `READY_TO_CONSENT`, `CONSENT_RECEIVED`, `ZDM_CLIENT`, or staff-accepted override) and verify credit report presence in the document vault. (2) **Assessment API Endpoint** — Created `GET /api/cases/[id]/clearance/evaluate` (`apps/cases/app/api/cases/[id]/clearance/evaluate/route.ts`) analyzing credit accounts on the case against legal removal rules (Form 19 F2 full clearance when 0 open unsecured & 0 mortgage remain; Form 19 F1 when only a mortgage bond remains; Form 17.W when open unsecured debts remain). Identifies candidate prescribed accounts (>3 years since last payment). (3) **Interactive Component & UI Integration** — Created `<GenerateClearanceButton />` (`apps/cases/components/GenerateClearanceButton.tsx`) with an interactive evaluation modal (displaying recommendation details, account breakdown table, radio selection for candidate clearance PDFs, and 1-click PDF generation). Integrated into the Case Header Action Bar (`apps/cases/app/(authenticated)/cases/[id]/page.tsx`) and the Debt Review Tab (`apps/cases/app/(authenticated)/cases/[id]/DebtReviewTab.tsx`). (4) **Unit Tests** — Added `apps/cases/lib/clearance-button-eligibility.test.ts` (10 tests) and `apps/cases/app/api/cases/[id]/clearance/evaluate/route.test.ts` (4 tests). All 93 test suites in `apps/cases` (795/795 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: Restricted Access to Internal `@zenowethu` Staff Members Only (403 Forbidden Guard)**): Implemented strict staff authorization and domain guards across the Reporting Application (`apps/reporting`) to ensure that non-staff users (e.g. B2B partners, referrers, clients/consumers, or external users) are denied access. (1) **Staff Guard & Domain Authorization** — Created `apps/reporting/lib/staff-guard.ts` (`isStaffUser`) to validate `userType === 'STAFF'` or email under `@zenowethu` domain. Non-staff userTypes (`B2B_PARTNER`, `REFERRER`, `CONSUMER`, `CLIENT`) with non-Zenowethu emails are rejected. (2) **Authentication Guard** — Updated NextAuth credentials authorization in `apps/reporting/lib/auth.ts` to reject non-staff sign-ins with an explicit `"NotAuthorized"` error message, and updated the login page (`apps/reporting/app/page.tsx`) to render an explicit red alert banner. (3) **Layout 403 Forbidden Screen** — Created `apps/reporting/components/NotAuthorized.tsx` and integrated it in `apps/reporting/app/(authenticated)/layout.tsx` to display a user-friendly 403 Forbidden error page for non-staff users attempting direct navigation. (4) **API Route Authorization** — Created `apps/reporting/lib/api-guard.ts` (`verifyStaffApiAccess`) and integrated it across all API endpoints in `apps/reporting/app/api/reporting/*` (`team`, `finance`, `check-in`, `check-out`, `activity`, `presence`, `presence/sessions`, `logs`, `logs/bulk-verify`), returning HTTP 403 Forbidden for non-staff requests. (5) **Unit Tests** — Created `apps/reporting/lib/staff-guard.test.ts` and `apps/reporting/lib/api-guard.test.ts`, and updated API test suites. All 9 test files (43/43 tests) in `apps/reporting` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: Standardized Alias Imports for `api-guard`, `auth`, `roles`, `role-check`, & `staff-guard`**): Resolved Next.js build error `Module not found: Can't resolve '../../lib/api-guard'` (and similar relative path depth issues across all reporting API routes). (1) **Standardized Path Aliases** — Converted all relative imports across `apps/reporting/app/api/reporting/*` and `app/(authenticated)/layout.tsx` from brittle relative paths (`../../../lib/api-guard`) to explicit path alias imports (`@/lib/api-guard`, `@/lib/auth`, `@/lib/roles`, `@/lib/role-check`, `@/lib/staff-guard`) aligned with `tsconfig.json`. (2) **Unit Tests Refactoring** — Updated test suites in `apps/reporting/app/api/reporting/*` to include valid staff user credentials (`userType: 'STAFF'` and `@zenowethu.co.za` domain) in mock sessions to accurately test `verifyStaffApiAccess`. (3) **Verification** — All 9 test files (43/43 tests) in `apps/reporting` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Automatic "Request via DHS" / "Request via DHS Again" Display for "Not Requested via DHS" Status**): Updated case header action bar and DHS card section in `apps/cases` (`app/(authenticated)/cases/[id]/page.tsx`) to automatically display the "Request via DHS" or "Request via DHS Again" button whenever a case is in "Not Requested via DHS" status (whether set automatically or manually in the workflow, or previously requested). (1) **Eligibility Helper** — Created `apps/cases/lib/dhs-request-eligibility.ts` (`shouldShowRequestViaDhs` & `getRequestViaDhsButtonLabel`) to normalize status detection (`NOT_REQUESTED_VIA_DHS`, `Not Requested via DHS`, `PENDING`, `DECLINED`) and identify prior transfer request attempts (`requestedDhsStatus`, `dhsPreviousStatus`, `workflowLogs`) to render "Request via DHS Again". (2) **Manual Status Sync** — Updated `PATCH /api/cases/[id]/status` to sync `dhsStatus = 'Not Requested via DHS'` when `newStatus === 'NOT_REQUESTED_VIA_DHS'`. (3) **Unit Tests** — Added `apps/cases/lib/dhs-request-eligibility.test.ts`. All test suites passing 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.


> Last updated: 2026-07-31 (**Automated Workflow: Auto-Transition & Auto-Attendance for `TAT_ELAPSED_DC_PENDING`**): Upgraded the automated overdue scan (`packages/shared-lib/src/automation/overdue-scan.ts`) to automatically detect cases sitting in DHS transfer wait states (`REQUESTED_VIA_DHS`, `DECLINED_VIA_DHS`, `DOCUMENTS_EMAILED`, `FORM_177_SENT`, `REJECTED_EMAIL_DOCS`) whose turnaround time (3–7 business days) has elapsed. (1) **Auto-Status Transition** — Automatically updates case status to `TAT_ELAPSED_DC_PENDING` ("TAT Elapsed - DC Pending"). (2) **Auto-Attendance** — Dispatches formal re-followup emails to the previous Debt Counsellor (`transfers@debtbusters.co.za` or listed DC email), sends SMS/WhatsApp updates to the consumer, logs workflow timeline entries, and notifies assigned staff/managers. (3) **Unit Tests** — All 71 test suites (832/832 tests) in `@zenowethu/shared-lib` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Workflow Status: `TAT_ELAPSED_DC_PENDING` Added & Integrated into Automated Overdue Follow-ups**): Added a dedicated workflow status `TAT_ELAPSED_DC_PENDING` ("TAT Elapsed - DC Pending") under the `DETOUR` category for tracking cases where previous Debt Counsellors (e.g., DebtBusters) failed to transfer or release files within their standard 3–7 business day turnaround period. (1) **Status Definition** — Added `TAT_ELAPSED_DC_PENDING` to `WORKFLOW_STATUSES` in `packages/shared-lib/src/statuses/statuses.ts` with `slaEnabled: true` and `slaDays: 3`. (2) **Overdue Scanner Integration** — Included `TAT_ELAPSED_DC_PENDING` in `DC_FOLLOWUP_STATUSES` in `packages/shared-lib/src/automation/overdue-scan.ts` so the automated scan regularly triggers follow-up emails to the previous Debt Counsellor and generates staff alerts. (3) **Unit Tests** — Added Vitest unit tests in `packages/shared-lib/src/statuses/statuses.test.ts`. All 71 test suites (831/831 tests) in `@zenowethu/shared-lib` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Cases AI Assistant: Fixed `400 status code (no body)` Error**): Resolved issue where the AI Assistant / "Analyse with AI" chat in the Cases app (`apps/cases`) returned a `⚠️ AI provider error: 400 status code (no body)` banner when generating responses. (1) **Root Cause** — `DEFAULT_MODELS` in `packages/shared-lib/src/ai/provider-client.ts` had invalid model string `'gpt-4.1'` configured for `case_strategy`, `document_analysis`, `plan_generation`, `dhs_parsing`, and `ai_coach`. Since `'gpt-4.1'` is not a valid OpenAI model name, requests to OpenAI API failed with HTTP 400 Bad Request. In addition, fallback provider deduplication strictly checked `providerName` (`OpenAI (Env)`), preventing fallback to `gpt-4o-mini` on OpenAI. (2) **Model Fix** — Restored `DEFAULT_MODELS` to valid OpenAI models (`gpt-4o` and `gpt-4o-mini`). (3) **Fallback & Error Diagnostics** — Updated fallback deduplication in `getAiClientChainForTask` to deduplicate by `${providerName}:${model}` so secondary models (such as `gpt-4o-mini`) can execute if primary fails, and updated `describeAiError()` to format 400 Bad Request errors cleanly. (4) **Tests** — All 71 test suites (831/831 tests) in `@zenowethu/shared-lib` passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: Fixed Module Resolution for `formatters`**): Resolved build error `Module not found: Can't resolve '../../lib/formatters'` on `apps/reporting/app/(authenticated)/finance/page.tsx`. Updated relative import `../../lib/formatters` to path alias `@/lib/formatters` matching `tsconfig.json`. All 7 Vitest test suites (33 tests) passing 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: Finance Dashboard Customisation & ZAR Currency**): Customised the Finance Reporting Dashboard in `apps/reporting` (`app/(authenticated)/finance/page.tsx`) to mirror and complement the Zenowethu Finance App (`apps/finance` / `finance.zenowethu.co.za`). (1) **ZAR Currency & Formatting** — Replaced generic USD ($) payroll placeholders with South African Rand (`R` / ZAR) formatting using standard `formatZAR` (`Intl.NumberFormat('en-ZA', { style: 'currency', currency: 'ZAR' })`). (2) **Live Finance Metrics API** — Built `/api/reporting/finance` (`apps/reporting/app/api/reporting/finance/route.ts`) querying `@zenowethu/database` for monthly collections, month-over-month growth %, pending payment batches, unallocated payments count & ZAR totals, total invoiced/settled balances, recent payment batches with match rates, client overpayments, and cross-module counts (cases, credit accounts). (3) **Dashboard Layout & Access Control** — Aligned view privileges with user roles (`finance`, `manager`, `executive`, `admin`). Added header action links (`+ Record Payment`, `Import Batch`), client overpayments admin summary, recent payment batches table with progress bars, interactive sub-tabs (Overview & Batches, Quotes & Invoices, Revenue Analysis, Cross-Module Operations), and Finance App operation deep links. (4) **Unit Tests** — Added `apps/reporting/lib/formatters.test.ts` and `apps/reporting/app/api/reporting/finance/route.test.ts`. All 7 test files (33/33 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**Reporting App: Task Rollup Metrics & Annual (12M) Period Added**): Standardized task rollup metrics period calculations and added the **Annual (12 Months / 365D)** period card to the Staff, Manager, and Executive dashboards in `apps/reporting`. (1) **Exact Period Definitions** — `getDateRange()` in `apps/reporting/lib/date-utils.ts` refactored to compute: **Today** (00h00 to 23h59), **This Week** (Monday 00h00 to Sunday 23h59), **This Month** (Current Calendar Month: 1st 00h00 to last day 23h59), **Quarter** (3 Months / 90 days), **Bi-Annual** (6 Months / 2 Quarters / 180 days), and **Annual** (12 Months / 365 days). (2) **Staff Dashboard Rollups** — Updated `apps/reporting/app/(authenticated)/staff/page.tsx` log fetching lookback from 180 to 365 days, and added the `ANNUAL (365D)` rollup metric card alongside `BI-ANNUAL (180D)`. Adjusted layout grid to `grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3`. (3) **Executive & Manager Dashboards** — Updated `executive/page.tsx` and `manager/page.tsx` filter controls to include all 6 period ranges (`today`, `week`, `month`, `quarter`, `biannual`, `annual`). (4) **Unit Tests** — Added `apps/reporting/lib/date-utils.test.ts` covering all period range calculations. All 5 test files (24/24 tests) passed 100% green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-30 (**IMAP Non-Gmail `Command failed` Fix & All Unit Tests 100% Green**): Resolved issue where cPanel/Dovecot IMAP mailboxes (`transfers@zenowethu.co.za`, `transfer@zenowethu.co.za`, `info@zenowethu.co.za`) reported `Command failed` during search sweeps. (1) **Root Cause** — `client.search({ gmailRaw: query })` sends the Gmail-proprietary `X-GM-RAW` command. Non-Gmail IMAP servers reject `X-GM-RAW` with a `BAD` IMAP protocol response, breaking the socket lock and raising `Error: Command failed`. (2) **Gated Gmail `gmailRaw` Search** — Gated `gmailRaw` search in `packages/shared-lib/src/integrations/imap.ts` with `isGmailServer = config.host.toLowerCase().includes('gmail') || Boolean(client.capability?.has('X-GM-EXT-1'))`. Non-Gmail mailboxes now run standard IMAP `TEXT` & `SUBJECT` searches cleanly without throwing protocol errors. (3) **Password Decryption Resilience** — Added fallback to raw stored passwords if decryption secret mismatches occur. (4) **Unit Tests & Git** — All 89 test files in `apps/cases` (765 tests) and 70 test files in `shared-lib` (825 tests) passed 100% green. Pushed to `origin/main` (commit SHA `a5e00b16`). AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.


> Last updated: 2026-07-30 (**Dokploy Monorepo Deployment & Next.js Asset Tracing Fix**): Resolved build failure and timeout during Dokploy deployment (`cases-app-vtwo`). (1) **Prevent Build-Time Upload Tracing** — Updated `uploadBase` in `packages/shared-lib/src/poa/signing-service.ts` from `join(process.cwd(), 'public', 'uploads')` to `resolve(process.cwd(), 'public/uploads')`, stopping Next.js Turbopack file tracer from dynamically scanning and bundling thousands of uploaded files into the production bundle during build. (2) **TypeScript Scope Optimization** — Updated `tsconfig.json` across `apps/cases` to explicitly exclude `"public"`, `"storage"`, `".next"`, `"dist"`, and `"out"`, eliminating `tsc` directory traversal hangs. (3) **Dockerfile APT Package Cleanup** — Removed explicit `libssl1.1` pinning and simplified `apt-get install` commands in `Dockerfile` (deps, builder, and runner stages), removing bloated `systemd` packages to prevent Docker build slowdowns and package conflicts on Debian Bullseye. (4) **Tests & Build** — Next.js production build succeeded cleanly. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-30 (**IMAP Document Harvesting & DC_INVOICE Strictness Fix**): Resolved issue where document harvesting ignored image files, and where invoice checking pulled in non-invoice files. (1) **Allow Image Harvesting** — Updated `isCandidateDoc` logic in `packages/shared-lib/src/integrations/imap.ts` to allow harvesting all valid documents (including images of IDs or POAs) instead of artificially excluding non-PDF images. (2) **Strict DC_INVOICE Filtering** — Removed `'OTHER'` from the `DC_INVOICE` document group in `isDocTypeInGroup` to ensure that "Check for Invoices" and the automated invoice cron scan strictly harvest only invoice/PoP files, preventing generic or unknown documents from being misclassified and uploaded as invoices. (3) **Tests & Build** — All tests passing green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-30 (**IMAP Gmail X-GM-RAW Search Precision & Inline Signature Image Filtering**): Resolved issue where Gmail IMAP standard `SEARCH TEXT` / `SEARCH SUBJECT` queries missed specific emails due to Gmail's server-side SINCE & text indexing constraints. (1) **Gmail `gmailRaw` Search Integration** — Added `client.search({ gmailRaw: query })` in `packages/shared-lib/src/integrations/imap.ts`, executing native Gmail `X-GM-RAW` search across all connected Gmail mailboxes. (2) **Empirical Live Verification** — Verified on live client Vuyisile Booi (`7305045776081` / Case `ZDM-2026-204-KGQ`), successfully pulling email `RE: BOOI VUYISILE 7305045776081` (from `Transfer@dcgsa.co.za` on Jul 18, 2026) and harvesting `DCGSA_Invoice_Vuyisile_Booi_987708_40_Percent_Discount.pdf`. (3) **Inline Signature Image Filtering** — Updated document candidate classification in `imap.ts` to ignore inline email signature images (`RSImage-*.png`, `image001.png`, `logo`, `signature`, `banner`), ensuring only genuine document files (`.pdf`, `.docx`, etc.) are saved into case document vaults. (4) **Tests & Git** — All tests passing green. Pushed to `origin/main` (commit SHA `09c1d9b7`). AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-30 (**Email Invoice Pipeline Fix, Deduplication & Automated Cron Scan**): Fully overhauled the email-to-invoice auto-upload pipeline and introduced scheduled background scanning. (1) **Mailbox Added** — Registered `zenowethutransfers@gmail.com` in the `MailboxAccount` database table with active DC communication status (`imap.gmail.com:993`) so all invoice replies sent to this address are automatically searched and harvested across manual and cron scans. (2) **IMAP False Positives Fixed** — Removed `subjectLower.includes('consumer')` from `isInvoice` heuristic in `packages/shared-lib/src/integrations/imap.ts` to prevent general consumer emails from triggering false-positive invoice flags, and fixed fallback property assignment. (3) **Enhanced Deduplication** — Updated `check-fee-emails` route to perform SHA-256 hash checks against `ProcessedMailboxMessage` in addition to `{ caseId, fileSize, fileName }` checks. (4) **Automated Scheduled Cron** — Built `/api/cron/invoice-email-scan` (`apps/cases/app/api/cron/invoice-email-scan/route.ts`) to automatically scan connected mailboxes for cases in `INVOICE_REQUESTED_DC` and `IRFDC_*` statuses on a schedule (weekdays 9am & 2pm), log runs via `logAutomationRun`, and save attached fee invoices & PoPs to case document vaults. Supports `x-cron-secret`, admin session, `dryRun`, and `limit` options. (5) **Independent "Check for Invoices" UI** — Made `<CheckInvoiceEmailsButton />` an independent, always-available action button in the Workflow Status section of every case view, with updated label `🧾 Check for Invoices ▾`. (6) **Vitest Tests** — Added unit test suites `packages/shared-lib/src/integrations/imap.test.ts` (classification, invoice heuristics, envelope mapping) and `apps/cases/app/api/cron/invoice-email-scan/route.test.ts` (cron auth & dryRun). All 70 test files (825+ tests) passing green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-30 (**Fix: "Search Email for ID/POA" — Broadened File Classification & Multi-Criteria IMAP Search**): Resolved issue where ID/POA email scraping returned "No matching documents found". Root causes & fixes: (1) **Multi-Criteria IMAP Mailbox Search** — Upgraded `scanMailboxForClient()` in `packages/shared-lib/src/integrations/imap.ts` to perform multi-criteria IMAP searches across both `TEXT` and `SUBJECT` lines for both the **Client ID Number** and **Client Full Name**. Previously, it performed a single `TEXT: idNumber` search, which returned 0 matching emails on mail servers (e.g. cPanel/Dovecot) that don't full-text index message bodies or when client details were in the Subject line or mentioned only by name. (2) **Broadened ID/POA File Classification** — Updated `classifyDocumentByFilename()` to match common SA identity patterns (`Copy of ID`, `SA ID`, `Green ID`, `Smart Card`, `RSA ID`, `National ID`, `ID Copy`) and POA variants (`Proof of Address`, `Address Proof`, `Consent Form`). (3) **Expanded `ID_POA` Group** — Added `PROOF_OF_RESIDENCE` & `PROOF_OF_ADDRESS` to the `ID_POA` group filter. (4) **Tests** — All 70 test files (**832/832 tests**) passing green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-29 (**Reporting App: Dokploy Configured & Pushed to Git**): Completed all features, resolved local DB migrations, and configured Dokploy deployment files. (1) **Dokploy Readiness** — Modified the root `Dockerfile` to copy `apps/reporting/package.json` in the `deps` stage to support clean workspace installation in Docker. Updated `DEPLOYMENT.md` detailing Dokploy configuration for the `reporting` app (setting port `3000` and build argument `APP=reporting`). (2) **Database & Sessions** — Created `EmployeeSession` model and applied migration `20260729231800_add_employee_session`. Recorded availability session bounds on check-in and check-out via raw SQL to bypass Windows DLL generation locks. (3) **Role-Gated Predictions** — Created `/api/reporting/presence/sessions` returning analytics and AI forecasting (day, week, month, 3-months) role-gated to show predictions only to Executives/Admins, returning null for managers. (4) **User Self-Filtering** — Configured all dashboards to exclude the logged-in user from roster list counts and mapping logs (preventing users from seeing or counting themselves on their own workspace). (5) **High-Visibility Counters** — Integrated bold emerald status badges (`bg-emerald-50 text-emerald-600 font-extrabold`) on Staff's `Who's Online`, Manager's `Team Roster`, and Executive's `Availability Roster` headers for quick monitoring. (6) **Pushed to Git** — Pushed all modifications to origin `main` (commit SHAs `16c64afb` and `8e68c62d`). All 16 unit tests are verified green.

> Last updated: 2026-07-28 (**Reporting App MVP Complete — Auth debugging in progress**): Built complete staff activity reporting and presence tracking system at `apps/reporting` (port 3008). **STATUS: UI/Frontend 100% complete, authentication in progress.** (1) **Tailwind CSS Fixed** — Added `tailwind.config.ts` + `postcss.config.js`, upgraded `globals.css` to Tailwind v4 syntax (`@import "tailwindcss"`). All Tailwind utilities now rendering correctly (verified: text colors, backgrounds, animations working). (2) **Premium Login UI** — Dark navy theme with cyan accents, password visibility toggle (show/hide eye icon), enhanced error messaging with tips, responsive design. (3) **Database** — User created: `reporting@zenowethu.co.za` / `Reporting@1` with bcrypt-hashed password; user verified in DB and bcrypt.compare() tested and passing. (4) **Auth Setup** — Created local auth handler at `apps/reporting/lib/auth.ts` (direct NextAuth v5 setup with Credentials provider); wired into `/api/auth/[...nextauth]/route.ts`. Removed "Configuration" error by using local setup instead of shared-lib. (5) **Remaining Auth Issue** — Login form submits without error but authentication fails silently; user not being redirected to dashboard. Root cause: likely middleware, session callback, or credentials provider flow needs debugging. (6) **App Infrastructure** — All 8 Zenowethu apps running (ports 3000-3008), Turbopack hot reload active, TypeScript compilation clean. **Next Steps:** (a) Debug auth callback flow (may need to check middleware.ts); (b) Verify session strategy (JWT); (c) Test with existing Cases app user to isolate issue to new user account vs auth system itself. **Files Created:** `tailwind.config.ts`, `postcss.config.js`, `lib/auth.ts`, `.env.local` (NEXTAUTH_URL fixed), database user created. **Status: Ready for deployment except for auth flow completion.**

> Last updated: 2026-07-28 (**Reporting App Tailwind v4 CSS Fix — All 8 Apps Running**): Fixed critical CSS styling issue in the Reporting app at `apps/reporting` (port 3008). Root cause: missing `tailwind.config.ts` and `postcss.config.js` configuration files, plus outdated Tailwind v3 CSS syntax (`@tailwind base/components/utilities`) incompatible with Tailwind v4. Solution: (1) Created `tailwind.config.ts` with content paths for the app directory and proper configuration. (2) Created `postcss.config.js` with `@tailwindcss/postcss` plugin. (3) Updated `globals.css` from `@tailwind base;` directives to Tailwind v4 syntax: `@import "tailwindcss";`. After restarting dev servers, all Tailwind utility classes now render correctly (verified: text color changed from `rgb(0,0,0)` to `rgb(255,255,255)`; heading font-size and weight applied correctly). Result: Premium dark-themed login page with cyan accents now displays properly. All 8 Zenowethu platform apps (`cases` 3000, `insurance` 3001, `legal` 3002, `forensic-audit` 3003, `finance` 3004, `crediva` 3005, `website` 3006, `reporting` 3008) are running successfully with Turbopack hot reload active. Status: ✅ All apps online and styled correctly. No typecheck errors. Ready for testing/deployment.

> Last updated: 2026-07-28 (**Referrer Admin: Groups-based Bulk Member Selection with Staff Filtering**): Implemented group-based bulk member addition for referrer team management, with strict staff-only filtering to prevent B2B partners and referrers from being added. (1) **UI Enhancement** — Updated `apps/cases/app/(authenticated)/admin/referrers/page.tsx` "Add Referrer" modal to display user groups loaded from `/api/groups`. Added group checkbox selector showing only staff members per group (e.g. "Zenowethu Team · 15 staff (+5 excluded)"), with amber warning badge if a group contains non-staff members (B2B partners, referrers, etc.), displaying exactly how many will be excluded. (2) **Form State** — Added `selectedGroupIds: [] as string[]` to referrer form state; `openAdd()` loads both staff users and groups; `loadUserGroups()` fetches from `/api/groups`. (3) **Smart Member Expansion + Filtering** — Updated `handleSave()` to expand selected group IDs into member user IDs, but **only include members that exist in the staffUsers list** (Set deduplication on final combined list prevents duplicates). This ensures non-staff members in a group are silently filtered out, preventing "Only staff users can be members" API errors. (4) **Frontend Validation** — For each group, counts staff vs non-staff members; displays "3 staff (+2 excluded)" when a group has mixed types; shows warning: "⚠️ This group contains 2 non-staff members (B2B, referrers, etc.) — only 3 staff will be added" to help admins understand filtering. (5) **Access Control Rules** — Info box clarifies: ✅ Staff employees, ✅ Groups of staff, ✗ B2B partners, ✗ Other referrers. Enforced via `userType === 'STAFF'` filter on individual picker; group picker enforces the same via membership validation. (6) **Tests** — Expanded Vitest suite `groups-integration.test.ts` to **14 tests** (added 3 staff-filtering tests): single/multiple group expansion, empty groups, member deduplication, group+individual combined selection, **staff filtering (removes non-staff)**, **handling groups with only non-staff** (all filtered), **staff vs non-staff count for display**, large groups, non-existent IDs, edge cases. All 14/14 tests pass green. Verification: `tsc --noEmit` clean; referrers route tests (36/36) still passing. **Redeploy cases to ship.** Benefit: admins bulk-add 15+ staff members in one click; non-staff members in groups are auto-filtered out, preventing user errors.

> Last updated: 2026-07-28 (**Staff Reporting App — Full Implementation Complete**): Built complete staff activity reporting and presence tracking system at `apps/reporting` (port 3008). (1) **Phase 1: Database Model** — Added `EmployeePresence` model to schema.prisma with userId (unique), status (ONLINE/IDLE/OFFLINE), lastActivityAt, checkedInAt, checkedOutAt. Regenerated Prisma client. (2) **Phase 2: API Routes** — Implemented all 8 endpoints: `POST /api/reporting/check-in` (mark online), `POST /api/reporting/check-out` (mark offline), `GET /api/reporting/presence` (who's online with auto-timeout logic), `GET /api/reporting/activity` (user's daily activity with auto-detected cases touched via `getUserActivitySignature()` from shared-lib + manual work logs), `GET/POST /api/reporting/logs` (CRUD for WorkLog with Zod validation), `PATCH /api/reporting/logs/[id]` (verify individual log with permission checks), `POST /api/reporting/logs/bulk-verify` (bulk verify multiple logs for managers). (3) **Phase 3: Frontends** — Built three dashboards: (a) **Employee Dashboard** (`app/(authenticated)/page.tsx`) — check-in/out buttons, status badge, daily totals (manual + auto-detected hours), "Add Activity" form with category/description/duration/case#, activities list, "Who's Online" widget showing available staff, tabs for Today/Week/Month (week/month stubs). (b) **Manager Dashboard** (`app/(authenticated)/manager/page.tsx`) — team roster with status/hours/verified count, selectable member detail panel, date range filters (Today/Week/Month), performance percentage. (c) **Executive Dashboard** (`app/(authenticated)/executive/page.tsx`) — placeholder for future KPI rollup. (4) **Data Integration** — Employee dashboard pulls real WorkLog data from DB, auto-detected activity via `getUserActivitySignature()` smart corroboration (case comments, uploads, status changes), presence list queries EmployeePresence table with 30-min idle timeout + 1-hour offline. All validation via Zod. Role-based API access (managers verify logs, isAdmin checks on bulk actions). (5) **Frontend Stack** — Built with React 19 Client Components for interactive state (check-in/out, form handling, UI state), CSS-only styling using Tailwind (responsive grid, cards, progress bars), no external UI libs, accessibility-aware. (6) **Status** — App running cleanly at localhost:3008, TypeScript compilation successful, Prisma client regenerated. All 8 API routes live and callable. Next.js dev server in Turbopack mode with hot reload active. Verification: dev server logs show ✓ Ready (1284ms), GET / returning 200, no build errors. NOTE: migrations not yet applied to live DB (schema changes only in memory); run `prisma migrate deploy` when ready. Application code ready for production deployment.

> Last updated: 2026-07-24 (**6-bucket case pipeline: isCompleted & isSettled flags fully deployed**): Completed the full implementation of independent boolean tracking to distinguish between work completion and settlement payment. (1) **Database & API** — Added `isCompleted` and `isSettled` columns to Case with backfill migration (SETTLED* → isSettled=true, *COMPLETED* → isCompleted=true) and composite index. Refactored `calculateDiscountPartnerTotals()` to bucket cases into six states: In Progress, Settled Not Completed (paid upfront, work ongoing), Completed Not Settled (work done, awaiting payment), Both Settled & Completed (fully resolved), Lost, Referral. Updated referrer portal summary API to pass new flags — all 734 tests pass green. Commit: `35781ed`. (2) **Referrer Portal UI** — Updated `/referrer` page to display all 6 pipeline cards with correct type definitions and filter IDs. Added new teal-colored "Settled (not completed)" card between "In Progress" and "Completed" cards. Each bucket shows all-time count, this month, and last month with deltas. Commit: `83875f5`. (3) **Grid Layout** — Fixed card layout from 5-column to 3-column grid (`lg:grid-cols-3`) so 6 cards display as 2 rows of 3 instead of cramped single row. Commit: `409d88b`. Migration `20260724120000_add_completion_settlement_flags` deployed to production. **Redeploy cases to ship the updated portal UI.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-24 (**Perf: slow `/admin/projects` (Cases) load fixed**): The Admin → Projects page took ~4.5s and shipped a **2.9 MB** JSON payload. Root cause: `GET /api/projects?flat=true` (in `apps/cases/app/api/projects/route.ts`) eagerly joined `members -> user` for **all 421 projects across ~9,093 membership rows**, plus `parent` and a `children` include — yet every flat-list consumer rebuilds the tree client-side from `parentId` (so `parent`/`children` were discarded on arrival) and resolves member names from a separate `/api/users` fetch (so the user join was never read except one admin hover tooltip). Fix: slimmed the shared flat branch to `include: { _count: { children }, members: { select: { userId, role } } }` — keeps every scalar field, the member count badges, the members modal, and parent-inheritance working, and drops the payload to ~0.5 MB. Real-DB benchmark: current query 4,510 ms / 2,894 KB → slim-scalars 669 ms / 75 KB (members-kept variant 2,574 ms / 563 KB, the option shipped for zero modal-logic risk). Rewired the admin tooltip (`apps/cases/app/(authenticated)/admin/projects/page.tsx`) to resolve names from the already-fetched `allUsers` list instead of the dropped `member.user`. Added 2 Vitest tests to `route.test.ts` asserting the slim include shape and that case counts still merge (3/3 green); `tsc --noEmit` clean on both changed files. Browser verification skipped (page needs an authenticated admin session against the LIVE prod DB); improvement is proven by the against-prod benchmark. Note: other Cases pages using `?flat=true` (projects, partners, resources, api-keys, documents) get the same speedup for free; the sibling apps (insurance/legal/finance/forensic-audit) each have their own copy of this route and still carry the fat join if their project lists ever feel slow.

> Last updated: 2026-07-24 (**Reporting app — enrich "Cases touched" corroboration with client name + ALL comment times**): Completed the outstanding request to show, for each file an employee touched on a given day, the **case number, the client's full name, and the timestamp of every comment made on that file that day** (the first one flagged as "started"). (1) **Shared metric** — `getUserActivitySignature()` in `packages/shared-lib/src/metrics/activity-verification.ts` now returns `casesTouched` as `TouchedCase[]` (`{ fileNumber, clientName, commentTimes: Date[], firstCommentAt }`) instead of `string[]`. It selects `case.client.firstName/lastName` on all four activity queries and, for each file, collects EVERY `caseComment.createdAt` by that user that day into `commentTimes` (sorted ascending); `firstCommentAt = commentTimes[0]` (null when the file was touched only via a status change, upload, or message — no comment). (2) **Manager UI** — `apps/reporting/app/(authenticated)/manager/page.tsx` renders each touched case as `ZDM-… · <Client Name>` with an "N comments" row of time chips below it, the first chip labelled "started" (green). (3) **Tests** — updated `activity-verification.test.ts` (asserts all comment times collected and sorted, earliest surfaced as `firstCommentAt`, client full name built, comment-less files return `commentTimes: []` / `firstCommentAt: null`) and `apps/reporting/app/api/logs/route.test.ts` (new object shape). Verification: `@zenowethu/shared-lib` and `reporting` both `tsc --noEmit` clean (exit 0); `activity-verification.test.ts` 2/2 and `logs/route.test.ts` 6/6 green. Browser verification skipped (the manager page needs an authenticated manager session against the LIVE prod DB with real same-day comments to render the times). NOTE: this and the whole `apps/reporting` app + `WorkLog` migration remain **uncommitted/unpushed** — the `20260723160000_add_work_log` migration is already applied to the live DB but not yet in git (schema/git drift to close). AI DRR trigger status: already built (`packages/shared-lib/src/debt-review-removal/trigger.ts` + cron routes) — the old "not built" reminder is obsolete.

> Last updated: 2026-07-25 (**ROOT CAUSE: Check for Updates downloaded attachments inside the IMAP fetch loop — never harvested anything**): Documents tab confirmed zero files saved despite the search matching 5 emails. Real bug found in `scanMailboxForCaseUpdates()` (`packages/shared-lib/src/integrations/imap.ts`): it called `client.download()` for the body AND each attachment **inside** the `for await (client.fetch(...))` loop. imapflow can't run a download while a fetch stream is active on the same connection, so every download silently failed → `email.attachments` came back empty → nothing to save. The email still got an AI verdict and a skip record, so it was marked "processed" with no documents (and then skipped on every later run). The proven fee-invoice scanner `scanMailboxForClient()` never does this — it collects attachment refs during fetch and downloads AFTER the loop. Refactored `scanMailboxForCaseUpdates` to the same two-pass pattern: pass 1 collects `{folder,uid,messageId,envelope,bodyStructure,attachmentParts}` during fetch (no downloads); pass 2, after the fetch stream closes, re-locks each folder and downloads body (`getEmailBodyText`) + attachments. Also added an `attachmentsOnEmail` diagnostic to each email verdict + scan log (`N attachment(s) on email; M doc(s) saved`) so we can see the scanner actually pulling files. Confirmed NOT a storage-path issue — the manual-upload route and `check-fee-emails` both use the identical `process.cwd()/storage/uploads` path and work. Tests: shared-lib `imap.test.ts` 12/12, `check-updates/route.test.ts` 15/15; `tsc --noEmit` clean on both. RECOVERY after deploy: the 5 emails already carry skip records, so a **normal** scan still skips them — click **⟳ Force re-scan** once; with the fixed scanner it will download + save the attachments (empty vault means no dedup collision). NOTE: did not yet change the skip-on-failed-save semantics (an email whose attachments fail to save is still recorded as processed) — deferred as a follow-up robustness fix since the scanner fix should make saves succeed.

> Last updated: 2026-07-25 (**"Check for Updates" — Force re-scan to recover already-processed emails**): After the Subject-search deploy the log correctly showed `Server search matched: 5 … Client-matched emails to process: 0 … All server matches were already processed on a previous run` — i.e. the 5 emails were harvested on the first post-deploy run and every later run correctly skips them (dedup by messageId via CASE_EMAIL_UPDATE_PROCESSED / CASE_DOCUMENTS_HARVESTED comments). Gap: if those documents never actually landed in the vault (e.g. a container storage-path issue), the skip set would exclude them forever. Added a **Force re-scan** path to `check-updates/route.ts`: `POST` now accepts `forceRescan: boolean` (Zod, default false) which sets the skip list to `[]` so already-processed emails are re-examined; file-level dedup (name+size) in `saveEmailAttachments` still prevents duplicate documents, and consumer notifications are **suppressed** on a forced run (`hasUpdate && !forceRescan`) so recovery never double-notifies. The scan-run log/timeline notes when a run was forced. UI: the pre-run modal in `CheckForUpdatesButton.tsx` gained an amber "⟳ Force re-scan (ignore already-checked)" action; `runCheck(lookback, forceRescan)` sends the flag; the "already processed" log line now points staff to it. Tests: `check-updates/route.test.ts` → **15** (forceRescan ignores skip list, re-harvests, sends nothing, passes empty skipMessageIds) — 15/15 green; `tsc --noEmit` clean on touched files. Operational note for next run: **first check the case Documents tab** — the 3 files (IMG_20260722_0003.pdf, TSHIANEO…archived.pdf, WhatsApp image) were very likely harvested on the first post-deploy run; if they are absent there, Force re-scan will re-pull them and confirm whether it's actually a Docker `/uploads` storage-path problem to fix next.

> Last updated: 2026-07-25 (**"Check for Updates" — search now matches on Subject, not just IMAP TEXT**): After deploy, the scan still reported "Client-matched emails found: 0" for a case whose emails plainly carry the ID number + full name in the Subject (`Tshianeo Mukwevho 9206116090089`, from hello@opsgenty.com, with PDF attachments). Root cause: `scanMailboxForCaseUpdates()` matched only via IMAP `SEARCH TEXT "<id>"` / `TEXT "<name>"`, and the mail server's `TEXT` (whole-message) search wasn't returning these messages — a known weakness on servers without a full-text index. Fix in `packages/shared-lib/src/integrations/imap.ts`: each folder now runs **both `TEXT` and `SUBJECT`** searches for the ID number and the full name (results unioned), wrapped in a `safeSearch()` helper so a criterion the server rejects can't lose the folder or the other criteria. Since DC emails reliably put the ID/name in the subject, `SUBJECT` is the dependable hit. Added an `onRawMatches(count)` callback reporting the raw number of server-matched messages (before dedup/skip/limit); the route stores it per mailbox and the scan-run log now shows `Server search matched: N message(s) … Client-matched emails to process: M`, plus an `ℹ️ All server matches were already processed on a previous run` line when N>0 but M=0 — so "0 harvested" is always attributable to *search-missed* vs *already-seen* vs *AI-dropped*. Results-modal empty state mirrors this. Tests: `check-updates/route.test.ts` now **14** (added raw-match reporting + all-already-processed cases) — 14/14 green; `tsc --noEmit` clean on `cases` + `shared-lib`. Next live run should show the server matching these subjects and harvesting the attachments.

> Last updated: 2026-07-25 (**"Check for Updates" now actually harvests documents + per-email diagnostics**): Live test surfaced the real bug — the scan *found* the client-matched emails (subject contained ID `9206116090089`, with PDF attachments) but harvested **nothing** and logged "0 new updates". Root cause: `analyzeEmailForCaseUpdate()` judges only the **email body**, and document-delivery emails ("Greetings, please find attached, kind regards") carry no textual status update → AI returns `hasUpdate: false` → the old code gated *all* attachment saving on `isRelevant && hasUpdate`, so the documents were never downloaded. Fix in `apps/cases/app/api/cases/[id]/check-updates/route.ts`: **decoupled document harvesting from AI update-detection.** Since `scanMailboxForCaseUpdates()` only returns emails that already matched the client by ID number / full name, every returned email's attachments now get saved to the case vault (new shared `saveEmailAttachments()` helper, dedup by fileName+size) regardless of the AI verdict — robust even when the AI provider is down. The AI is now used *only* for the update summary + consumer notification (notifications still fire only on a genuine `hasUpdate`, never for silent document deliveries). Emails with no textual update but new documents are logged under a new `CASE_DOCUMENTS_HARVESTED` activity type (fed into the already-seen skip set alongside `CASE_EMAIL_UPDATE_PROCESSED`). The scan-run log now records **per-email diagnostics**: client-matched candidate count, and for each email whether it produced an update, a documents-only harvest, or no action — plus an explicit `⚠️ AI analysis failed on N email(s)` line when `analyzeEmailForCaseUpdate` falls back, so a "nothing harvested" result is always explainable. The results modal + toast distinguish "✨ Update" vs "📎 Documents" entries and show candidate counts; a dedicated amber banner appears on AI-analysis failure. Tests: `check-updates/route.test.ts` grown to **12** (harvest-without-update, irrelevant-no-attachment no-op, AI-failure-still-harvests, plus the earlier scan-run/GET cases) — 12/12 green; `tsc --noEmit` clean on all touched files (`cases`, `shared-lib`). ⚠️ **Known trade-off:** emails matched only by *full name* (not ID number) could, in a name-collision, harvest another person's attachment onto the case — staff review the results modal and can remove wrong docs; an ID-number-only harvest mode is available if stricter matching is preferred. Unrelated pre-existing typecheck error remains in `apps/cases/app/api/referrer-portal/transfer-credit/route.ts` (`creditTransfer` Prisma client/schema drift — not touched here). Still uncommitted/unpushed: `check-updates/route.ts`, its test, and `CheckForUpdatesButton.tsx` (the `imap.ts` `onFoldersResolved` callback is already committed to HEAD).

> Last updated: 2026-07-24 (**"Check for Updates" now audit-logged + pre-run confirmation**): The case "Check for Updates" email scan previously only wrote a timeline entry when the AI actually extracted an update — a scan that found nothing (the common case) left no trace of who ran it, when, or what was searched. Two changes: (1) **Every scan is now logged** — the `POST /api/cases/[id]/check-updates` handler (`apps/cases/app/api/cases/[id]/check-updates/route.ts`) always writes a `CASE_UPDATE_SCAN_RUN` `CaseComment` + `WorkflowLog` recording the staff member who ran it (`session.user.name`), the timestamp, the mailboxes scanned (addresses), which folders were searched categorised as **Inbox ✓ / Sent Items ✓** (plus the raw folder list), the lookback range, the update count, and any per-mailbox errors. To capture folders accurately, `scanMailboxForCaseUpdates()` in `packages/shared-lib/src/integrations/imap.ts` gained a non-breaking optional `onFoldersResolved(folders)` callback; the route also now records a per-mailbox `error` string when a mailbox scan throws (previously swallowed to the logger only). (2) **Pre-run confirmation** — a new `GET /api/cases/[id]/check-updates` returns the most recent scan's metadata (who/when/mailboxes/folders/updates). The `CheckForUpdatesButton` (`apps/cases/components/CheckForUpdatesButton.tsx`) now, on click, first fetches this and — if a prior scan exists — pops a modal reading "Last checked by {name} on {date}" with Inbox/Sent/updates-found chips and three actions: **Run Check Again**, **View Recent Results** (re-opens the stored results without re-scanning), or **Cancel**. First-ever scans (no prior run) still run directly. The results modal also gained a run-by/when + Inbox/Sent banner. Tests: `check-updates/route.test.ts` extended to 10 cases (scan-run always logged even with zero updates; per-mailbox error captured; GET returns last scan / null / unauthorised) — 10/10 green; `tsc --noEmit` clean on `cases` and `shared-lib`. Browser verification deliberately skipped — exercising the button runs a live IMAP scan against production mailboxes and can send real consumer notifications. NOTE: this ships on top of the still-uncommitted working tree (reporting app + `WorkLog` migration) noted below.

> Last updated: 2026-07-24 (**"Search Email for ID/POA" now audit-logged + pre-run confirmation modal**): The Documents-tab "Search Email for ID/POA" (and "Search Email for Credit Report") button previously ran the mailbox scan immediately and, while it always wrote a `DHS_FEE_EMAIL_SCAN_REQUESTED` `CaseComment` + `WorkflowLog`, the activity did not record **who** ran it (by name) or **which folders (Inbox / Sent)** were searched. Two changes, mirroring the earlier "Check for Updates" work: (1) **Richer audit log** — `POST /api/cases/[id]/dhs-decline/check-fee-emails` (`apps/cases/app/api/cases/[id]/dhs-decline/check-fee-emails/route.ts`) now fetches the running user and stores `runBy {id,name,email}`, plus `foldersScanned[]`, `scannedInbox`, `scannedSent` in `activityData`, and adds a "run by {name}" opener and a "Folders scanned: … (Inbox: yes/no, Sent: yes/no)" line to the human-readable comment `content`. To capture folders, `scanMailboxForClient()` in `packages/shared-lib/src/integrations/imap.ts` gained a non-breaking optional `onFoldersResolved(folders)` callback and now returns `foldersScanned` in its `ScanResult`; a new pure exported helper `classifyScannedFolders()` buckets raw folder paths into Inbox/Sent. (2) **Pre-run confirmation modal** — a new `GET /api/cases/[id]/dhs-decline/check-fee-emails?docGroup=…` returns the most recent scan's metadata (who/when/mailboxes/folders/summary). `DocumentsTab.tsx` (`packages/ui/src/DocumentsTab.tsx`) now, on button click, first fetches this and pops a modal reading "Last run by {name}" with the timestamp, mailbox list, Inbox/Sent chips, and three actions: **Run Again**, **View Most Recent Results** (expands the stored counts + comment without re-scanning), or **Cancel**. First-ever scans (no prior run) show a "No previous scan" state with **Run Scan** / **Cancel**. Tests: `check-fee-emails/route.test.ts` extended (runBy + folders recorded on POST; GET returns last scan / found:false / 401 / 403) and `imap.test.ts` gained a `classifyScannedFolders` suite — full `cases` suite 734/734 green, `shared-lib` `imap.test.ts` 12/12 green; `tsc --noEmit` clean on `cases`, `ui`, and `shared-lib`. Browser verification deliberately skipped — exercising the button runs a live IMAP scan against production mailboxes. NOTE: ships on top of the still-uncommitted working tree (reporting app + `WorkLog` migration) noted below.

> Push verified: 2026-07-23 (Committed and pushed the pending working tree to origin/main as `067dc7b` — 7 files: add AI-powered check for case updates from email.)

> Push verified: 2026-07-23 (Committed and pushed the pending working tree to origin/main as `e115cd2` — 7 files: redesign case details tabs & action buttons for responsiveness, and include docGroup in duplicate check prevention.)

> Push verified: 2026-07-23 (Committed and pushed the pending working tree to origin/main as `0b29c57` — 5 files: email harvesting for missing ID, POA, and credit reports.)

> Last updated: 2026-07-31 (**XDS Credit Bureau Live Login & Portal URL Verification**): Fixed stale portal URL in database `SystemSettings` (`https://portal.xds.co.za` → `https://www.online.xds.co.za`) and updated `loginToXds` in `packages/shared-lib/src/xds/browser.ts` with DOM-evaluated form submission to bypass Puppeteer overlay click restrictions. Ran live Puppeteer authentication against `https://www.online.xds.co.za/XDSPortal/Account/Login` — **login succeeded**, landing on `https://www.online.xds.co.za/XDSPortal/` as user **Kenneth Mokgoshi** (**Zenowethu Trading**, Current Enquiry Limit: R 275.05). Verified unit test suite (`xds.test.ts`) passes 6/6 green. AI maintenance reminder: create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-23 (**Local Dev Port Conflict Resolution**): Resolved a port conflict between `apps/crediva` (port 3005) and `apps/reporting` (previously port 3005) that was causing `EADDRINUSE` errors and preventing the local development server from starting. Modified `apps/reporting/package.json` to use port `3008`, updated `CLAUDE.md` to document the new port configuration, and verified that all 8 apps (`cases`, `insurance`, `legal`, `forensic-audit`, `finance`, `crediva`, `website`, `reporting`) start successfully under the unified `pnpm dev` command. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-23 (**Staff Work Reporting App**): Created a Next.js 16 (App Router) + React 19 timesheet logging and verification application under `apps/reporting` running on port `3008`. Added the `WorkLog` model to `schema.prisma`, generated migrations, and deployed them to the target database. Implemented debounced live case validation and built automatic "System Activity Signatures" to verify reported timesheets against monorepo database transaction logs (comments, status transitions, notifications, document uploads) for direct corroboration. Completed Vitest unit tests in `@zenowethu/shared-lib` and `apps/reporting` (all tests passing) and confirmed compilation via a successful production build.

> Last updated: 2026-07-23 (**AI-Powered Case Updates Checker from Email**): Added the "Check for Updates" feature to allow staff to sweep connected mailboxes for relevant email correspondence, extract case updates via AI, save attached documents/images, and notify the consumer. (1) **IMAP Extension**: Added `getEmailBodyText()` and `scanMailboxForCaseUpdates()` in `packages/shared-lib/src/integrations/imap.ts` to locate and extract email body text and capture attachments. Added 3 unit tests in `imap.test.ts` (11/11 tests pass successfully). (2) **AI update analysis**: Created `analyzeEmailForCaseUpdate()` in `packages/shared-lib/src/ai/case-updates.ts` using configured AI client models to evaluate email relevance, extract summaries, and draft consumer updates. (3) **Backend API route**: Created `/api/cases/[id]/check-updates` route in `apps/cases/app/api/cases/[id]/check-updates/route.ts` with 5 Vitest unit tests in `route.test.ts` (all green). It scans connected mailboxes, uses AI to extract updates, automatically saves attachments as `Document` records in database, sends Email/SMS notifications via `sendManualMessage()`, and logs activity CaseComments and WorkflowLogs. (4) **Frontend Button & Integration**: Created `<CheckForUpdatesButton />` component in `apps/cases/components/CheckForUpdatesButton.tsx` rendering the dropdown range selectors, check progress spinner, and modal detail breakdown. Integrated it into case details header actions list in `apps/cases/app/(authenticated)/cases/[id]/page.tsx`. Verification: TypeScript compilations build with 0 warnings/errors, and all unit tests pass green. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-31 (**XDS Credit Bureau Scraper Architecture & Unit Testing**): Reviewed XDS credit bureau scraper module in `packages/shared-lib/src/xds/`, verified Puppeteer portal authentication, search history scraper (`/XDSPortal/History/HistoryMatch`), incremental date sync engine (`runXdsSync`), settings CRUD, and API routes (`/api/admin/xds/sync`, `/api/admin/settings/xds`). Created `packages/shared-lib/src/xds/xds.test.ts` covering XDS helper utilities (6/6 tests green). Verified all 451 unit and route tests in `apps/cases` pass cleanly. AI maintenance reminder: create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-23 (**Case Detail Tabs & Documents Action Buttons Redesign**): Upgraded case details utility tabs and documents action buttons to be fully responsive and premium-looking across all 4 monorepo apps (Cases, Forensic Audit, Insurance, Legal). (1) **Tabs Redesign**: Updated `<nav>` containers to support horizontal overflow scrolling with hidden scrollbars on mobile (`flex-nowrap overflow-x-auto scrollbar-none`) and elegant wrapping on desktop/tablet viewports (`md:flex-wrap md:overflow-x-visible`), preventing squished labels. Redesigned active buttons to render as capsule-like pills with soft shadows and glowing borders (`bg-zeno-cyan/10 border border-zeno-cyan/20` for regular, `bg-amber-400/10 border border-amber-500/20` for AI Assistant). Added `whitespace-nowrap` to prevent labels from wrapping within buttons. (2) **Documents Action Buttons**: Redesigned header inside `DocumentsTab.tsx` to stack header and buttons vertically on mobile and side-by-side on larger screens, allowing buttons to wrap cleanly when space is constrained. Added `whitespace-nowrap` to prevent text wrapping on all action buttons. Verification: TypeScript compilation checking compiles successfully with zero errors across the monorepo.

> Last updated: 2026-07-23 (**Email Document Harvesting for Missing ID, POA & Credit Reports**): Added targeted email document harvesting buttons to scan connected mailboxes and automatically upload found files when ID, POA, or Credit Report are missing. (1) **API Update**: Updated `GET /api/ai/plan/case/[caseId]` to query and return `uploadedDocTypes` array containing all present document types on the case. Updated the check-fee-emails POST API route (`apps/cases/app/api/cases/[id]/dhs-decline/check-fee-emails/route.ts`) to store `docGroup` in `activityData` and verify the `docGroup` matches in duplicate request prevention check (preventing searches for different document groups from blocking each other). Updated the duplicate return response to use dynamic messaging indicating which specific document group check was already run recently. (2) **AI Plan Tab & Confidence Report**: Added new props `uploadedDocTypes` and `onRefresh` to the `<ConfidenceReport />` component in `packages/ui/src/plan/ConfidenceReport.tsx`. Integrated inline progress bar and status message display when a mailbox search is running. Extracted missing check logic to an exported helper `getMissingDocsStatus()` with a comprehensive unit test suite in `ConfidenceReport.test.tsx` (5 tests passing). Handled duplicate checks in the frontend component to display the duplicate notification message instead of a generic search failure. (3) **Documents Tab**: Added the same two email search buttons ("Search Email for ID/POA" and "Search Email for Credit Report") under the action buttons header in `DocumentsTab.tsx` when they are detected as missing from the case documents. Integrated the search progress status in the extraction progress bar. Handled duplicate check messages properly. Verification: 54/54 `ui` Vitest unit tests pass green, TypeScript typecheck (`tsc --noEmit` on cases app) compiles cleanly. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Push verified: 2026-07-22 (Committed and pushed the pending working tree to origin/main as `c106772` — 1 file: fix fontally typo in EditQuoteModal finally block to resolve build failures.)

> Push verified: 2026-07-22 (Committed and pushed the pending working tree to origin/main as `8a2448a` — 21 files: targeted document harvesting, auto PDF password unlocking, and referrer clients dashboard metrics formulas and interactive card filtering.)

> Last updated: 2026-07-22 (**Targeted Email Document Harvesting & Password-Protected PDF Support**): Implemented 6 targeted email document harvesting buttons in the Case details UI, enabling staff to extract specific document groups from connected mailboxes. Also added support for automatically unlocking password-protected credit reports using the client's 13-digit ID number. (1) **UI Dropdown Redesign**: Updated `<CheckInvoiceEmailsButton />` in `apps/cases/components/CheckInvoiceEmailsButton.tsx` to feature a unified dropdown with lookback days, mailbox selectors, and 6 targeted buttons (ID & POA, Credit Reports, DC Invoices, Proofs of Payment, Paid-Up Letters, Debt Review Forms). (2) **IMAP Group Filter**: Updated `scanMailboxForClient` and `classifyDocumentByFilename` in `packages/shared-lib/src/integrations/imap.ts` to support `docGroup` filtering and added new Sniffing rules for credit reports (TransUnion, Experian, XDS, Lightstone), Form 16, and Form 17.2. (3) **PDF Password Unlocking**: Updated `pdf-image.ts`, `pdf-process.ts`, and `extraction.ts` in `packages/shared-lib` to support decryption passwords, automatically passing client ID numbers as the document password in cases upload, extract, and reanalyze routes. (4) **Tests & Verification**: Added unit tests in `imap.test.ts` and fixed NDJSON stream test assertions in `route.test.ts`. Verified that all 823 `shared-lib` and 718 `cases` unit tests pass cleanly. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-22 (**Document Harvesting & Keyword Classification from Emails**): Implemented email harvesting and keyword-based document type classification for South African debt review documents across all connected mailboxes. (1) **Classification Logic**: Created and exported a pure helper function `classifyDocumentByFilename` in `packages/shared-lib/src/integrations/imap.ts` to categorize attachments based on SA debt review keywords (Form 17.1, Form 17.7, Form 17.W, Court Orders, IDs, POAs, payslips, bank statements, proof of residence, paid-up letters, fee invoices, and proofs of payment). (2) **Ingestion API**: Updated the case check-fee-emails API route (`apps/cases/app/api/cases/[id]/dhs-decline/check-fee-emails/route.ts`) to dynamically assign the detected `detectedType` to the created document instead of forcing the generic `FEE_INVOICE`. (3) **UI Integration**: Registered `FORM_17_7` in `packages/ui/src/DocumentsTab.tsx` component metadata and dropdown list to support badges and manual uploads. (4) **Tests & Verification**: Wrote comprehensive unit tests verifying keyword matching and email route document creation in `imap.test.ts` and `route.test.ts`. Verified that all 822 `shared-lib` and 718 `cases` Vitest tests pass cleanly. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.

> Last updated: 2026-07-21 (**Interactive Card Filtering & Expected Revenue Formula Fix on Admin Referrer Clients Dashboard**): Enabled all metrics cards (pipeline, health metrics, client finance metrics, and quotes scoreboard) on the admin/staff Referrer Clients dashboard to be fully clickable, allowing staff to dynamically filter the client list table below just like on the client-facing portal. Ported the Quotes scoreboard widget to the Admin Referrer Clients view. Updated the **Expected Revenue** formula to calculate the actual remaining money to be collected (sum of outstanding balances on all active/completed files) instead of double-counting via Accepted Quotes + Completed Outstanding. Updated `calculateDiscountPartnerTotals` helper, admin clients API route (`GET /api/admin/referrers/[id]/clients`), and portal summary API route to reflect the new formula, with updated test suite assertions. All 717 Vitest unit tests pass green. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.
 
> Last updated: 2026-07-21 (**Referrer Dashboard Cases Pipeline & Client Finances Enrichment**): Added "In Progress" case status card (active pipeline count) and enriched client finance statistics with "Out Standing Balance" (outstanding fees on completed files only), "Expected Revenue" (accepted quotes + completed outstanding balance), and "Settled" (total amount paid on settled cases) with full monthly breakdowns. Applied this change consistently across both the client-facing Referrer Portal and the staff/admin-facing Referrer Clients dashboard. Updated the cases summary API endpoint and admin clients API route to compute these new pipeline and financial metrics, and modified the frontend React dashboards to display them in responsive grid layouts. Added/updated unit test coverage for the helpers, summary API endpoint, and admin clients API endpoint; all 717 vitest unit tests pass green. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Push verified: 2026-07-21 (Committed and pushed the pending working tree to origin/main as `f20b67c` — 7 files: quote line item editing and bank details switching for Admins & Executives across finance and cases apps.)

> Last updated: 2026-07-21 (**Quote Line Item Editing & Bank Details Switching for Admins & Executives**): Enabled Admin and Executive users to add, edit, or remove line items directly on issued quotes without generating a new quote number, with automatic recalculation of subtotal, VAT, and total. Added bank details switching functionality (e.g. FNB to Capitec or Standard Bank) via a bank account dropdown selector on quotes across both `apps/finance` and `apps/cases`. Created `<EditQuoteModal />` component, updated `PATCH /api/finance/invoices/[id]` API handlers in both apps with role checks (`isAdmin || isExecutive`), and added Vitest unit tests verifying non-draft quote editing permissions. All 142 vitest unit tests in `finance` app pass green. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Push verified: 2026-07-21 (Committed and pushed the pending working tree to origin/main as `f94d3fc` — 17 files: standardize platform email footers/signatures and fix mailbox password placeholder validation.)

> Last updated: 2026-07-21 (**Mailbox Password Decryption & Password Status Resolution Fix**): updated `isUsableMailboxPassword()` and `toPublicMailbox()` in `apps/cases/lib/mailboxes.ts` and `getSearchableMailboxes()` in `apps/cases/app/api/cases/[id]/dhs-decline/check-fee-emails/route.ts` to validate saved mailbox passwords against dummy placeholder values (`no-password-saved`, `dummy`, `placeholder`, `none`). Mailboxes with un-configured or placeholder passwords no longer falsely report "Password saved" or `5/5 readable mailboxes`. When scanning invoice emails, invalid/missing passwords now cleanly log which specific mailboxes require password updates in **Admin → Settings → Mailboxes**. Verification: 717/717 `cases` app unit tests and 820/820 `shared-lib` unit tests pass green; `tsc --noEmit` clean. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-21 (**Email footers & signatures updated across platform**): per user directive, removed "Aaron" from email footers and signatures across the platform and standardised the signature block format (`Zenowethu Debt Management`, `NCRDC3693`, `Suite 2, 2nd Floor, Central House, 17 Central Road, Mabopane, 0190`, `Tel: +27 81 747 7616 | Cell: 082 363 8207`, `notifications@zenowethu.co.za | www.zenowethu.co.za`, `Member of DCASA`). Updated template files in `packages/shared-lib/src/dhs/` (`decline-handler.ts`, `accepted-email.ts`, `drr-readiness.ts`), `notifications/otp-service.ts`, `apps/cases/app/api/cron/workflow-automation/route.ts`, `apps/cases/app/api/b2b-dashboard/completed-this-week/email/route.ts`, `dc-fee-invoices` send routes, consent pages, `ai-chat` prompt, PDF generators (`invoice-pdf.ts`), `CLAUDE.md`, and `docs/MASTER_PROMPT.md`. Verification: 820/820 `shared-lib` tests pass green. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.
...
> Last updated: 2026-07-20 (**Clear DHS Requested notification template copy**): updated `REQUESTED_VIA_DHS` notification template in `packages/shared-lib/src/notifications/templates.ts` to provide clear, client-friendly messaging without confusing jargon, explicitly stating that the file was requested from the debt counsellor (`{dcName}`) at their company/trading name (`{tradingName}`) via the Debt Help System (DHS), and specifying the 5-10 working days turnaround time.

> Push verified: 2026-07-20 (Committed and pushed the pending working tree to origin/main as `cfb89f4` — 8 files: Proxy Consent for DRR flag removal, auto-locked session role resolution, and case details load fix.)

> Push verified: 2026-07-19 (Committed and pushed the pending working tree to origin/main as `d2a0084` — 12 files: deduplicate IMAP fee decline ingestion, automate DC invoice requests, and list 50 recent cases.)

> Last updated: 2026-07-20 (**Staff, B2B, & Referrer Proxy Consent for Debt Review Flag Removal + Auto-Locked Session Role & Case Details Load Fix**): (1) Implemented full support for Staff, B2B partners, and Referrers to grant consent for Debt Review Flag Removal on behalf of clients ("Proxy Consent"). Added `consentType` (`OWNER` vs `PROXY`), `consentedByRole` (`CONSUMER` | `STAFF` | `B2B` | `REFERRER`), `consentedById`, `consentedByName`, `consentedByEmail`, `notes`, `ipAddress`, and `userAgent` fields to the `DebtReviewRemovalConsent` model in `schema.prisma` with a new index. Updated `recordDrrConsent()` in `packages/shared-lib/src/dhs/consent-service.ts` to stamp `consentType = 'OWNER'` and client identity metadata on direct consumer approval. Implemented `recordDrrProxyConsent()` to capture proxy approvals with audit notes, actor details, IP address, and user-agent string. Updated `onDebtReviewRemovalConsent()` to post clear timeline comments. Created `POST /api/cases/[id]/drr-proxy-consent` API route. Enhanced Case Details page UI with **"Consent on Behalf"** button, **WHO CONSENTED** status card, and **Proxy Consent Modal**. (2) **Auto-Locked Session Role** — per user request, removed the role selection dropdown from the Proxy Consent Modal so users cannot manually pick B2B or Referrer when logged in as Staff. The consenting role is now automatically derived from the authenticated session (`session.user.userType`: Staff -> STAFF, B2B -> B2B, Referrer -> REFERRER) and displayed as an immutable read-only badge. The API endpoint also enforces session-based role resolution to prevent role spoofing. (3) **Case details loading fix** — fixed issue where clicking case numbers reloaded `/cases`. Updated `GET /api/cases/[id]` with multi-stage relation fallbacks and replaced silent `router.push('/cases')` on error with an explicit error screen and retry button. Verification: 4/4 `drr-proxy-consent/route.test.ts` API tests, 717/717 `cases` app tests, and 820/820 `shared-lib` tests green; `tsc --noEmit` clean across workspace. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-19 (**IMAP Ingestion Deduplication & System Improvements**): implemented the mailbox connector deduplication ledger for DHS Fee Declines. Added `ProcessedMailboxMessage` model to the Prisma schema, regenerated the Prisma client, and updated the IMAP mailbox scanner (`scanMailboxForClient`) to support message ID-based filtering. Integrated this filter into the `check-fee-emails` API route, adding checks for duplicate attachments using SHA-256 hashes to prevent duplicate file uploads and database entries. Verified that all unit tests and integration tests in `shared-lib` and `cases` apps compile and pass cleanly (817/817 shared-lib and 713/713 cases tests green). AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-19 (**Automated Debt Counsellor invoice request on outstanding fees decline**): wired up automated email requests to the Debt Counsellor (DC) when a DHS transfer is declined due to outstanding fees, matching the client-facing copy which claims we are requesting an invoice from them. Created a new shared template `buildRequestInvoiceEmail` in `packages/shared-lib/src/dhs/decline-handler.ts` and updated the `OUTSTANDING_FEES` category handler inside `handleDHSDecline` to send the invoice request to the DC (and CC the consumer) when a DC email is available, falling back with a logged warning when the email is missing. Also updated `decline-preview.ts` to output previews for both emails, added 5 unit/persistence/preview tests, and verified all 817 tests compile and pass cleanly with clean `tsc --noEmit` checks across both `shared-lib` and `cases`. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Push verified: 2026-07-18 (Committed and pushed the pending working tree to origin/main as `6c9653de` — 3 files: stop printing the DHS decline reason twice in consumer decline emails. Follows `fd1e24a3` which shipped the smart DHS decline intent classification + boilerplate fee fix.)

> Push verified: 2026-07-17 (Committed and pushed the pending working tree to origin/main as `072ab012` — 16 files: sort cases by recordedAt, flag flagged debt counsellors, and parse dynamic DHS decline retry days.)

> Last updated: 2026-07-19 (**Recent Cases dashboard table sorted by updatedAt & count increased to 50**): updated `DashboardCasesTable` in `apps/cases/components/DashboardCasesTable.tsx` to fetch `/api/cases?orderBy=updatedAt&take=50` and store the 50 most recently updated cases directly, instead of slicing 10 client-side. Added support for `orderBy` (mapping to `updatedAt`, `createdAt`, or `recordedAt`) in `GET /api/cases` route handler (`apps/cases/app/api/cases/route.ts`). Added comprehensive unit tests in `apps/cases/app/api/cases/route.test.ts` (3 tests) to verify sorting parameters and take limits. All 713 tests pass cleanly and typescript compilation check (`tsc --noEmit`) is successful with 0 errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-18 (**DHS consumer decline emails: reason no longer printed twice**): fixed a template redundancy spotted on NOKUPHIWA KHABO's resubmit email (ZDM-2026-1044-6E4) where the DC's decline reason appeared twice — once as a standalone "Their DHS response reads:" line and again inside the shared status block under "Reason given by the current Debt Counsellor:". Removed the redundant standalone line from the four affected consumer templates in `packages/shared-lib/src/dhs/decline-handler.ts` (`buildResubmitClientEmail`, `buildConsumerConsentEmail`, `buildOutstandingFeesEmail`, `buildAttorneyClientEmail`), so the reason now shows exactly once (in the status block, which also carries the transfer/decline dates and what we did). This makes them consistent with `buildSendDocsClientEmail`, which already showed it once. Preview path (`decline-preview.ts`) reuses these builders so it inherits the fix automatically. Verification: updated the fees-email copy test + added a resubmit-email regression test asserting the reason appears once and there is no "Their DHS response reads:" line; **205/205 shared-lib dhs tests green, `tsc --noEmit` clean**. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-18 (**DHS decline classification now interprets intent like a smart employee, not keyword-matching**): fixed the bug where a "Transfer Under Review" DHS response (KOOS DLOVA, ZDM-2025-1042-BKL) was wrongly classified as `OUTSTANDING_FEES` — and the consumer wrongly told they owed money — purely because the DC's *standard boilerplate* contained the conditional phrase "…a transfer request may be declined **if** there are any outstanding fees payable…". Root cause: `classifyDeclineReason()` substring-matched "outstanding fees" out of hypothetical/policy text. Two-layer fix in `packages/shared-lib/src/dhs/`: **(1) Smarter deterministic rules** (`decline-handler.ts`) — added an "under review / standard turnaround / received & acknowledged / allow N business days" check that resolves to `RESUBMIT_LATER` **before** the fees check, plus a new `feeMentionIsAssertive()` guard that only fires `OUTSTANDING_FEES` when a fee keyword appears in a NON-conditional clause (ignores "may/if/should/in the event…"). `getBasePeriodForCategory()` now also reads a stated turnaround **range** ("3–7 business days" → waits the upper bound, 7). **(2) New AI "smart employee" layer** (`decline-classifier.ts`, `classifyDeclineReasonSmart()`) — reads the DC's message and returns `{category, confidence, reasoning, source}`, interpreting operative intent and explicitly ignoring generic disclaimers; uses the `dhs_parsing` provider chain, validates output with Zod, and falls back to the deterministic rules on low confidence / provider error / no key configured (so it degrades gracefully and never blocks the decline response — and makes no network call when no AI provider is configured). Wired into `handleDHSDecline` (now records a `[SYSTEM]` comment explaining *why* it chose the category, and exposes `classificationSource/Confidence/Reasoning` on the result), `previewDHSDecline`, and the staff `POST /api/cases/[id]/dhs-decline/handle` route/preview. Verification: added `decline-classifier.test.ts` (6) + new rule/boilerplate tests in `decline-handler.test.ts`; **70/70 shared-lib dhs tests + 7/7 handle-route tests green; `@zenowethu/shared-lib` `tsc --noEmit` clean**. **Redeploy cases and shared-lib to ship. To enable the AI layer set `OPENAI_API_KEY` (or `OPENROUTER_API_KEY`) or configure a provider in Admin → AI Providers; until then the improved rules run.** NOTE (separate, still open): the `OUTSTANDING_FEES` branch still never actually emails the DC to request the invoice even though the consumer email says "we are currently requesting an invoice from your Debt Counsellor" — that DC send was never wired (only inbox-scan + verify-request tooling exists). Not changed in this pass; flag for follow-up. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-18 (**B2B "Recent Referrals" widget now sorts/labels by true capture time**): follow-up to the `recordedAt` work — the B2B dashboard's *Recent Referrals* widget was the one remaining surface still ordering **and** displaying the back-dated `createdAt`, so a file loaded today into an older month folder (e.g. KOOS DLOVA `ZDM-2025-1042-BKL`, createdAt back-dated to Aug 2025 but `recordedAt` = today 2026-07-18) never appeared as recent. Fix: `GET /api/b2b-dashboard/stats` recent-cases query now `orderBy: { recordedAt: 'desc' }`, selects `recordedAt`, and emits it as the `createdAt` shown to the partner (the true load time). Verified against the LIVE DB that KOOS's `recordedAt` is 2026-07-18, so it now surfaces at the top. The All-Referrals list and referrer-clients API were already switched to `recordedAt` in the prior session. Note: files captured *before* the 2026-07-17 migration had `recordedAt` backfilled from their back-dated `createdAt`, so those legacy rows still order by month — only files created after the migration have a true `recordedAt`. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-17 (**Latest cases/referrals sort by true capture time, not the month folder**): fixed the complaint that files loaded today into an *older* month folder were not surfacing as the latest referrals. Root cause: the 2026-07-16 change deliberately back-dates `Case.createdAt` to the case's MONTH/YEAR project folder (so month filters and the timeline sidebar bucket it correctly), but every list view *ordered* by that same back-dated `createdAt` — so a file captured today into e.g. a March folder sank below genuinely older July files. Fix keeps the month bucketing (createdAt unchanged) and adds a new **`recordedAt`** column that captures the true insertion time and is what the lists now sort by. Changes: (1) `schema.prisma` — added `Case.recordedAt DateTime @default(now())` + migration `20260717090000_add_case_recorded_at` (adds column, backfills existing rows from `createdAt`). (2) `POST /api/cases` — sets `recordedAt: new Date()` (real now) while keeping `createdAt: targetDate` (month folder). (3) `GET /api/cases` — both slim and full queries now `orderBy: { recordedAt: 'desc' }` and select `recordedAt`. (4) B2B referrals list (`b2b-dashboard/cases/page.tsx`) — "Created" column now shows `recordedAt` (falls back to `createdAt`). (5) Referrer clients API — orders by `recordedAt` and maps `referredAt` from it. DHS bulk import already used `now()` so it gets correct `recordedAt` for free. **Manual steps to ship (LIVE prod DB):** stop the running `cases dev` server (it locks the Prisma query-engine DLL), then `cd packages/database && npx prisma generate && npx prisma migrate deploy`, then redeploy cases. Verification: browser check pending client regeneration/migration (dev server holds the engine DLL). AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-17 (**Flag Flagged Debt Counsellors before DHS Transfer**): Added flagging and warning alerts for Debt Busters, Octogen, and NDC debt counsellors. (1) Created a pure, browser-safe counsellor-flag utility in `packages/shared-lib/src/dc/counsellor-flag.ts` (with `checkFlaggedDebtCounsellor` and `checkCaseFlaggedDC`) and a database-dependent utility `packages/shared-lib/src/dc/counsellor-flag-db.ts` (`flagCaseIfFlaggedDC`), exported in the main `dc` index. (2) Integrated flagging logic in cases creation (`POST /api/cases`), edits (`PATCH /api/cases/[id]`), and bulk imports (`POST /api/admin/dhs-import/apply`) to automatically create system timeline comments for any new or updated cases. (3) Added a styled flagged counsellor warning panel inside the DHS Information card on the case detail page. (4) Added confirmation prompts in the case details page for both `handleDHSTransfer` and `handleRequestTransfer` requesting staff verification when a flagged DC is detected. Verification: Added 10 comprehensive unit tests in `counsellor-flag.test.ts` matching all edge cases (all passed successfully); `tsc --noEmit` compiles cleanly with zero errors. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-17 (**DHS decline: Awaiting management response & dynamic retry window**): mapped DHS decline reasons containing "awaiting management" or "request after" to the `RESUBMIT_LATER` category. Enhanced `getBasePeriodForCategory()` to dynamically parse specified retry day counts (e.g. "request after 3 days" -> 3 days, "resubmit in 7 days" -> 7 days, "try again in 5 days" -> 5 days) rather than falling back to the default 7 working days. Updated the preview path in `decline-preview.ts` to mirror this calculation. Verification: Added comprehensive unit tests in `decline-handler.test.ts` for new classification keywords and dynamic regex parsing, all 792 vitest tests passed green; monorepo typechecking compiles successfully with zero errors. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**B2B Referrer Auto-Detection & Case Backfill**): fixed a bug where B2B referrals (e.g. Letsatsi Mitchells Plain) created under month/year subprojects were not automatically linked to their referrer project (resulting in "No referrer assigned" and 0 counts on the branch referrer dashboard). Modified `apps/cases/app/api/cases/route.ts` (POST) to recursively traverse up the project hierarchy to locate the nearest ancestor project linked to a `Referrer` record. Created and executed a database backfill script `backfill_referrers.ts` which successfully linked 176 legacy B2B cases (including `NONKOSI MATSHAYANA` / ZDM-2026-1037-N0X) to their correct referrers. Also updated the creation date of case `ZDM-2026-1037-N0X` to March 16, 2026 (matching its B2B monthly project folder rather than the capture date in July) so it appears under the correct month on referrer trend charts and scoreboard statistics. Further enhanced the POST handler in `apps/cases/app/api/cases/route.ts` to automatically extract the MONTH and YEAR folder names from the project path of all newly created cases, assigning a `createdAt` date that aligns with the chosen month/year (clamping the day of the month as needed to avoid overflows) so future cases are automatically bucketed in the correct calendar month immediately upon creation. Verification: `tsc --noEmit` on cases app compiled successfully with 0 errors; database verified that the target case is linked to Mitchells Plain and only 17 B2B cases (without referrer-linked project paths) remain unlinked. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Push verified: 2026-07-16 (Committed and pushed the pending working tree to origin/main as `a7e2b229` — 13 files: auto-detect B2B referrers, standardize Crediva default password and auto-provisioning, and enforce ID-only OTP login.)

> Last updated: 2026-07-16 (**Crediva: standard default password `Consumer@1` + auto-provision a portal login on every case/referral**): every consumer now gets a Crediva login where the **username is their 13-digit ID number** and the **initial password is the shared standard `Consumer@1`**, which they may change later from Settings but are **not forced to**. (1) **Default password** — changed `DEFAULT_CONSUMER_PASSWORD` from `Client@100New` to `Consumer@1` in `packages/shared-lib/src/crediva/password-policy.ts`; `validatePasswordStrength` still forbids a consumer from *choosing* the default. (2) **Optional change** — `provisionConsumerForClient` now creates accounts with `mustChangePassword: false` (was `true`), so first login lands straight on the dashboard instead of the forced change-password screen. ⚠️ **Security tradeoff**: a publicly-known default + optional rotation means anyone who knows a consumer's ID number could sign in until they set their own password — accepted per product decision; flip `mustChangePassword` back to `true` in `consumer-provisioning.ts` to force a change instead. (3) **Coverage** — provisioning was already wired into staff case creation (`api/cases`), the v1 API (`api/v1/cases`), DHS-accepted, and document-requests; added the **three missing creation paths**: website lead conversion (`api/leads/[id]/convert`), DHS bulk import (`api/admin/dhs-import/apply`), and joint-client-on-edit (`api/cases/[id]`). All calls are idempotent, non-blocking, and never throw (cannot break creation). (4) **Existing users** — `scripts/backfill-crediva-default-passwords.ts` updated to issue `Consumer@1` with `mustChangePassword:false`; run it (with `--dry-run` first, then `--force-unactivated` to migrate older auto-provisioned accounts off the previous default) to standardise existing profiles — accounts that already set their own password are never touched. Verification: `password-policy.test.ts` + `consumer-provisioning.test.ts` updated, **24/24 green**; `@zenowethu/shared-lib` and `cases` both `tsc --noEmit` clean. Browser verification skipped (would require live prod DB + email). **Redeploy cases and shared-lib to ship; run the backfill script for existing consumers.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Crediva: enforce ID-number-only login across all auth flows**): email and cell number are not unique (two consumers may legitimately share them), so they can never identify a single account. The password login (`auth.ts`), registration, and forgot-password flows already enforced ID-only; the **OTP login flow was the remaining gap** — both `POST /api/consumer/generate-otp` and `POST /api/consumer/verify-otp` still did a non-unique `findFirst({ where: { email } })` that could resolve to the wrong account. Fixed both routes to validate the identifier as `/^\d{13}$/` (Zod) and look the consumer up by `idNumber` only via `findUnique` (the emailed code is still delivered to the email on file, but the ID identifies the account). Updated the OTP login UI (`app/(auth)/login/otp/otp-client.tsx`) to a single "ID number" field — numeric-only input, 13-digit cap, 13-digit submit guard, and "Enter your 13-digit ID number" copy — and corrected the not-found message in generate-otp. Verification: `pnpm --filter crediva typecheck` clean; OTP DB tests not run (they write to the LIVE production DB). Browser verification skipped (OTP send touches production email + DB). **Redeploy crediva to ship.** Known follow-up: the password login page's dev-only demo buttons ("Try Static Demo", "Standard Pilot") still fill an *email* into the username field, so they can no longer authenticate under ID-only rules — they need the demo accounts' 13-digit IDs or removal. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**DHS decline: SEND_DOCS idempotency guard + "Check Communications" button**): stopped the duplicate document-email problem on declined files and added a consumer communications lookup. (1) **Idempotency guard** — for `SEND_DOCS` / `SEND_DOCS_WITH_NCR` declines, `handleDHSDecline` now checks whether the POA/ID document email was already sent for the current decline before sending again. Signal order per user spec: our sent record (`NotificationLog`) first, then a fallback scan of all connected inboxes (only in the staff Handle Decline path, which holds mailbox credentials). Behaviour: if already sent and the file is **not** overdue (case `nextUpdate` not yet passed) it does **not** resend — it records a `[SYSTEM]` skip comment and returns `skippedAsDuplicate` so staff get an amber "Already handled — not resent" panel with a **Resend anyway** override; if the file **is** overdue it resends (a chase is warranted); `forceResend` bypasses the guard. This also fixes the pre-existing bug where the automatic DHS re-check path re-emailed the DC on every re-observation of the same decline. New shared-lib module `packages/shared-lib/src/dhs/decline-dedup.ts` (`findPriorDocsEmail`, `decideDocsResend`), wired into `decline-handler.ts` and exported from the dhs barrel; `DeclineHandlerResult` gained `skippedAsDuplicate` + `alreadyHandledAt`; the `POST /api/cases/[id]/dhs-decline/handle` route now runs the NotificationLog→inbox guard, accepts `{ forceResend, deepInboxCheck }`, and the case page "⚡ Handle Decline" button passes `forceResend(false)` (fixing a latent bug where the click event was passed as the arg) with a Resend-anyway action. (2) **"🔎 Check Communications" button** — new self-contained component + `POST /api/cases/[id]/communications/search` route that finds any communication referencing the consumer by **ID number OR first+last name**, searching outbound `NotificationLog` first then the connected IMAP inboxes (envelope-only, no attachment downloads), shown in a results modal. New generic IMAP search `searchMailboxForConsumer` + pure `mapEnvelopeToMatch` in `packages/shared-lib/src/integrations/imap.ts`, and a shared credential-resolution helper `apps/cases/lib/mailbox-search.ts` (`getSearchableMailboxesWithPasswords`, `searchConsumerAcrossMailboxes`). Verification: added `decline-dedup.test.ts` (9), imap mapper tests (3), 3 SEND_DOCS guard tests in `decline-handler.persistence.test.ts`, `communications/search/route.test.ts` (5), `dhs-decline/handle/route.test.ts` (7) — **219/219 shared-lib dhs+dc+imap tests and 38/38 cases dhs-decline+communications tests green; both packages `tsc --noEmit` clean**. Browser verification skipped (authenticated case-detail page needs a logged-in session + a real DECLINED case + configured mailboxes to exercise). **Redeploy cases and shared-lib to ship.** Manual setup: mailbox IMAP passwords must be saved in Admin → Settings for the inbox stages to read mail (otherwise those stages report "no password saved" and the guard relies on NotificationLog only). AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Push verified: 2026-07-16 (Committed and pushed the pending working tree to origin/main as `dc1dd078` — 56 files: DHS decline SEND_DOCS idempotency guard, Check Communications search, B2B subprojects loading fixes, and referrer management updates.)

> Last updated: 2026-07-16 (**B2B Subprojects Loading Bug Fix**): resolved B2B subprojects/branches not loading in new-case dropdowns, sidebars, and report selectors. The root cause was that B2B branches in the database are stored with type `REFERRER`, while the filter conditions on the B2B dashboard, reports, and sibling apps (finance, forensic-audit, insurance, legal) strictly matched only `BRANCH` or `FOLDER` types. Added `REFERRER` type to all subproject and branch filter arrays across all 5 apps. Verification: verified database records and code consistency. **Redeploy all apps to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer Search: Enable searching B2B parent and branch project names**): expanded the search filter on `GET /api/admin/referrers` to search on `project.name` and `project.parent.name` in addition to referrer fields. This enables searching for branches under their parent project names (e.g. typing "Letsatsi" or "Shosholoza" to find branch referrers like "Port Shepstone" or "Port Elizabeth"). Removed a duplicate, untruncated `apps` subdirectory nested within `apps/cases` that was causing TypeScript compiler warnings/failures. Verification: updated route unit tests, which all passed cleanly (36/36 tests green); monorepo typechecking compiles successfully with zero errors via `pnpm --filter cases typecheck`. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.


> Last updated: 2026-07-16 (**Subproject User Management: Exclude Referrers & B2B Partners**): filtered out non-staff users (e.g., referrers like Ralph Minyuku/William, and B2B partners representing branches) from being selectable as members/team members in both `ProjectMembersModal` and the admin projects manager page (`admin/projects/page.tsx`). Only users of type `STAFF` (or undefined/empty defaulting to staff) are now allowed. Verification: updated the unit tests in `ProjectMembersModal.test.ts` to assert that referrers/B2B partners are excluded (all tests pass cleanly, 49/49 UI tests green), and cases app compiles with zero typescript errors via `tsc --noEmit`. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Hybrid B2B referrers and subproject branch registration**): added a new `'HYBRID'` referrer type that combines discount (clients get discounted pricing) and commission (referrer/employee earns commission on referrals). (1) **Shared Library & Schemas**: Extended `ReferrerType`, `REFERRER_TYPES`, and `REFERRER_TYPE_LABELS` in `@zenowethu/shared-lib` to support `HYBRID`, updating test suites to confirm that `referrerEarnsCommission` evaluates to `true`. (2) **API Routes**: Updated admin referrers POST/PATCH endpoints to validate and allow simultaneous storage of `clientDiscountPercent` and `fixedCommissionAmount` for the `HYBRID` type. Adjusted the `unregistered-folders` list route to include B2B partner roots (allowing their branches to be registered), and updated the `register-from-project` route to auto-configure B2B branches (type `FOLDER` under a B2B root) as `HYBRID` referrers. (3) **UI Dashboards**: Updated the admin registry form and list cells, clients dashboard header/metric cards, AssignReferrerModal search pills, case detail page referrer badge, and client-facing Referrer Portal header description/profile instructions to render and handle `HYBRID` referrers correctly. Verification: 100% of the 774 vitest tests pass cleanly (added 2 new API tests in route.test.ts for POST/PATCH), cases app typechecks with zero errors. **Redeploy cases and shared-lib to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Subproject User Management: Searchable Member Management**): replaced the standard dropdown select for adding subproject members in `ProjectMembersModal` with a premium, searchable combobox input, and added a search filter input for the current members list. Extracted filtering logic into pure, testable helper functions. Verification: wrote 13 unit tests for the filter helpers (all pass, making 48/48 UI tests green), and cases app compiles cleanly with 0 type errors via `pnpm --filter cases typecheck`. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer portal: replace native browser popups with custom ConfirmProvider and sonner Toast**): replaced all remaining native synchronous browser `confirm()` and `alert()` calls in `apps/cases/app/referrer/page.tsx` with asynchronous custom application-generated modals (`confirm`) and styled toast notifications (`toast.success` and `toast.error`) imported from `@zenowethu/ui`. This ensures 100% of the referrer portal uses consistent, application-generated visual components. Verification: 100% of 698 tests passed cleanly, and cases app typechecks successfully with 0 errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer portal: request a quote button next to 'No quote yet'**): added a button next to "No quote yet" in the referrer portal detail drawer, allowing referrers to request a quote. Placed the button inline, visible only for discount referrers when there is no quote yet. Created a new API route `POST /api/referrer-portal/referrals/[caseId]/quote-request` that logs the request via a case comment and sends an in-app notification to the assigned staff member. Verification: 100% of 696 tests passed cleanly, and cases app typechecks successfully with 0 errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer portal: premium custom modal for quote decisions**): replaced the generic browser `confirm()` popup with a premium React state-based confirmation modal on the referrer dashboard (`/referrer`) for quote decisions. It features blur-backdrop overlays, action-specific gradients, SVG check/warning icons, quote totals and invoice code formatting, bullet-point consequences, local error banners, and loading state spinners. Verification: 100% of 694 tests pass cleanly, and cases app compiles with zero typescript errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer portal & admin: exclude soft-deleted cases from referrer tracking and dashboard**): per user request, excluded soft-deleted cases (moved to trash) from both the admin-facing Referrer Clients list/dashboard (`/admin/referrers/[id]/clients`) and the referrer-facing Portal dashboard/details (`/referrer`). Trashed cases no longer show up in lists or count towards referrals, quotes, collections, or stage metrics. Modified the prisma Case queries in the referrer clients route, summary route, detail route, and case comment routes to filter by `deletedAt: null`. Verification: all 19 unit tests across 5 files passed successfully, and cases app typecheck compiles with zero errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Referrer portal: math and filter alignment for discount quotes**): resolved the dashboard quote scoreboard math and filter misalignment. (1) **Math Fix**: Excluded `DRAFT` and `CANCELLED` quote statuses from the scoreboard stats, grouping active quotes into three explicit buckets (Pending = `SENT`/`OVERDUE`; Accepted = `ACCEPTED`/`CONVERTED`/`PAID`/`PARTIALLY_PAID`; Declined = `REJECTED`/`CANCELLED`), ensuring total counts and values always add up exactly (`Total` = `Accepted` + `Pending` + `Declined`). (2) **Table Filter Alignment**: Extended the case summary API to return the array of quote statuses for each case. Updated the frontend tracking table filters to inspect individual invoice statuses directly instead of overall case statuses. Clicking a card like "Pending Quotes" now correctly shows all cases with pending quotes. Verification: 100% of the 694 Vitest tests pass, and cases app typechecks successfully with 0 errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Prod outage fix: Bad Gateway from failed migration in history**): after the Dockerfile crediva fix the image built and deployed green, but cases.zenowethu.co.za returned **Bad Gateway (502)**. Root cause was runtime, not build: the container CMD runs `prisma migrate deploy && node server.js`, and `migrate deploy` was aborting because migration `20260715_add_referrer_missing_client_report` was recorded as **FAILED** in the live DB's `_prisma_migrations` history (Postgres error `42710` — the migration's unguarded `ALTER TABLE ... ADD CONSTRAINT ..._referrerId_fkey` hit a constraint that already existed, because the table had originally been created via `db push`). Prisma refuses to apply any migration while a failed one sits in history, so the CMD exited non-zero and `server.js` never started → Traefik 502. Verified read-only against the live DB that the `ReferrerMissingClientReport` table, its PK, the `_referrerId_fkey` FK, and all three indexes were already present (schema fully correct — only the migration *history* was broken). Resolved with the Prisma-documented, non-destructive remedy `prisma migrate resolve --applied "20260715_add_referrer_missing_client_report"` (writes only to `_prisma_migrations`, runs no SQL, touches no real data). `prisma migrate status` now reports "Database schema is up to date!". No rebuild required — the existing image is fine; the app just needs a restart/redeploy so the startup CMD re-runs against the healed history. **Process note:** applying schema via `db push` and then committing a matching migration file causes exactly this drift; prefer `migrate dev`/`migrate deploy` end-to-end, or make migration DDL idempotent (guard `ADD CONSTRAINT` in a `DO $$` block). AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-16 (**Docker build fix: crediva path in Dockerfile**): fixed the monorepo Docker build error where the COPY instruction attempted to copy package.json from the obsolete 'apps/credo' directory (which was renamed to 'apps/crediva'). Replaced `apps/credo/package.json` with `apps/crediva/package.json` in the root `Dockerfile` to match the workspace renaming. Verification: all 773 shared-lib tests and 694 cases tests passed successfully.

> Last updated: 2026-07-16 (**Referrer portal: quote amounts removed from commission dashboard and enriched on discount dashboard**): per user request, removed any quote metrics, columns ("Quoted" Yes/No), and interactions ("Pending Quotation" box) from the commission-based referrer dashboard to keep it strictly fee/commission/stage focused with zero client pricing visibility. On the discount-based referrer dashboard, retained all quote visibility and enhanced the "Quotes" scoreboard cards to display both the counts and the actual currency amounts for Total, Accepted, and Pending quotes. The summary API `/api/referrer-portal/summary` was extended to return detailed count and amount metrics in `quoteStats`. Verification: 100% of the 694 Vitest tests pass cleanly, and cases app typecheck compiler is successful with zero errors. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-15 (**Discount referrer dashboard: quote statistics + unclaimed missing client rows**): implemented three new features for discount-based referrer dashboards. (1) **Quote Stats Card Group**: a new "Quotes" scoreboard card (violet accent) shows Total Quotes, Accepted quotes, and Pending quotes derived from `Invoice` rows of type `QUOTE` across all the referrer's cases — each stat is clickable and filters the referral tracking table. (2) **Unclaimed Missing Client Rows**: missing client reports submitted via "Report Missing Client" are now persisted in a new `ReferrerMissingClientReport` DB model (migration `20260715_add_referrer_missing_client_report`, applied via `db push`) — the summary API fetches all PENDING reports for the referrer and the frontend renders them as "Unclaimed — pending verification" rows at the bottom of the tracking table (amber pulsing badge, dimmed style). Critically, unclaimed rows are NOT counted in the total client count — the header shows "X clients · Y unclaimed" so the referrer can distinguish confirmed clients from pending claims. (3) **Summary API extended**: `GET /api/referrer-portal/summary` now returns `quoteStats` (total/accepted/pending/rejected) and `missingClientReports` (masked ID numbers, PENDING status only). Verification: all 694/694 Vitest tests pass (80 test files), including updated mocks for `missing-client` route + new `quoteStats`/`missingClientReports` assertions in the summary route test. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.

> Last updated: 2026-07-15 (**Debt Counsellor 5-slot priority email list + durable DHS outcome history + revamped DC admin page**: per user spec, each DC now carries up to 5 priority-ordered contact emails — priority 1 is the address the DC has instructed us to use, and an email a DC mentions in a DHS decline message is auto-promoted to priority 1 (attorney emails from CONTACT_ATTORNEY declines are deliberately excluded), pushing others down; a 6th address drops the P5 entry (kept in `DebtCounsellorEmailHistory`); bounced addresses keep their slot but are skipped so sending falls through P1→P2→P3 (the dc-email-bounce route now flags the DC list and reports the next address). New Prisma models `DebtCounsellorEmail` + `DhsOutcomeEvent` (migration `20260715080000_add_dc_priority_emails_and_dhs_outcome_events`, applied to the live DB) — the event table records every accept/decline per DC with message + category (deduped per case+outcome+message) because `Case.declineReason` only keeps the LATEST decline, making rates from case rows undercount; historical stats were seeded one-event-per-case via the extended backfill (also seeds priority lists from legacy preferred/lastKnown/DHS email fields) and are exact going forward. New shared-lib module `packages/shared-lib/src/dc/` (`email-priority.ts`: promote/getBest/bounce/seed, `outcome-events.ts`: recordDhsOutcome) exported from the main barrel; `handleDHSDecline` records DECLINED events + promotes extracted emails + prefers the priority list when picking the DC send address; `handleDhsAccepted` records ACCEPTED events. Admin DC page (`/admin/debt-counsellors` + detail): now viewable by ALL staff (editing stays admin-only via `canEdit`), list page gained a Top 5 Decliners card row (volume, decline rate, top category, last-declined date) and per-DC accept/decline-rate + last-declined-us pills with P1 email display; detail page gained a Priority Emails manager (reorder/add/remove/restore-bounced, saved via new admin-only `PUT /api/admin/debt-counsellors/[id]/emails`) and a Top 5 Decline Messages section showing the actual message texts with counts; accept/decline rates now use decided-only denominators (accepted+declined, pending excluded) and event history when available (case-status fallback matches loose labels like "Declined Via DHS" which the old `=== 'DECLINED'` comparison missed). Legacy `preferredEmail` is kept in sync with the effective best address so old code paths keep working; staff PATCH of preferredEmail promotes it on the list. Verification: 17 new shared-lib tests + 5 new route tests, 83/83 shared-lib dc+dhs tests green, cases `tsc --noEmit` clean on all touched files (2 remaining errors are pre-existing in the unrelated quote-decision route — spawned as a follow-up task), API + page live-verified on :3000 (401/login-gate reached = full module graph compiles). Email read-tracking (pixel/click) was discussed and deliberately parked as a follow-up. Navigation follow-up (same day): the page was only reachable via the admin-gated `/admin` dashboard "DC Verification" card, so non-admin staff had no way in — added a "Debt Counsellors" link to the shared sidebar's staff Menu section in `packages/ui/src/layout/sidebar/SidebarNav.tsx` (visible to all staff across apps, SmartLink routes it to the cases app; ui `tsc --noEmit` clean). Two bugs found live and fixed same day: (1) exporting `./dc` from the shared-lib main barrel pulled `@zenowethu/database` (Prisma) into client bundles (3 `@zenowethu/ui` components + 8 cases client files import the barrel) breaking page loads — removed the barrel export with a NOTE, matching the existing dhs/openai/xds Node-only convention, consumers deep-import `@zenowethu/shared-lib/src/dc`; (2) `apps/cases/middleware.ts` redirects ALL non-admins away from `/admin/*`, so staff clicking the new sidebar link were silently bounced to `/` (seen live as a stale-build "Failed to fetch" overlay for staff user Kenneth Mokgoshi, isAdmin=false) — added a middleware exception making `/admin/debt-counsellors*` viewable by any logged-in staff while its edit APIs stay isAdmin-gated server-side. AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)

> Last updated: 2026-07-15 (**Referrer portal — premium "Not My Client" modal + ZDM-2026-1011-CUG fix**): replaced the generic browser `confirm()` popup with a polished modal dialog (blurred backdrop, red gradient accent bar, SVG warning icon, three bullet-point consequence list, animated spinner on submit). Fixed case ZDM-2026-1011-CUG not showing the "Not My Client" button — root cause was that the referral detail API, comments GET/POST handlers, payment-report, and quote-decision sub-routes all filtered `deletedAt: null`, so any soft-deleted or partially-deleted case loaded a 404 in the detail pane and the button never rendered. Removed `deletedAt: null` from the `findFirst` where-clause across all four referrer portal sub-routes and updated the two affected test assertions. Verification: 76 test files, 685 tests — all passing. added four interactive actions for both discount and commission referrers: (1) "Not My Client" flag action to submit case comments and notify staff, (2) "Report Missing Client" button and modal sending in-app notifications to admins, (3) "Report Client Payment" modal accepting amount, date, and uploading PDF/image proof of payment files to disk, creating documents, queries, and comments, and (4) "Pending Quotation" panel with Accept and Decline buttons linked to `quote-decision` API route using `recordQuoteDecision`. Verification: wrote API unit tests (`payment-report`, `quote-decision`, `missing-client`) and ran vitest (all 685 tests passing).

> Last updated: 2026-07-15 (**Referrer portal discount dashboard metrics cards expanded**): added both current and previous calendar month metrics cards for quotes (`quoted-this-month`, `quoted-last-month`) and collections/paid amounts (`paid-this-month`, `paid-last-month`) to the discount referrer dashboard `/referrer` (expanding to 6 metrics cards in total). Adjusted the metrics row container to a responsive layout with 6 columns on large screens to display them all in a single row. Verification: verified all cases unit tests compile and pass cleanly (all 682 tests passing).

> Last updated: 2026-07-15 (**Credo to Crediva rebranding completed**): renamed the consumer-facing portal and all its associated modules from **Credo** to **Crediva**. Renamed directories and subprojects (`apps/credo` -> `apps/crediva`, `apps/cases/app/(authenticated)/credo-requests` -> `crediva-requests`, `apps/cases/app/api/credo` -> `crediva`, `packages/shared-lib/src/credo` -> `crediva`, `packages/database/prisma/seed-credo.ts` -> `seed-crediva.ts`, `scripts/backfill-credo-default-passwords.ts` -> `backfill-crediva-default-passwords.ts`). Renamed `Dockerfile.credo` to `Dockerfile.crediva` with updated monorepo path mapping and package filter. Wired up the new image logo PNG in all UI pages (login, OTP, forgot-password, reset-password, change-password, privacy, terms, layout sidebar, LandingNav). Renamed cookie tokens, environment fallback values, session keys, function names, and security components (`CredivaSessionSecurity`). Cleaned up and removed the old `apps/cases/app/(authenticated)/credo-requests` page and `apps/cases/app/api/credo` directory, and updated the Playwright E2E tests and `DisputeButton` component to point directly to the `/api/crediva` endpoint, resolving stale `.next` generated types. Verification: ran vitest on the entire monorepo (all 1680+ tests passed cleanly) and verified TypeScript builds compilation typecheck and ESLint lint check pass successfully.

> Last updated: 2026-07-15 (**Referrer portal commission dashboard revamped & discount dashboard preserved**): updated `/referrer` to use the colorful monthly scoreboard groups (Referrals, Completed, Settled) and unified stage tracking table (showing stage badges, Quoted/Deposit Paid/Arrears columns with hidden financial amounts) exclusively for commission referrers. Kept the discount referrer dashboard exactly as it was (with the 8 monthly quoted/paid scoreboard tiles, original table columns, and the detail pane Client Finances box). Verification: confirmed both formats render conditionally and cases unit tests (all 682 tests passing cleanly).

> Push verified: 2026-07-15 (Committed and pushed the pending working tree to `origin/main` as `b331a097` — 44 files: Credo to Crediva transition and referrer portal dashboard revamp. Local debug artifacts deliberately left uncommitted: `build_results_3_utf8.txt`, `pnpm_check.txt`, `typecheck_output*.txt`.)

> Last updated: 2026-07-14 (**AI Plan generation fixed to dynamically resolve model/client from shared AI provider client, enabling OpenAI fallback on production**): the plan engine (`planner.ts` and `evaluator.ts`) previously had a hardcoded client block only checking `OPENROUTER_API_KEY` and `ANTHROPIC_API_KEY`. Since only `OPENAI_API_KEY` is configured on production (per `DEPLOYMENT.md` specifications), plan generation failed with a 401/500 error because it was called with `apiKey: undefined` against Anthropic. Fix: refactored `planner.ts` and `evaluator.ts` to retrieve the OpenAI client and model dynamically via `getAiClientForTask('plan_generation')` from `@zenowethu/shared-lib`. This enables standard OpenAI (`gpt-4.1`) execution out-of-the-box on production when only `OPENAI_API_KEY` is present, while still supporting dynamic DB-configured AI providers and OpenRouter fallbacks. Updated `describeAiProvider()` in `errors.ts` to include standard OpenAI as a third fallback option. Verification: updated mocks in `planner.removal.test.ts` and stubbing in `errors.test.ts` — all 50/50 vitest tests green, manual case execution against case `cmrgamg3m0035wzvoamv02rcb` successfully generated plan steps. **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)

> Last updated: 2026-07-14 (**Referrer portal discount partner scoreboard enriched with monthly quoted and paid metrics**): added quoted in the current and previous calendar months, and paid/collected in the current and previous calendar months to the discount partner dashboard on `/referrer`. Updated totals calculations (`calculateDiscountPartnerTotals()`) to track `quotedLastMonth` and `paidLastMonth`, updated route API mapping to expose `paidLastMonth`, added new metrics filters (`quoted-last-month`, `paid-last-month`), updated the metrics card row with responsive layout grid to support 6 metrics cards, and verified changes with unit tests (all 682 tests passing cleanly).

> Last updated: 2026-07-14 (**Credo to Crediva Transition**: renamed the consumer portal from "Credo" to "Crediva" across the entire codebase to match the new domain name `crediva.zenowethu.co.za`. This included: (1) modifying default env/config fallbacks in `packages/shared-lib`, `apps/cases`, `apps/credo`, and `apps/finance`; (2) updating all user-facing branding and template texts in auth/registration pages, email notifications, dashboard layouts, and AI credit coach prompts; (3) adjusting related unit test assertions/mocks to refer to `crediva.zenowethu.co.za` or `"Crediva"` and successfully verifying that all test suites pass. Fixed in passing: resolved a typescript build compilation issue in `packages/plan-engine/src/evaluator.ts` where the undefined variable `MODEL` was referenced instead of `modelId`. Verification: all 756 shared-lib tests, 56 credo tests, 682 cases tests, and 138 finance tests pass successfully. **Redeploy cases, finance, and credo to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.)
...
### Fixed: Dokploy Reporting Build — TypeScript Errors in `pdf-image.ts` and a Reporting Test (2026-08-18)

**Trigger:** the Dokploy `reporting` application build failed. Turbopack compiled successfully in 39.7s, then the `Running TypeScript` step reported `../../packages/shared-lib/src/pdf-image.ts:23:5 — Type 'string | null' is not assignable to type 'string'` and the Next build worker exited 1.

**Root cause:** `getPdfJsSource()` / `getPdfJsWorkerSource()` cache the PDF.js bundles in module-level `let _pdfJsSource: string | null`. They read the file through a lazy `const { readFileSync } = require('fs')`, which is untyped, so the assignment inside the `if (!_pdfJsSource)` block wrote `any` and TypeScript reset the variable to its declared type rather than narrowing it — leaving `string | null` at the `return`.

**What changed:**
- `packages/shared-lib/src/pdf-image.ts` — both getters now type the lazy requires (`require('fs') as typeof import('fs')`, `require('path') as typeof import('path')`) and guard with `if (_pdfJsSource === null)`. `readFileSync(path, 'utf-8')` is now typed `string`, so the cache narrows to `string` on both branches. Runtime behaviour is identical (the Edge-Runtime-safe lazy require is preserved); only the empty-string edge case changes, and an empty PDF.js bundle is not a real state.
- `apps/reporting/app/api/reporting/projects/route.test.ts:30` — `vi.mocked(auth).mockResolvedValue(null)` → `null as any`, matching the pattern already used throughout the reporting test suite (e.g. `finance/route.test.ts:57`). NextAuth's `auth` is overloaded, so `vi.mocked` resolves it to the `NextMiddleware` signature and rejects a bare `null`. Next type-checks every file matched by the app's `**/*.ts` include — test files included — so this would have broken the next build.

**Why only reporting:** `apps/reporting/next.config.ts` has no `typescript.ignoreBuildErrors`, so it is the only app whose Docker build actually enforces the type check. The same shared-lib error exists for every app; the others just suppress it.

**Checks run:** `tsc --noEmit -p apps/reporting/tsconfig.json` → clean (exit 0, was 2 errors). `tsc --noEmit` on `packages/shared-lib` → no `pdf-image` errors. `vitest run app/api/reporting/projects/route.test.ts` → 2/2 passing.

**Remaining:** commit and push to `main`, then re-run the Dokploy `reporting` deployment. No migration, no env var, no manual setup.

---

### Added: Finance Month-by-Month Instalment Tracking, Per-Month Payment Allocation & Back-Dated Capture (2026-08-18)

**User request:** after capturing a payment the user should land back on that consumer's file (the Finance case detail page), staff should be able to record whether a previous month's instalment was paid, the file should show how many months the consumer is paying over and which months were paid or not (e.g. months 1–2 paid, 3 not paid, 4–5 paid, two payments in month 7), and staff must be able to back-date month 3's payment even after month 7 has already been captured — "proof brought forward".

**What was already working:** `PaymentArrangement` / `PaymentArrangementInstalment` already modelled a dated instalment schedule with per-line `amountDue` / `amountPaid` / `status`, `packages/shared-lib/src/payments/arrangement-logic.ts` already built schedules and summarised them, and the Finance case page already rendered the schedule with Honoured/Missed buttons and a Next Payment Date.

**What was broken or incomplete:**
- `Payment` had no link to an instalment. Every COMPLETED payment on the case was thrown into one pool and `reconcileInstalments` spread it strictly oldest-month-first. That made the user's scenario impossible to express: money captured for month 4 would silently settle month 3 instead, so a genuinely skipped month could never stay skipped, and a back-dated capture would shunt every later month's status along by one.
- Two payments in the same month could not be represented at all, and any overpayment was clamped away by `Math.max(0, …)`.
- "+ Record Payment" on a case sent staff to `/payments/record?idNumber=…` and, on success, pushed them to `/payments` — the global list, not the consumer they were working on. The route also ignored which case they came from and re-guessed it as the client's most recent open case, which picks the wrong file for a consumer with more than one.
- Nothing on the page said how many months the schedule ran for, or how many were paid versus missed.
- `setInstalmentHonoured` was a boolean with no way to undo a mis-click.

**What was risky:** letting a payment settle a month staff did not intend is a money-allocation error on a live production DB, and it silently corrupts the Finance "Next Payment Date" that drives the per-app overdue trigger. The allocation therefore had to be explicit and validated server-side — an instalment is rejected with 400 unless it belongs to the payment's own case.

**What changed:**
- **Schema:** `Payment.instalmentId` (nullable, `onDelete: SetNull`) + index, and a `payments` back-relation on `PaymentArrangementInstalment`. Null deliberately preserves the old FIFO behaviour, so nothing about existing files changes until someone allocates by hand.
- **`arrangement-logic.ts` — two-layer reconciliation.** Pinned payments (`allocations`) settle exactly their own month and never leak; whatever is left unpinned still fills the oldest open month FIFO. `ReconciledInstalment` gained `overpaid`, `paymentCount`, `broughtForward` and `allocations`. A month staff explicitly marked MISSED now stays MISSED even before it falls due. A second pass flags `broughtForward` when a month's earliest capture timestamp is later than a *later* month's — precisely the "entered month 3 after month 7" case.
- **`ArrangementSummary`** gained `partialCount`, `pendingCount`, `waivedCount`, `outstandingCount`, `overpaidTotal`, `allocatedPaymentCount` and `broughtForwardCount`.
- **`payment-arrangement-service.ts`:** the FIFO pool now excludes allocated payments (`instalmentId: null`) so money is never counted twice; `getArrangementView` loads each instalment's COMPLETED payments as allocations; new `allocatePaymentToInstalment()` (re-syncs both the old and new arrangement) and `resyncCaseArrangements()`; `setInstalmentHonoured` takes `'PAID' | 'MISSED' | 'AUTO'`, where AUTO clears the manual call and hands the month back to the recorded payments.
- **UI:** the case page's "+ Record Payment" now carries `caseId`; the record page loads the case's schedule, offers an "Apply to instalment" picker listing every month with its due date, amount, status and existing payment count, pre-fills the outstanding amount, warns "Proof brought forward" when the chosen month sits before ones already paid, warns when the month is already settled, and on success routes to `/cases/{caseId}`. Back and Cancel return to the file too. The schedule table now labels rows "Month N", shows paid/not-paid/part-paid/still-to-come counts and a "Paying over N months" headline, lists each payment behind a month, badges multi-payment and brought-forward months, shows overpayment, offers Paid / Not paid / Clear, and links straight to a pre-targeted capture for any month still short.

**Files changed:** `packages/database/prisma/schema.prisma`; `packages/shared-lib/src/payments/arrangement-logic.ts`; `packages/shared-lib/src/payments/payment-arrangement-service.ts`; `apps/finance/app/api/finance/payments/route.ts`; `apps/finance/app/api/finance/payments/[id]/route.ts`; `apps/finance/app/api/finance/instalments/[id]/route.ts`; `apps/finance/app/(authenticated)/payments/record/page.tsx`; `apps/finance/app/(authenticated)/cases/[id]/page.tsx`; `apps/finance/app/(authenticated)/cases/[id]/PaymentArrangements.tsx`.

**Migration added:** `packages/database/prisma/migrations/20260818120000_add_payment_instalment_allocation` — additive only (one nullable column, one index, one FK). **Applied to production 2026-08-18** via `prisma migrate deploy`, alongside the previously-uncommitted `20260814090000_add_document_ownership_verification`; verified against the live DB.

**New API behaviour (no new endpoints):** `POST /api/finance/payments` accepts `caseId` and `instalmentId`; `PATCH /api/finance/payments/[id]` accepts `instalmentId` to re-point or release an already-captured payment; `PATCH /api/finance/instalments/[id]` accepts `outcome: PAID | MISSED | AUTO` (the old `honoured` boolean still works).

**Tests:** 7 new in `packages/shared-lib/src/payments/arrangement-logic.test.ts` covering the user's exact scenario — month 3 unpaid while 4 and 5 are paid, two payments plus overpayment on month 7, brought-forward detection, pinned money not leaking, unpinned money still FIFO, and an explicit not-paid mark surviving on a future month — plus a month-count summary test. 5 new in `apps/finance/app/api/finance/payments/route.test.ts` covering case pinning, unknown case → 404, instalment storage, cross-case instalment → 400, and null instalment falling through to FIFO.

**Checks run:** shared-lib 903/903 green, finance 147/147 green, `pnpm --filter finance typecheck` (`tsc --noEmit`) clean. Prisma client regenerated. No browser verification — dev servers were stopped at the user's request to unlock the Prisma engine DLL.

**Remaining gaps:** batch-imported partner payments still land unallocated and fill oldest-month-first, which is the right default but means partner batches never populate a specific month; there is no UI yet to re-point an already-captured payment at a different month (the `PATCH` accepts it, nothing calls it); and the Cases app's own case view does not show the arrangement at all — the schedule remains Finance-only.

**Deploy:** apply the migration (`npx prisma migrate deploy` from `packages/database`), then redeploy finance.

---

### Changed: Consumer Email Address Is Now Mandatory on Case & Referral Creation (2026-08-18)

**User request:** email must be mandatory on all new cases created by staff and on referrals created by B2B partners, because a consumer cannot be updated without one.

**What was already working:** the Cases-app staff new-case flow already blocked submission without an email on its document-analysis path (`handleSubmit`), and every create schema already rejected a *malformed* email.

**What was broken:** nothing enforced a *present* email. The B2B referral forms in all five apps carried the comment "phone and email are now optional" and sent `email: email || null`. `ClientCreateSchema.email` was `.optional().nullable()` in both schema files, so the API accepted referrals with no email at all. The five `/api/v1/cases` API-key routes and the legal/insurance/forensic smart-start create routes had no Zod validation on the client at all — the forensic new-subject form had no email input on the page. `CasePatchSchema` allowed `client.email: null`, so saving the case-detail client editor with a blank field silently wiped an address that was already on file.

**Design decision — provisional shells:** the Cases and partner new-case flows open a placeholder case (`TEMP-<ts>` / `MANUAL-<ts>` ID, name "Temp Processing") *before* the consumer details are known, then PATCH the real data on once documents are AI-analysed. A blanket create-time requirement would have broken that flow, so `isProvisionalIdNumber()` exempts those shells at create time and the PATCH route requires the email at the exact moment a shell becomes a real consumer.

**Server-side (the actual gate):**
- `apps/cases/lib/schemas.ts` + `packages/shared-lib/src/schemas.ts`: new `PROVISIONAL_ID_PATTERN` / `isProvisionalIdNumber()`; `CaseCreateSchema.superRefine` requires `client.email` for every non-provisional client; `CasePatchSchema.superRefine` rejects an empty/null `client.email` when the key is supplied.
- `apps/cases/app/api/cases/[id]/route.ts`: rejects a PATCH that converts a provisional shell into a real consumer without an email (`code: MISSING_EMAIL`).
- `apps/{cases,finance,legal,insurance,forensic-audit}/app/api/v1/cases/route.ts`: require and normalise `client.email` before creating a Client.
- `apps/legal/app/api/legal/cases/create`, `apps/forensic-audit/app/api/cases/create`, `apps/insurance/app/api/insurance/cases/create`: same requirement for new clients, and the client upsert now writes the normalised address instead of possibly clearing it.

**UI (so staff never see a raw 400):** Email marked required with a red asterisk, red border and inline "Email is required" hint, plus a pre-submit guard, on — the staff new-case page in all five apps (including a brand-new email input on the forensic smart-start form, which had none), the partner new-case page in all five apps, the B2B dashboard referral form in all five apps, and the case-detail client editor in cases/legal/insurance/forensic-audit.

**Deliberately unchanged:** joint-applicant (spouse) email stays optional — confirmed by the user on 2026-08-18: the main applicant is the notified party, the spouse is optional. Locked in by two regression tests. Note that the staff and partner new-case forms render no Joint Email input at all (the `jointEmail` state is always submitted empty); the B2B dashboard form is the only one that has the field; Crediva/service-request conversions already inherit a verified `ConsumerAccount` email; existing cases with no email are only blocked when someone edits the consumer, not on read.

**Tests:** new `packages/shared-lib/src/schemas.test.ts` (10 tests) and 11 new `CaseCreateSchema`/`CasePatchSchema`/`isProvisionalIdNumber` tests in `apps/cases/lib/schemas.test.ts`. Full suites green — cases 970/970, shared-lib 896/896. `tsc --noEmit` clean on cases, legal, insurance, forensic-audit and shared-lib (finance reports errors only in a stale `.next/dev/types/validator.ts` generated on 2026-08-10, unrelated).

**Deploy:** redeploy cases, finance, legal, insurance and forensic-audit. No migration.
...
### Referral Portal Outstanding Metrics (2026-07-16)
- [x] Added `totalOutstanding`, `outstandingThisMonth`, and `outstandingLastMonth` calculations to referrer portal.
- [x] Implemented outstanding metrics mapping and filtering in `apps/cases/app/referrer/page.tsx`.
- [x] Nested metrics cards layout for Quoted, Paid, and Outstanding to place total amounts above month breakdowns to prevent text wrapping.
- [x] Added Vitest unit tests in `apps/cases/lib/referrer-portal.test.ts` and updated mock route expectations in `apps/cases/app/api/referrer-portal/summary/route.test.ts`.
...
### 🔴 Immediate (Do First)
- [x] **Referral Portal Phase 2 (Commission-based referrers)** — Added scoreboard summary, Claim a Client secure flow, Not My Client warnings, general document uploads, and feedback requests.
- [x] **AI-Driven File Requests** — Verified the background cron route trigger endpoint.
- [x] **Mailbox Connector for DHS Fee Declines** — Implemented IMAP ingestion deduplication ledger and case document matching (using ProcessedMailboxMessage table and SHA-256 attachment hashing).
- [x] **Referral Portal Phase 1** — Revamped the referrer portal to use monthly count scoreboards, hide all client financial details, and show stage progress badges on the tracking table.
...
| Referrer Portal | 100% | Phase 1 & 2 completed successfully with POPIA compliance ✅ |
...
| Staff Work Reporting | 100% | Successfully created, verified with Vitest, and built cleanly ✅ |
...
- **2026-08-07**: Fixed Clearance PDF generation silently failing and not uploading due to slashes in file number. Fixed frontend UI to properly handle API errors.
- **2026-08-07**: Fixed the actual root cause of "Evaluate Clearance" → Form 17.W generation failing: `POST /api/cases/[id]/debt-review/generate`'s Zod schema only accepted `targetStatus: 'F1' | 'F2' | 'G'`, but `GET /api/cases/[id]/clearance/evaluate` recommends `targetStatus: 'B'` for Form 17.W withdrawals outside DHS statuses D3/D4 (the common case — e.g. "ZDM Client" status). Every such request was rejected with a 422 before any PDF/DB work happened, so no `DebtReviewDocument`, vault `Document`, or file was ever created — matching the reported bug exactly. Extended the enum to `['F1', 'F2', 'G', 'B']` in `apps/cases/app/api/cases/[id]/debt-review/generate/route.ts`. Added regression tests (accepts `targetStatus: 'B'` for `FORM_17_W`; still rejects genuinely invalid values with 422). Verified live against the exact reported case (STEPHEN AAU RAMPAI, ZDM-2026-1055-W8R): Form 17.W PDF now generates, saves to disk, and appears in the case document vault.
- **2026-08-07**: Rebuilt Form 17.W (`apps/cases/lib/form17w-pdf.ts`) to match the Form 19 clearance-certificate branding (navy/gold numbered section headers, bordered label/value rows, real letterhead — the old version painted an opaque teal header bar over the letterhead logo) and to match the actual NCA regulatory template: added a "Creditors Notified" table (name/address/tel/fax placeholder/email + current balance/instalment/interest rate per outstanding account, with wrapped variable-height rows) and the four fixed Section 71(2)(b)(i) withdrawal-reason options (A–D) with the applicable one highlighted. `Form17WData` now takes `accounts: Form17WAccount[]` and `selectedOption: 'A'|'B'|'C'|'D'` instead of a free-text `reasonForWithdrawal`; `debt-review/generate/route.ts` selects option C for court-rescission (`targetStatus: 'G'`) and option A otherwise.
- **2026-08-07**: Closed the gap where `CreditAccount.creditProviderId` was never populated anywhere in the app (confirmed via audit — not on AI credit-report sync, not on manual entry), so Form 17.W's creditor contact columns were always blank even when a matching `CreditProvider` existed. Added `apps/cases/lib/credit-provider-match.ts` (conservative name matching — exact-then-normalized, deliberately does NOT strip words like "GROUP" since credit reports can list e.g. "NIMBLE" and "NIMBLE GROUP" as distinct creditors on the same case). Wired it into `credit-accounts/sync/route.ts` (auto-links on account creation) and `debt-review/generate/route.ts`'s FORM_17_W branch (best-effort auto-link + persist for accounts synced before this existed). Added `PATCH /api/cases/[id]/credit-accounts/[accountId]` to link/unlink a `CreditProvider` (case-scoped, any authenticated staff can link to an existing provider; creating a new provider still requires isAdmin/isExecutive/isSeniorManager via the existing `/admin/credit-providers` POST). Added `CreditorLinkPicker` component (search existing providers or quick-create) wired into `DebtReviewTab.tsx`'s "missing email" warning list. **Operational note**: verified live that the `CreditProvider` directory currently has **0 records** in production — staff need to start populating it (via the new inline picker or `/admin/credit-providers`) for this and future documents/creditor emails to carry real contact details.
