...
> Last updated: 2026-07-29 (**Reporting App: Self-Filtering & Roster Badges Deployed**): Deployed session history tracking, role-gated productivity forecasting, and user self-filtering for the Reporting app at `apps/reporting` (port 3008). (1) **Database & Sessions** — Created `EmployeeSession` model and applied migration `20260729231800_add_employee_session`. Recorded availability session bounds on check-in and check-out via raw SQL to bypass Windows DLL generation locks. (2) **Role-Gated Predictions** — Created `/api/reporting/presence/sessions` returning analytics and AI forecasting (day, week, month, 3-months) role-gated to show predictions only to Executives/Admins, returning null for managers. (3) **User Self-Filtering** — Configured all dashboards to exclude the logged-in user from roster list counts and mapping logs (preventing users from seeing or counting themselves on their own workspace). (4) **High-Visibility Counters** — Integrated bold emerald status badges (`bg-emerald-50 text-emerald-600 font-extrabold`) on Staff's `Who's Online`, Manager's `Team Roster`, and Executive's `Availability Roster` headers for quick monitoring. (5) **Tests & Builds Verify** — All 16 Vitest unit tests pass and build compiles.
