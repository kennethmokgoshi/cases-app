...
> Last updated: 2026-07-30 (**Reporting App Dokploy Deployment & Monorepo Runner Fix**): Resolved Dokploy build failure for `personal-projects-reporting-jm7r4o` (`Dockerfile.reporting`). Root Cause & Fix: (1) **Monorepo Standalone Runtime Resolution** — `Dockerfile.reporting` previously attempted to copy `.next/standalone` without workspace `node_modules` or `packages/`. In Next.js monorepo builds with external packages (`@prisma/client`, `bcryptjs`), standalone execution failed at runtime due to missing modules. Updated `Dockerfile.reporting` runner stage to copy full built `node_modules`, `packages/`, and `apps/reporting` directory, running `cd apps/reporting && pnpm start` (matching the proven root `Dockerfile`). (2) **Fixed Vitest Mock Type Errors** — Fixed 5 TS2345 type errors in `apps/reporting` test files (`check-in`, `bulk-verify`, `logs`, `presence/sessions`) by properly casting `auth` mock calls, bringing `npx tsc --noEmit` and `npx vitest run` to 100% green (16/16 tests passing). (3) **Dockerfile APT Package Cleanup** — Removed obsolete `libssl1.1` pinning and cleaned up heavy APT package lists across `deps`, `builder`, and `runner` stages to match root `Dockerfile`. (4) **Updated tsconfig.json Exclude List** — Updated `apps/reporting/tsconfig.json` to explicitly exclude `"public"`, `"storage"`, `".next"`, `"dist"`, and `"out"`, preventing `tsc` directory traversal hangs during Next.js production build in Docker. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.
