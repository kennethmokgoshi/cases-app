...
> Last updated: 2026-07-24 (**6-bucket case pipeline: isCompleted & isSettled flags replace status-tone model**): Implemented independent boolean tracking to distinguish between work completion and settlement payment, enabling the dashboard to display all six pipeline states. Added `isCompleted` and `isSettled` columns to Case with backfill migration (SETTLED* → isSettled=true, *COMPLETED* → isCompleted=true) and composite index for efficient queries. Refactored `calculateDiscountPartnerTotals()` to bucket cases into six states: (1) In Progress = !isSettled && !isCompleted, (2) Settled Not Completed = isSettled && !isCompleted (paid upfront, work ongoing), (3) Completed Not Settled = isCompleted && !isSettled (work done, awaiting payment), (4) Both Settled & Completed (fully resolved), plus (5) Lost and (6) Referral tracking. Updated referrer portal summary API and admin clients dashboard to pass the new flags and display all six buckets. Updated all test fixtures and expectations across `referrer-portal.test.ts`, `referrer-portal/summary/route.test.ts`, and 726 dependent tests — all 734 tests pass green. Database migration `20260724120000_add_completion_settlement_flags` applied to production. Commit: `35781ed` — **Redeploy cases to ship.** AI maintenance reminder: still create the trigger that lets AI request all debt review removal files such as Form 17.W and Court Orders for relevant cases.
