...
> Last updated: 2026-07-15 (**Referrer portal commission dashboard revamped & financial info hidden**): updated `/referrer` to use the colorful monthly scoreboard groups (Referrals, Completed, Settled) with this and last calendar month counts and trend deltas for both commission and discount referrers. Removed all financial tiles showing quote/paid amounts or commission payouts. Unified the tracking table to show File, Consumer, Stage (representing commission stage, styled with custom status badges), Quoted (Yes/No), Deposit Paid (Yes/No), Payment Status, and Agreement Status, hiding all R currency amounts. Revamped the details panel to display only workflow progress and requested documents, hiding all client financials and commission grids to ensure client financial privacy. Removed statement download option, follow-up submission form, and follow-up history cards. Verification: updated all mock tests in `summary/route.test.ts` and `[caseId]/route.test.ts` to expect `commissionStage` and updated assertions (all 682 tests passing cleanly).

> Push verified: 2026-07-14 (Committed and pushed the pending working tree to `origin/main` as `acb164df` — 33 files: AI plan engine fixes (step-registry tree-shaking fix, failed-step retry, plan-generation error classifier), referrer portal status/dashboard overhaul, and the case-detail Referrer card with Assign/Change/Remove. Local debug artifacts deliberately left uncommitted: `build_results_3_utf8.txt`, `pnpm_check.txt`, `typecheck_output*.txt`, `apps/cases/scratch/test-ai-plan.js`, and `packages/plan-engine/src/test-real.test.ts` (hits the live DB/AI with a hardcoded case ID — would fail CI).)
