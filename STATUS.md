...
> Last updated: 2026-07-23 (**Email Document Harvesting for Missing ID, POA & Credit Reports**): Added targeted email document harvesting buttons to scan connected mailboxes and automatically upload found files when ID, POA, or Credit Report are missing. (1) **API Update**: Updated `GET /api/ai/plan/case/[caseId]` to query and return `uploadedDocTypes` array containing all present document types on the case. (2) **AI Plan Tab & Confidence Report**: Added new props `uploadedDocTypes` and `onRefresh` to the `<ConfidenceReport />` component in `packages/ui/src/plan/ConfidenceReport.tsx`. Integrated inline progress bar and status message display when a mailbox search is running. Extracted missing check logic to an exported helper `getMissingDocsStatus()` with a comprehensive unit test suite in `ConfidenceReport.test.tsx` (5 tests passing). (3) **Documents Tab**: Added the same two email search buttons ("Search Email for ID/POA" and "Search Email for Credit Report") under the action buttons header in `DocumentsTab.tsx` when they are detected as missing from the case documents. Integrated the search progress status in the extraction progress bar. Verification: 54/54 `ui` Vitest unit tests pass green, TypeScript typecheck (`tsc --noEmit` on cases app) compiles cleanly. AI maintenance reminder: always create a trigger letting AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.
