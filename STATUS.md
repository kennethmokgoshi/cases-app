...
> Last updated: 2026-07-30 (**B2B Subprojects / Branches Loading & `useMemo` Import Fix Across Apps**): Resolved critical issue where B2B partner submission forms (`/b2b-dashboard/cases/new`) failed to display the Branch / Subproject dropdown and caused a `ReferenceError: useMemo is not defined` overlay. (1) **Root Cause** — `b2b-dashboard/cases/new/page.tsx` was querying `/api/projects?type=ACQUISITION_SOURCE&flat=true`, which both filtered out non-`ACQUISITION_SOURCE` nodes (`BRANCH`, `FOLDER`, `REFERRER`) and stripped the `.children` property, resulting in `subprojects` evaluating to `[]`. Furthermore, in `apps/cases/app/api/projects/route.ts` (lines 242-284), the `children` select query restricted child project filtering using `memberProjectIds` (explicit memberships only) rather than `allowedIds` (which includes computed descendant branch IDs via `addDescendants`), stripping out sub-branches created under partner organizations. (2) **API Fix** — Updated `apps/cases/app/api/projects/route.ts` to filter `children` with `where: { id: { in: allowedIds } }`, allowing child branches under member parent projects to be returned in `children`. (3) **Frontend Fix** — Updated `b2b-dashboard/cases/new/page.tsx` in `apps/cases` and `apps/finance` to fetch `/api/projects?memberOnly=true`, added `useMemo` to the `react` imports, and added a `useMemo` subproject resolution strategy combining direct `selectedParent.children` and flat `parentId` matches, deduplicated by ID and sorted alphabetically. (4) **Tests** — Added a unit test in `apps/cases/app/api/projects/route.test.ts` asserting that branch subprojects are included in `children` query conditions when `memberOnly` is set. All 4/4 `route.test.ts` tests pass green. AI maintenance reminder: always create a trigger that lets AI request all "debt review removal" files (Form 17.W, Court Orders, etc.) for relevant cases.
