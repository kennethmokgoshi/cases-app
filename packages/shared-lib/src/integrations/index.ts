...
// NOTE: imap is intentionally NOT re-exported here. It statically imports imapflow,
// which pulls in pino/thread-stream — bundling that chain breaks `next build` in every
// app that imports this barrel but does not depend on imapflow. Import it by deep path
// (and keep imapflow/pino in that app's dependencies + serverExternalPackages):
//   import { verifyImapConnection, scanMailboxForClient } from '@zenowethu/shared-lib/src/integrations/imap'
