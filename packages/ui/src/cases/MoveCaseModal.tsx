...
import { DestinationPathSelector } from '../projects/DestinationPathSelector';
...
export function MoveCaseModal({ isOpen, onClose, selectedCaseIds, onSuccess }: MoveCaseModalProps) {
    ...
    const handleMove = async () => {
        ...
        try {
            ...
            // 3. Perform Move
            const moveRes = await fetch('/api/cases/move', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (!moveRes.ok) {
                ...
                throw new Error(text || 'Failed to move cases');
            }
            ...
            <div className="w-full max-w-lg bg-zeno-navy border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ring-1 ring-white/20 animate-in fade-in zoom-in duration-200">
                <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">Move Cases</h2>
                            <p className="text-xs text-gray-500">Transfer cases to another folder</p>
                        </div>
                    </div>
                ...
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                    <div className="p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-xl">
                        <p className="text-sm text-cyan-300">
                            You are moving <span className="font-bold text-white">{selectedCaseIds.length}</span> case(s).
