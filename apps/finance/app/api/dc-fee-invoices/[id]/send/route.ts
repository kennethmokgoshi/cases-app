/**
 * POST /api/dc-fee-invoices/[id]/send  (Finance)
 *
 * Email a DC fee invoice/quotation (PDF attachment) to the debt counsellor.
 * Defaults the recipient to the DC email on the document. Uses the Finance
 * email helper (`@/lib/email`). Body: { to?: string; message?: string }
 */
...
export async function POST(
  ...
  try {
    ...
    if (!to) {
      return NextResponse.json(
        { error: 'No debt counsellor email on the document — provide a recipient address.' },
        { status: 400 },
      )
    }
    if (invoice.status === 'CANCELLED') {
      return NextResponse.json({ error: 'Cannot send a cancelled document' }, { status: 409 })
    }
    ...
    if (!emailResult.success) {
      logger.error(`[dc-fee-invoice send] (finance) ${invoice.invoiceNumber} email failed: ${emailResult.error}`)
