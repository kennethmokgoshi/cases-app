...
export async function generateInvoicePdf(data: InvoiceData): Promise<Uint8Array> {
  ...
  // ── Page factory ──────────────────────────────────────────────────────────
  // showTableHeader: true  → draw a repeated table header (continuation pages)
  //                  false → caller will draw their own content (page 1 / totals page)
  function addPage(showTableHeader: boolean): void {
    ...
    if (letterhead) {
      ...
      drawText(page, 'Tel: +27 81 747 7616  |  notifications@zenowethu.co.za  |  www.zenowethu.co.za', MARGIN, H - 88, regular, 7, rgb(0.60, 0.60, 0.60))
      page.drawLine({ start: { x: MARGIN, y: FY + 26 }, end: { x: RIGHT, y: FY + 26 }, thickness: 0.5, color: ORANGE })
      drawText(page, 'Zenowethu Debt Management | NCRDC3693 | Suite 2, 2nd Floor, Central House, 17 Central Road, Mabopane, 0190', MARGIN, FY + 14, regular, 6.5, GRAY_TEXT)
      drawText(page, 'Tel: +27 81 747 7616  |  Cell: 082 363 8207  |  notifications@zenowethu.co.za  |  www.zenowethu.co.za  |  Member of DCASA', MARGIN, FY + 2, regular, 6.5, GRAY_TEXT)
    }

    cursor = CONTENT_TOP

    // Continuation pages: small label so staff can see it continues
  ...
  // 1. Document title block (right-aligned)
  ...
  // Right column: invoice/quote details, positioned below the title block
  ...
  let rowIndex = 0  // reset each page for correct alternating stripe
  ...
  for (let i = 0; i < data.lineItems.length; i++) {
    ...
    // If this row won't fit, close the current table and start a new page
    if (cursor - rowH < CONTENT_BOTTOM) {
      // Close current page with a bottom rule
    ...
    // Numeric columns align to the first (top) line baseline
  ...
  // ── 5. TOTALS — move to a new page if they won't fit ─────────────────────
  ...
  // For letterhead: draw just above the pre-printed footer band
  // For fallback:   draw on the footer line already rendered by addPage()
