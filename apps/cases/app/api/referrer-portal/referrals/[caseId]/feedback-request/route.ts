...
const logger = createLogger('api/referrer-portal/referrals/feedback-request');
...
export async function POST(request: Request, { params }: { params: Promise<{ caseId: string }> }) {
    try {
        ...
        // Verify case exists and belongs to this referrer
        ...
        // Create case comment
        const commentContent = `[SYSTEM AUTO-MESSAGE] [FEEDBACK REQUEST] Referrer requested an update / feedback on this case.`;
        ...
        if (referralCase.assignedToId) {
            await prisma.inAppNotification.create({
                data: {
                    ...
                    title: `Feedback requested on ${referralCase.fileNumber}`,
                    message: `${access.referrer.firstName} ${access.referrer.lastName} is asking for an update on this case.`,
                    caseId,
                    commentId: comment.id,
                    linkUrl: `/cases/${caseId}`,
                },
            }).catch((err: unknown) => logger.error('Failed to notify staff of feedback request', err));
        }
        ...
        return NextResponse.json({ error: 'Failed to request feedback' }, { status: 500 });
    }
}
