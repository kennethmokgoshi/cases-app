...
function req() {
    return new Request('http://localhost/api/referrer-portal/referrals/case-123/quote-request', {
        method: 'POST',
    });
}

describe('POST /api/referrer-portal/referrals/[caseId]/quote-request', () => {
    beforeEach(() => vi.clearAllMocks());

    it('creates quote request comment and staff notification', async () => {
        ...
        expect(prisma.caseComment.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                caseId: 'case-123',
                content: expect.stringContaining('[QUOTE REQUEST]'),
            }),
        }));
    ...
    it('returns 404 if case not found or not owned by referrer', async () => {
        vi.mocked(getCurrentReferrerPortalAccess).mockResolvedValueOnce({
            ok: true,
            sessionUserId: 'user-1',
            referrer: { id: 'ref-1', firstName: 'William', lastName: 'Maesela' },
        ...
        expect(json.error).toBe('Referral not found');
    });
});
