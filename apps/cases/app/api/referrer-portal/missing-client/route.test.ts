...
function req(body: unknown) {
    return new Request('http://localhost/api/referrer-portal/missing-client', {
        method: 'POST',
...
describe('POST /api/referrer-portal/missing-client', () => {
    ...
    it('creates missing client notification and matches existing client case', async () => {
        vi.mocked(getCurrentReferrerPortalAccess).mockResolvedValueOnce({
            ok: true,
            sessionUserId: 'user-1',
            referrer: { id: 'ref-1', firstName: 'William', lastName: 'Maesela' },
        });
        vi.mocked(prisma.case.findFirst).mockResolvedValueOnce({ id: 'case-99', fileNumber: 'ZDM-99' } as never);
        ...
        vi.mocked(prisma.referrerMissingClientReport.create).mockResolvedValueOnce({ id: 'report-1' } as never);

        const res = await POST(req({
            clientName: 'Sipho Nkosi',
            idNumber: '8001015009087',
            notes: 'Referred on Monday.',
        }));
        const json = await res.json();

        expect(res.status).toBe(201);
        expect(json.success).toBe(true);
        expect(json.matchedCase).toEqual({ id: 'case-99', fileNumber: 'ZDM-99' });
