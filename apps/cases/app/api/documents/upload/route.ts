...
export async function POST(request: Request) {
    try {
        ...
        logger.info(`[UPLOAD_TRACE] B2BUser: ${isB2BPartner}, B2BCase: ${isB2BCase}, B2BProj: ${isB2BProject}, Flag: ${skipAnalysisRaw}`);
        ...
        for (const file of files) {
            ...
            } else if (file.fieldName === 'file_PAYSLIP') {
                docType = 'PAYSLIP';
            ...
            if (isCombined && combinedDocBase64) {
                ...
                try {
                    ...
                    try {
                        ...
                        throw new Error(`AI Extraction failed: ${aiError instanceof Error ? aiError.message : String(aiError)}`);
                    }
                }
            }
        ...
        // --- CASE AUTOMATION TRIGGER: Fire when a relevant document is uploaded ---
        ...
        if (hasRelevantDoc) {
            import('@zenowethu/shared-lib/src/ai/case-automation-trigger').then(({ runCaseAutomationTrigger }) => {
                runCaseAutomationTrigger(caseId, 'DOCUMENT_UPLOADED').catch(err => {
