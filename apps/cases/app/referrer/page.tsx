...
const REFERRAL_FILTERS: Record<ReferralFilterId, { label: string; matches: (row: ReferralRow) => boolean }> = {
    ...
    'in-progress': { label: 'In progress', matches: (row) => !row.isSettled && !row.isCompleted && row.statusTone !== 'lost' },
    'in-progress-this-month': { label: 'In progress this month', matches: (row) => !row.isSettled && !row.isCompleted && row.statusTone !== 'lost' && isInCalendarMonth(row.createdAt, 0) },
    'in-progress-last-month': { label: 'In progress last month', matches: (row) => !row.isSettled && !row.isCompleted && row.statusTone !== 'lost' && isInCalendarMonth(row.createdAt, 1) },
    'settled-not-completed': { label: 'Settled (not completed)', matches: (row) => row.isSettled && !row.isCompleted },
    'settled-not-completed-this-month': { label: 'Settled this month', matches: (row) => row.isSettled && !row.isCompleted && isInCalendarMonth(row.settledAt, 0) },
    'settled-not-completed-last-month': { label: 'Settled last month', matches: (row) => row.isSettled && !row.isCompleted && isInCalendarMonth(row.settledAt, 1) },
    ...
    'work-pipeline-settled': { label: 'Settled (awaiting work)', matches: (row) => row.isSettled && !row.isCompleted },
    ...
    'settled-paid': { label: 'Settled Amount', matches: (row) => row.isSettled && row.totalPaid > 0 },
    'settled-paid-this-month': { label: 'Settled Paid this month', matches: (row) => row.isSettled && row.paidThisMonth > 0 },
    'settled-paid-last-month': { label: 'Settled Paid last month', matches: (row) => row.isSettled && row.paidLastMonth > 0 },
    'completed-outstanding': { label: 'Outstanding Balance (Completed files)', matches: (row) => row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0 },
    'completed-outstanding-this-month': { label: 'Outstanding Balance (Completed files) this month', matches: (row) => row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0 && isInCalendarMonth(row.completedAt, 0) },
    'completed-outstanding-last-month': { label: 'Outstanding Balance (Completed files) last month', matches: (row) => row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0 && isInCalendarMonth(row.completedAt, 1) },
    'expected-revenue': { label: 'Expected Revenue files', matches: (row) => (row.quoteStatuses && row.quoteStatuses.some((status) => ['ACCEPTED', 'CONVERTED', 'PAID', 'PARTIALLY_PAID'].includes(status))) || (row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0) },
    'expected-revenue-this-month': { label: 'Expected Revenue files this month', matches: (row) => (row.quoteStatuses && row.quoteStatuses.some((status) => ['ACCEPTED', 'CONVERTED', 'PAID', 'PARTIALLY_PAID'].includes(status)) && isInCalendarMonth(row.quoteDate, 0)) || (row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0 && isInCalendarMonth(row.completedAt, 0)) },
    'expected-revenue-last-month': { label: 'Expected Revenue files last month', matches: (row) => (row.quoteStatuses && row.quoteStatuses.some((status) => ['ACCEPTED', 'CONVERTED', 'PAID', 'PARTIALLY_PAID'].includes(status)) && isInCalendarMonth(row.quoteDate, 1)) || (row.isCompleted && (row.quoteTotal ?? 0) - row.totalPaid > 0 && isInCalendarMonth(row.completedAt, 1)) },
};
        ...
        ? [
            ...
            },
            {
                ...
                stats: [
                    ...
                    {
                        id: 'in-progress-this-month' as ReferralFilterId,
                        label: calendarMonthName(0),
                    ...
                    { id: 'in-progress-last-month' as ReferralFilterId, label: calendarMonthName(1), value: discountSummary.inProgressLastMonth + discountSummary.settledNotCompletedLastMonth },
                ],
                breakdown: [
