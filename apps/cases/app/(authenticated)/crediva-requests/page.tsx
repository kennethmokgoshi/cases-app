...
export default function CredivaRequestsPage() {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/crediva/requests");
  ...
  const handleConvert = async (id: string) => {
    ...
    setConverting(id);
    try {
      const res = await fetch(`/api/crediva/requests/${id}/convert`, { method: "POST" });
      const data = await res.json();
