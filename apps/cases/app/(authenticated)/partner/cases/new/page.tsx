...
function NewCaseWithAIComponent() {
    ...
    // B2B only for partner page
    const acquisitionType = 'B2B' as const; // Locked to B2B for partners

    // Auto-select based on partnerIdParam when projects are loaded
    useEffect(() => {
        if (partnerIdParam && parentProjects.length > 0) {
            // Try to find matching project, prioritizing B2B typed projects
            let match = null;
            const lowerParam = partnerIdParam.toLowerCase();

            // Helper to find project with preference for B2B
            const findProject = (searchStr: string) => {
                // First try direct B2B match
                const b2bMatch = parentProjects.find(p =>
                    p.name.toLowerCase().includes(searchStr) && p.clientType === 'B2B'
                );
                if (b2bMatch) return b2bMatch;

                // Fallback to any match
                return parentProjects.find(p => p.name.toLowerCase().includes(searchStr));
            };

            // Map partner IDs to expected project name fragments
            if (lowerParam.includes('letsatsi')) {
                match = findProject('letsatsi');
            } else if (lowerParam.includes('shosholoza')) {
                match = findProject('shosholoza');
            } else if (lowerParam.includes('future')) {
                match = findProject('future');
            } else {
                // Generic fallback: try to find exact name match or fuzzy match
                match = parentProjects.find(p => p.id === partnerIdParam) || findProject(lowerParam);
            }
    ...
    logger.info('Is B2B Partner:', isB2BPartner);
        ...
        async function fetchProjects() {
            try {
                ...
                if (!partnerIdParam && uniqueProjects.length > 0) {
                    const firstB2BProject = uniqueProjects.find(p => p.clientType === 'B2B');
                    if (firstB2BProject) {
                        setSelectedParentId(firstB2BProject.id);
                    }
                }
            } catch (error) {
    ...
    // Auto-select first B2B partner for B2B Partner users
    useEffect(() => {
        if (isB2BPartner && parentProjects.length > 0 && !selectedParentId && !partnerIdParam) {
            const firstB2BProject = parentProjects.find(p => p.clientType === 'B2B');
            if (firstB2BProject) {
                setSelectedParentId(firstB2BProject.id);
            }
        }
    }, [isB2BPartner, parentProjects, selectedParentId, partnerIdParam]);

    // Reset parent selection when acquisition type changes (but not for B2B partners)
    useEffect(() => {
        if (!isB2BPartner) {
            setSelectedParentId('');
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="mb-8">
                <Link href="/partner/cases" className="text-zeno-cyan hover:text-cyan-300 text-sm mb-4 inline-block">
                    ← Back to Cases
                </Link>
            ...
            {step === 1 && (
                <div className="bg-zeno-blue/20 rounded-xl border border-white/5 p-6">
                    ...
                    {/* Source Type - Fixed to B2B for Partners */}
                    <div className="mb-6">
                        <div className="p-4 rounded-lg border-2 bg-indigo-600/30 border-indigo-500">
                            <div className="text-lg font-semibold text-white">B2B Partner Case</div>
                            <div className="text-xs mt-1 text-gray-300">No R350 registration fee required</div>
                        </div>
                    </div>

                    {/* 3. Main Source Project */}
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-400 mb-2">
                            3. Main Source <span className="text-red-400">*</span>
                            {filteredParentProjects.length === 0 && parentProjects.length > 0 && (
                                <span className="text-yellow-400 text-xs ml-2">
                                    (No partner sources configured)
                                </span>
                            )}
                        </label>
                        <select
                            ...
                            required
                        >
                            <option value="">Choose partner source...</option>
                    ...
                    {selectedParentId && (
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-400 mb-2">
                                4. Branch/Subproject <span className="text-gray-600">(Optional)</span>
                            </label>
                            <select
                        ...
                        <div className="mb-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                <button
                                    ...
                                    className={`p-4 rounded-xl border transition-all text-left group ${!formData.isJointApplication ? 'bg-indigo-600/20 border-indigo-500 shadow-lg shadow-indigo-900/20' : 'bg-zeno-navy border-white/10 hover:border-white/30'}`}
                                >
                            ...
                            <div className="p-4 bg-zeno-navy/50 rounded-lg border border-white/5">
                                ...
                                <p className="text-sm text-gray-400 mt-2">
                                    Services: <span className="text-zeno-cyan">{selectedServices?.length || 0} selected</span>
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="bg-zeno-blue/20 rounded-xl border border-white/5 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div className="space-y-4">
                            ...
                            {/* Joint Application Toggle (Review Step) */}
                            ...
                            {formData.isJointApplication && (
                                <div className="mt-4 p-4 bg-indigo-900/10 rounded-lg border border-indigo-500/20 animate-in fade-in slide-in-from-top-2">
