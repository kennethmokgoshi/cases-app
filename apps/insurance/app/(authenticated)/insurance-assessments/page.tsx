...
function InsuranceAssessmentsContent() {
  ...
  const fetchAssessments = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      // search is used as case file number filter
      if (search) params.set('search', search)

      const res = await fetch(`/api/insurance/assessments?${params}`)
      if (res.ok) {
        ...
        setAssessments(list)
        setTotal(tot)
        setPages(Math.max(1, Math.ceil(tot / PAGE_SIZE)))
        setPage(pg)

        // Derive stats from the full count (fetch all non-cancelled for stats)
        const statsRes = await fetch('/api/insurance/assessments?take=1000&skip=0')
        if (statsRes.ok) {
          const statsData = await statsRes.json()
          const all: Assessment[] = statsData.assessments ?? []
          const accepted = all.filter(a => a.status === 'ACCEPTED').length
          const totalMonthlySavings = all.reduce((sum, a) => sum + Number(a.monthlySavings ?? 0), 0)
          setStats({ total: statsData.total ?? all.length, accepted, totalMonthlySavings })
        }
      }
    } catch (err) {
      logger.error(err)
