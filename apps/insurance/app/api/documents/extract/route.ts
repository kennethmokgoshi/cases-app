...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (documentId) {
            ...
            logger.info({ documentId: existingDoc.id, fileName: existingDoc.fileName }, '🔄 Extract API using existing document:');
        } else {
            // Use uploaded file
            if (!file) {
            ...
            logger.info({ caseId, fileName: file.name }, '📤 Extract API called with new file:');
        ...
        for (const extractedDoc of extraction.extractedDocuments) {
            ...
            if (extractedDoc.type === 'CREDIT_REPORT') {
                try {
                    ...
                    logger.error({ error: err }, '⚠️ Detailed extraction failed, falling back to combined summary:');
        ...
        // 7. Trigger re-underwriting if insurance assessment exists
        ...
        if (assessment) {
            try {
                await runAutomatedUnderwriting(assessment.id, session.user.id);
                logger.info({ assessmentId: assessment.id }, '🔄 Re-underwriting triggered after OCR extraction');
            } catch (err) {
                logger.warn({ err, assessmentId: assessment.id }, 'Re-underwriting after OCR failed (non-blocking)');
            }
        }
        ...
        logger.error({ error }, '❌ Document extraction error:');
        ...
        try {
            ...
            logger.error({ error: activityErr }, 'Failed to log activity:');
        }
