...
export async function GET(request: Request) {
    try {
        ...
        // DOMAIN FILTER: Only return cases with an Insurance Assessment
        const where: any = {
        ...
        if (urlYear || urlMonth || urlSource) {
            ...
            if (urlYear && urlMonth) {
                ...
                targetIds = allProjects
                    .filter(p => p.name === urlSource && (p.parentId === null || (projectMap.get(p.parentId!) as any)?.type === 'ROOT'))
                    .map(p => p.id);
            }
        ...
        const getFullPath = (id: string): string => {
            let curr: { id: string; name: string; parentId: string | null; type: string } | undefined = projectMap.get(id) as { id: string; name: string; parentId: string | null; type: string } | undefined;
            if (!curr) return '';
