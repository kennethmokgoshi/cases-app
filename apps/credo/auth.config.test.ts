...
import { CREDO_IDLE_TIMEOUT_SECONDS } from "./lib/session-security";

const authorized = authConfig.callbacks?.authorized;

if (!authorized) {
  throw new Error("Expected Credo authConfig to define an authorized callback");
}

function authorizePath(pathname: string, loggedIn: boolean): ReturnType<typeof authorized> {
  const requestUrl = new URL(`https://credo.test${pathname}`);
...
describe("Credo auth config", () => {
  it("uses a five-minute JWT session lifetime", () => {
    expect(authConfig.session?.maxAge).toBe(CREDO_IDLE_TIMEOUT_SECONDS);
    expect(authConfig.jwt?.maxAge).toBe(CREDO_IDLE_TIMEOUT_SECONDS);
  });

  it("blocks protected Credo pages when no consumer is signed in", () => {
