    ...
    try {
        ...
        if (combinedDoc) {
            ...
            if (!existsSync(filePath)) {
                ...
                return NextResponse.json({ error: 'Combined document file not found' }, { status: 404 });
            }

            const fileBuffer = await readFile(filePath);
            const base64Pdf = fileBuffer.toString('base64');

            logger.info('🔄 Running comparison analysis on combined document...');
            analysis = await analyzeCombinedDocument(base64Pdf);
        } else {
            ...
            for (const doc of caseRecord.documents) {
                ...
                if (existsSync(filePath)) {
                    ...
                    logger.warn(`⚠️ Batch document not found at: ${filePath}`);
                }
            }
            ...
            logger.info('🔄 Running batch comparison analysis on', docsForAnalysis.length, 'documents...');
            analysis = await batchAnalyzeDocuments(docsForAnalysis);
        }
        ...
        const personalFields = [
            { field: 'idNumber', label: 'ID Number', oldValue: caseRecord.client.idNumber, newValue: analysis.id?.idNumber || null },
            { field: 'lastName', label: 'Surname', oldValue: caseRecord.client.lastName, newValue: analysis.id?.surname || null },
            { field: 'firstName', label: 'Full Names', oldValue: caseRecord.client.firstName, newValue: analysis.id?.names || null },
            { field: 'phone', label: 'Cell Number', oldValue: caseRecord.client.phone, newValue: analysis.poa?.cellNumber || analysis.creditReport?.consumer?.cellNumber || null },
        ];
        ...
        const creditFields = [
            { field: 'totalDebtAmount', label: 'Balance Exposure', oldValue: caseRecord.totalDebtAmount ? Number(caseRecord.totalDebtAmount) : null, newValue: analysis.creditReport?.summary?.totalDebt || null },
            { field: 'totalMonthlyInstallment', label: 'Monthly Instalment', oldValue: caseRecord.totalMonthlyInstallment ? Number(caseRecord.totalMonthlyInstallment) : null, newValue: analysis.creditReport?.summary?.totalInstallment || null },
            { field: 'openAccounts', label: 'Active Accounts', oldValue: caseRecord.openAccounts, newValue: analysis.creditReport?.summary?.activeAccounts || null },
            { field: 'closedAccounts', label: 'Closed Accounts', oldValue: caseRecord.closedAccounts, newValue: analysis.creditReport?.summary?.closedAccounts || null },
        ];
        ...
        // Restructuring Info - Check both creditReport.debtRestructuring and root-level debtRestructuring
        const debtRestructuring = analysis.creditReport?.debtRestructuring || analysis.debtRestructuring || {};
        ...
        return NextResponse.json({
            error: error instanceof Error ? error.message : 'Failed to run comparison analysis'
        }, { status: 500 });
    }
}
