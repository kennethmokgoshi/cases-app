/**
 * One-time backfill: give every Credo consumer account WITHOUT a password the
 ...
 *   npx tsx scripts/backfill-credo-default-passwords.ts --dry-run
 *   npx tsx scripts/backfill-credo-default-passwords.ts
...
async function main() {
  console.log(`\n=== Credo Default Password Backfill (${DRY_RUN ? 'DRY RUN' : 'LIVE'}) ===\n`);
